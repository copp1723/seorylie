...
const TEMPO_ENDPOINT =
  process.env.TEMPO_ENDPOINT || "http://localhost:4318/v1/traces";
const TEMPO_HEADERS = process.env.TEMPO_HEADERS
  ? JSON.parse(process.env.TEMPO_HEADERS)
  : {};
...
export function initTracing() {
  ...
  sdk
    ...
    .then(() => {
      diag.info(
        `OpenTelemetry tracing initialized for ${SERVICE_NAME} (${ENVIRONMENT})`,
      );
 ...
 */
export function createSandboxSpan(
  name: string,
  sandboxId: number,
  userId?: number,
) {
...
export function createToolSpan(
  ...
  if (params) {
    ...
    Object.entries(params).forEach(([key, value]) => {
      ...
      if (
        ["password", "token", "secret", "key", "credential"].some((s) =>
 ...
 */
export function createCrossServiceSpan(
  name: string,
