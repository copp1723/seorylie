...
import { BlogRequestSchema } from "../shared/schemas/blog-request-schema";
...
const withSpan = async (name: string, fn: Function) =>
  fn({ setAttributes: () => {}, setAttribute: () => {}, end: () => {} });
const createCrossServiceSpan = () => ({
  setAttributes: () => {},
  setAttribute: () => {},
  end: () => {},
});
...
class ObservableWebSocketServer {
  ...
  private handleConnection(ws: WebSocket, request: any): void {
    ...
    withSpan("websocket_connection_established", async (span) => {
      span.setAttributes({
        ...
        "websocket.connection.user_agent":
  ...
  private handleDisconnection(
    ...
    withSpan("websocket_disconnection", async (span) => {
      ...
      const duration =
        (new Date().getTime() - connection.connectedAt.getTime()) / 1000;
