...
router.post('/analyze', 
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const formattedConversation = conversation_history.map((msg: any) => 
      ...
      let dossier;
      try {
        const dossierPromise = generateHandoverDossier(formattedConversation, customerScenario);
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Dossier generation timed out')), 10000);
        });
      ...
      const handoverRecommended = 
        urgency === 'high' || 
        dossier.customerInsights?.some((insight: any) => 
      ...
      return res.json({
        ...
        urgency,
        dossier: {
          customer_summary: dossier.conversationSummary || 'No summary available',
          vehicle_interests: dossier.vehicleInterests || [],
          key_insights: dossier.customerInsights || [],
          suggested_approach: dossier.suggestedApproach || 'No approach suggested',
          next_steps: ['Contact customer promptly', 'Follow up within 24 hours'],
          escalation_reason: dossier.escalationReason || trigger_reason || 'Not specified'
        },
      ...
      return res.status(500).json({
        ...
        fallback: {
          ...
          dossier: {
            ...
            vehicle_interests: [],
            key_insights: [],
            suggested_approach: 'Recommend contacting sales team for assistance',
            next_steps: ['Contact customer promptly', 'Apologize for any inconvenience'],
