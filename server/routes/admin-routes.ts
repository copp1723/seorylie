...
const checkDealershipAccess = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
router.put("/dealerships/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      existingSubdomain[0].id !== dealershipId
    ) {
      return res
        .status(400)
        .json({ error: "Subdomain is already taken by another dealership" });
    }
 ...
 */
router.put(
  "/dealerships/:id/branding",
  checkDealershipAccess,
...
router.delete("/dealerships/:id", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      message: "Dealership and all associated data deleted successfully",
    });
  } catch (error) {
