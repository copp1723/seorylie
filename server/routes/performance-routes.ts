...
import { ResponseHelper, asyncHandler, AppError, ErrorCode } from '../utils/error-codes';
...
router.get('/conversations', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isNaN(dealershipId)) {
    throw new AppError(ErrorCode.INVALID_INPUT, 'dealershipId is required and must be a number.', { traceId });
  }

  const cacheKeyOptions = { dealershipId, page, limit, status: status || 'all' };
  const cacheKey = createCacheKey('conversations_list', cacheKeyOptions);
  const tags = ['conversations_list', dealershipTag(dealershipId), statusTag(status)];

  const result = await cacheService.getOrSet(
    cacheKey,
    async () => {
      logger.info('Cache miss - fetching conversations from database', { ...cacheKeyOptions, traceId });
      ...
      return await executeQuery(async () => {
        ...
        const conversations = await db.execute(sql.raw(queryText, ...queryParams));
        ...
        const totalResult = await db.execute(sql.raw(countQueryText, ...countParams));
  ...
  ResponseHelper.success(res, result, 'Conversations retrieved successfully.');
}));
...
router.get('/conversations/:id', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (isNaN(conversationId)) {
    throw new AppError(ErrorCode.INVALID_INPUT, 'Conversation ID must be a number.', { traceId });
  }
  ...
  const result = await cacheService.getOrSet(
    cacheKey,
    async () => {
      logger.info('Cache miss - fetching conversation details from database', { ...cacheKeyOptions, traceId });

      return await executeQuery(async () => {
        const conversationResult = await db.execute(sql.raw(
        ...
        const messages = await db.execute(sql.raw(
          `SELECT
            id,
            content,
            is_from_customer,
          ...
          LIMIT $2`, conversationId, messageLimit
        ));
  ...
  if (!result) {
    throw new AppError(ErrorCode.NOT_FOUND, 'Conversation not found.', { traceId, conversationId });
  }

  ResponseHelper.success(res, result, 'Conversation details retrieved successfully.');
}));
...
router.get('/search', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!searchTerm) {
    throw new AppError(ErrorCode.INVALID_INPUT, 'Search term is required.', { traceId });
  }
  ...
  const results = await cacheService.getOrSet(
    cacheKey,
    async () => {
      logger.info('Cache miss - searching conversations in database', { ...cacheKeyOptions, traceId });
      ...
      return await executeQuery(async () => {
        ...
        return await db.execute(sql.raw(
          ...
          LIMIT $3`, dealershipId, searchPattern, limit
        ));
      }, { traceId });
    },
...
router.get('/analytics', asyncHandler(async (req: Request, res: Response) => {
  ...
  const tags = [KPI_CACHE_TAG_PREFIX, 'analytics', 'conversation_analytics', dealershipTag(dealershipId)];

  const analytics = await cacheService.getOrSet(
    cacheKey,
    async () => {
      logger.info('Cache miss - generating conversation analytics (KPI)', { ...cacheKeyOptions, traceId });
      ...
      return await executeQuery(async () => {
        ...
        return await db.execute(sql.raw(
          ...
          ORDER BY date`, dealershipId, startDate.toISOString()
        ));
      }, { traceId });
    },
  ...
  ResponseHelper.success(res, analytics, 'Conversation analytics retrieved successfully.');
}));
...
router.post('/cache/clear', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (tag) {
    ...
    ResponseHelper.success(res, { invalidatedTag: tag }, `Cache cleared by tag: ${tag}`);
  } else if (pattern) {
    await cacheService.invalidatePattern(pattern, { prefix });
    logger.info(`Cache cleared by pattern: ${pattern}`, { prefix, traceId });
    ResponseHelper.success(res, { invalidatedPattern: pattern, prefix }, `Cache cleared by pattern: ${pattern}`);
  } else {
    await cacheService.clear(prefix);
    logger.info('Cache cleared', { prefix: prefix || 'all', traceId });
    ResponseHelper.success(res, { clearedPrefix: prefix || 'all' }, prefix ? `Cache cleared for prefix: ${prefix}` : 'Entire cache cleared');
  }
}));
...
router.get('/cache/stats', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    ResponseHelper.success(res, stats, 'Cache statistics retrieved successfully.');
  } catch (error) {
    // This path should ideally not be hit if cacheService.getInfo() is robust
    // but as a fallback:
    logger.error('Failed to get cache stats from getInfo', error, { traceId });
    const basicStats = cacheService.getStats(); // Fallback to basic stats
     ResponseHelper.success(res, basicStats, 'Basic cache statistics retrieved (getInfo failed).');
  }
}));
