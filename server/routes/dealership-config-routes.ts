...
router.get("/mode", async (req, res) => {
  try {
    ...
    const dealershipId =
      Number(req.query.dealershipId) || (req.session as any).dealershipId || 1;
...
router.get("/config", async (req, res) => {
  try {
    ...
    const config =
      await dealershipConfigService.getDealershipConfig(dealershipId);
...
router.post("/mode", async (req, res) => {
  try {
    // Get dealership ID from query parameter, request body, or default to 1 for testing
    const dealershipId =
      Number(req.body.dealershipId) || (req.session as any).dealershipId || 1;
...
router.get("/working-hours/status", async (req, res) => {
  try {
    ...
    const isDirectMode =
      await dealershipConfigService.isDirectAgentMode(dealershipId);
    ...
    const isWithinWorkingHours =
      await dealershipConfigService.isWithinWorkingHours(dealershipId);

    // Get current working hours
    const workingHours =
      await dealershipConfigService.getAgentWorkingHours(dealershipId);
