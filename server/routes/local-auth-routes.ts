...
router.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    if (!user.password) {
      logger.warn(
        `Login attempt failed: No password set for user ${loginIdentifier}`,
      );
      return res
        .status(401)
        .json({ error: "Please use magic link authentication" });
    }
    ...
    if (!isValidPassword) {
      logger.warn(
        `Login attempt failed: Invalid password for user ${loginIdentifier}`,
      );
      return res.status(401).json({ error: "Authentication failed" });
    }
