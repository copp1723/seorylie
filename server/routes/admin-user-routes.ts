...
const userUpdateSchema = z.object({
  ...
  name: z.string().optional(),
  role: z
    .enum(["super_admin", "dealership_admin", "manager", "user"])
    .optional(),
  ...
  try {
    ...
    if (userData.username) {
      const existingUsername = await db
        ...
        .from(users)
        .where(
          and(eq(users.username, userData.username), ne(users.id, userId)),
        );
    ...
    // If dealership_id is provided, check if it exists
    if (
      userData.dealership_id !== undefined &&
...
router.post("/invitations", async (req: Request, res: Response) => {
  try {
    ...
    const existingInvitation = await db
      .select({ id: magicLinkInvitations.id })
...
router.get("/invitations", async (req: Request, res: Response) => {
  try {
    ...
    if (dealershipIds.length > 0) {
      const dealershipResults = await db
        .select({
          id: dealerships.id,
          name: dealerships.name,
        })
    ...
    const enrichedInvitations = allInvitations.map((inv) => ({
      ...inv,
      dealership_name:
        inv.dealership_id !== null
          ? dealershipMap.get(inv.dealership_id)
          : null,
      invited_by_name:
        inv.invited_by !== null ? inviterMap.get(inv.invited_by) : null,
    }));
