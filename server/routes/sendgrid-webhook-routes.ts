...
function verifyWebhookSignature(req: Request): boolean {
  ...
  if (
    !process.env.SENDGRID_VERIFICATION_ENABLED ||
    process.env.SENDGRID_VERIFICATION_ENABLED !== "true"
  ) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info("Processing SendGrid inbound email", {
        ...
        attachments: req.body.attachment_info
          ? JSON.parse(req.body.attachment_info)
          : [],
      });
      ...
      const adfAttachments = emailData.attachments.filter(
        (att: any) =>
      ...
      for (const attachment of adfAttachments) {
        try {
          ...
          if (!xmlContent) {
            logger.warn("Attachment content not found", {
      ...
      logger.error("SendGrid webhook processing error", {
        error: error.message,
      });
