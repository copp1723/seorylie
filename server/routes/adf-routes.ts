 ...
 */
async function getDealershipFromEmail(
  toEmail: string,
  fromEmail: string,
): Promise<number | null> {
  try {
    ...
    */
    ...
    if (toDomain.includes(".cleanrylie.com") && toDomain.startsWith("adf-")) {
      const identifier = toDomain
        .replace("adf-", "")
        .replace(".cleanrylie.com", "");
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info("ADF lead status updated", {
        leadId,
        oldStatus: existingLead.leadStatus,
        newStatus: status,
        notes,
      });
      ...
      logger.error("Error updating ADF lead status", {
        error,
    ...
    try {
      ...
      if (!xmlContent) {
        ...
        return res.status(400).json({
          ...
          message:
            'Provide XML content in request body or as "xml"/"xmlContent" field',
          timestamp: new Date().toISOString(),
        });
      }
      ...
      const result = await adfService.processAdfXml(
        xmlContent,
        source,
        dealershipId,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (result.mappedLead) {
        ...
        const existingLead = await db.query.adfLeads.findFirst({
          where: eq(
            adfLeads.deduplicationHash,
            result.mappedLead.deduplicationHash!,
          ),
          columns: { id: true },
        });
        ...
        const [newLead] = await db
          .insert(adfLeads)
          .values(result.mappedLead as any)
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const timeframe =
        (req.query.timeframe as "hour" | "day" | "week") || "day";
      ...
      // Get statistics
      const [totalLeads, processedLeads, failedLeads, emailsInQueue] =
        await Promise.all([
          db
            .select({ count: count() })
            .from(adfLeads)
      ...
      const stats = {
        ...
        successRate: totalLeads[0]?.count
          ? (
              ((processedLeads[0]?.count || 0) / totalLeads[0].count) *
              100
            ).toFixed(2) + "%"
          : "0%",
...
router.get(
  ...
  [
    ...
    query("status")
      .optional()
      .isIn(["pending", "processed", "failed", "quarantined"]),
  ],
  async (req: Request, res: Response) => {
    try {
      const errors = validationResult(req);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info("Email processing retry initiated", {
        queueId,
        attempt: queueEntry.processingAttempts + 1,
      });
...
const verifySendGridSignature = (req: Request, res: Response, next: any) => {
  ...
  if (signature && timestamp) {
    ...
    logger.warn(
      "SendGrid webhook received without signature - ensure webhook security is configured",
    );
  }
...
const parseWebhookBody = (req: Request, res: Response, next: any) => {
  ...
  if (req.get("content-type")?.includes("multipart/form-data")) {
    ...
    res
      .status(400)
      .json({ error: "Expected multipart/form-data content type" });
  }
};
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const dealershipId = await getDealershipFromEmail(
        req.body.to,
        req.body.from,
      );
      ...
      for (let i = 1; i <= attachmentCount; i++) {
        ...
        if (attachmentInfo && attachmentData) {
          ...
          if (
            info.filename?.toLowerCase().endsWith(".xml") ||
      ...
      if (xmlAttachments.length === 0) {
        ...
        return res
          .status(200)
          .json({ success: true, message: "No ADF attachments found" });
      }
