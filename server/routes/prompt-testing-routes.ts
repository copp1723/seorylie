...
router.post("/test", async (req, res) => {
  try {
    ...
    const analysis = await generateResponseAnalysis(
      processedPrompt,
      testMessage,
    );
  ...
  try {
    ...
    if (!conversationHistory && !customerScenario) {
      return res.status(400).json({
        error: "Conversation history or customer scenario is required",
      });
    }
    ...
    // Generate handover dossier
    const handoverDossier = await generateHandoverDossier(
      conversationText,
      customerScenario || conversationText,
    );
