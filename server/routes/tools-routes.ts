...
router.get("/agents/:agentId/tools", auth, async (req, res) => {
  try {
    ...
    if (isNaN(agentId)) {
      return res
        .status(400)
        .json({ success: false, error: "Invalid agent ID" });
    }

    // Get agent tools
    const agentToolsResult = await db
      .select({
        id: agentTools.id,
    ...
    logger.error(
      `Error fetching agent tools for agent ${req.params.agentId}:`,
      error,
    );
    return res.status(500).json({
      success: false,
...
router.post("/agents/:agentId/tools", auth, async (req, res) => {
  try {
    ...
    await db.insert(agentTools).values(toolsToAdd);
    ...
    logger.error(
      `Error enabling tools for agent ${req.params.agentId}:`,
      error,
    );
    return res.status(500).json({
      success: false,
...
router.delete("/agents/:agentId/tools/:toolId", auth, async (req, res) => {
  try {
    ...
    if (isNaN(agentId) || isNaN(toolId)) {
      return res
        .status(400)
        .json({ success: false, error: "Invalid agent ID or tool ID" });
    }
    ...
    logger.error(
      `Error disabling tool ${req.params.toolId} for agent ${req.params.agentId}:`,
      error,
    );
    return res.status(500).json({
      success: false,
