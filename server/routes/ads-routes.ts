...
const trackMetrics = (route: string) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.send = function (body) {
      routeMetrics.requestCounter
        .labels(route, req.method, res.statusCode.toString())
        .inc();
      routeMetrics.requestDuration
...
const handleAdsApiError = (
  ...
  if (err instanceof AuthenticationError) {
    logger.warn("Google Ads authentication error", {
      error: err.message,
      path: req.path,
    });
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Generate OAuth URL
      const authUrl = adsApiService.getAuthUrl(
        userId,
        dealershipId,
        sandboxId,
        state,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isNaN(userId)) {
        return res
          .status(400)
          .json({ error: "invalid_user_id", message: "User ID is required" });
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isNaN(accountId)) {
        return res
          .status(400)
          .json({ error: "invalid_account_id", message: "Invalid account ID" });
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!campaign) {
        return res
          .status(404)
          .json({ error: "campaign_not_found", message: "Campaign not found" });
      }
