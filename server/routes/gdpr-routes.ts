...
const validateDataOwnership = async (
  ...
  try {
    ...
    if (!userHasAccess) {
      ...
      return res
        .status(403)
        .json({ error: "Not authorized to access this data" });
    }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res.status(200).json({
        message:
          "If your email exists in our system, all associated personal data has been anonymized. This process may take up to 30 days to complete across all our systems.",
      });
    } catch (error) {
      ...
      return res.status(500).json({
        message:
          "An error occurred processing your request. Please try again later or contact our support team.",
      });
    }
  },
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Server error checking consent status" });
    }
  },
);
