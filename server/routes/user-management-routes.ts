...
router.post('/dealerships/:dealershipId/invitations', async (req, res) => {
  try {
    ...
    if (req.user.role !== 'super_admin' && req.user.role !== 'dealership_admin') {
    ...
    if (role === 'dealership_admin' && req.user.role !== 'super_admin') {
      return res.status(403).json({ error: 'Only super admins can create dealership admin accounts' });
    }
...
router.delete('/dealerships/:dealershipId/invitations/:invitationId', async (req, res) => {
  try {
    const dealershipId = parseInt(req.params.dealershipId);
