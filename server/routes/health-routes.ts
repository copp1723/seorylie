...
router.get('/detailed', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const healthCheckPromise = healthCheckService.runAllChecks();
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Detailed health check timeout')), 5000);
    });
    ...
    const subsystems = report.checks.reduce((acc, check) => {
      acc[check.name] = {
        status: check.status === 'healthy' ? 'ok' : check.status,
        responseTime: `${check.responseTime}ms`,
        message: check.message || (check.status === 'healthy' ? 'Operating normally' : 'See error details'),
...
router.get('/services', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Services health check timeout')), 5000);
    });
    
    const servicesHealth = await Promise.race([servicesHealthPromise, timeoutPromise]);
    const responseTime = Date.now() - startTime;
    ...
    const normalizedServices = Object.entries(servicesHealth.services).reduce((acc, [serviceName, serviceHealth]) => {
      acc[serviceName] = {
        ...serviceHealth,
        status: serviceHealth.status === 'healthy' ? 'ok' : serviceHealth.status
      };
      return acc;
    }, {} as Record<string, any>);

    res.status(statusCode).json({
      status: servicesHealth.status === 'healthy' ? 'ok' : servicesHealth.status, // DEP-009: Primary status
      success: statusCode === 200,
      timestamp: new Date(),
      responseTime: `${responseTime}ms`,
      ...
      summary: {
        ...
        healthy: Object.values(servicesHealth.services).filter(s => s.status === 'healthy').length,
        degraded: Object.values(servicesHealth.services).filter(s => s.status === 'degraded').length,
        unhealthy: Object.values(servicesHealth.services).filter(s => s.status === 'unhealthy').length
      }
    });
  } catch (error) {
    const responseTime = Date.now() - startTime;
...
router.get('/services/:serviceName', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await Promise.race([serviceCheckPromise, timeoutPromise]) as any;
    const responseTime = Date.now() - startTime;
    ...
    const normalizedStatus = result.health.status === 'healthy' ? 'ok' : result.health.status;
...
router.get('/metrics', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const metricsPromise = new Promise((resolve) => {
      ...
      if (memUsage.rss > 1024 * 1024 * 1024) { // > 1GB RSS
    ...
    const result = await Promise.race([metricsPromise, timeoutPromise]) as any;
    const responseTime = Date.now() - startTime;
...
router.get('/config', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await Promise.race([configCheckPromise, timeoutPromise]) as any;
    const responseTime = Date.now() - startTime;
...
router.get('/ready', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { ready, criticalServices, servicesHealth } = await Promise.race([readinessCheckPromise, timeoutPromise]);
    const responseTime = Date.now() - startTime;
...
router.get('/startup', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { started, checks } = await Promise.race([startupCheckPromise, timeoutPromise]);
    const responseTime = Date.now() - startTime;
    ...
    res.status(statusCode).json({
      ...
      timestamp: new Date(),
      responseTime: `${responseTime}ms`,
      message: started ? 'Application has started successfully' : 'Application startup incomplete',
      checks,
      summary: {
        total: checks.length,
...
router.get('/history', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const lastReport = await Promise.race([historyPromise, timeoutPromise]) as any;
    const responseTime = Date.now() - startTime;
