...
router.post('/api/v1/customers/:customerId/personas',
  body('name').isString(),
...
router.get('/api/v1/customers/:customerId/personas', async (req, res) => {
  const { customerId } = req.params;
  const items = await db.select().from(personas).where(eq(personas.dealershipId, parseInt(customerId,10)));
...
router.patch('/api/v1/personas/:personaId', async (req, res) => {
  ...
  await db.update(personas).set(update).where(eq(personas.id, parseInt(personaId,10)));
