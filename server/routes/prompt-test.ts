...
router.post('/system-handover', async (req: Request, res: Response) => {
  try {
    ...
    const dossier = {
      ...
      conversationSummary: "This is a test handover from the system prompt testing interface",
      urgency: "medium",
      customerInsights: [
    ...
    return res.status(500).json({ message: 'Server error processing test handover' });
  }
});
...
router.post('/test', async (req: Request, res: Response) => {
  try {
    ...
    if (validatedData.variantId) {
      ...
      return res.status(400).json({ error: 'Either variantId or systemPrompt must be provided' });
    }
    
    // Replace placeholders in the prompt content
    promptContent = promptContent
      .replace('[ARG-Agent Name]', 'Rylie')
      .replace('[ARG-Employer Name]', validatedData.dealershipContext.dealershipName)
      .replace('[ARG-Information About Employer]', validatedData.dealershipContext.brandTypes)
      .replace('[ARG-Employer Contact Details]', validatedData.dealershipContext.dealershipLocation);
      
    // Format vehicle inventory if provided
    if (validatedData.relevantVehicles && validatedData.relevantVehicles.length > 0) {
      const inventoryText = validatedData.relevantVehicles.map(vehicle => 
      ...
      promptContent = promptContent.replace('[INPUT-Product Inventory]', inventoryText);
    } else {
      promptContent = promptContent.replace('[INPUT-Product Inventory]', 'No specific inventory data available');
    }
    
    // Add customer name
    promptContent = promptContent.replace('[INPUT-CUSTOMER NAME]', validatedData.customerInfo.name);
    
    // Format conversation history if provided
    if (validatedData.conversationHistory && validatedData.conversationHistory.length > 0) {
      const historyText = validatedData.conversationHistory.map(msg => 
        `${msg.role === 'customer' ? 'Customer' : 'Agent'}: ${msg.content}`
      ).join('\n');
      promptContent = promptContent.replace('[INPUT-CONVERSATION]', historyText);
    } else {
      promptContent = promptContent.replace('[INPUT-CONVERSATION]', 'No prior conversation history');
    }
    ...
    const customerInsights = validatedData.formatOptions?.detectCustomerInsights ? [
      ...
      { key: 'Preferred Features', value: 'Safety, Fuel Efficiency', confidence: 0.92 },
    ...
    const handoverKeywords = ['speak to representative', 'talk to someone', 'agent', 'call me', 'contact me'];
    const lowercaseResponse = response.toLowerCase();
    const handoverRecommended = handoverKeywords.some(keyword => lowercaseResponse.includes(keyword));
    
    // Generate a handover dossier if requested
    const handoverDossier = handoverRecommended && validatedData.formatOptions?.generateHandoverDossier ? {
      ...
      vehicleInterests: validatedData.relevantVehicles?.[0] 
        ...
        : [],
      customerInsights,
      conversationSummary: "Customer is interested in a family-friendly SUV with good safety features and fuel economy. They're planning to purchase within the next two weeks and have a budget around $30,000.",
      recommendedActions: [
...
router.post('/handover', async (req: Request, res: Response) => {
  try {
    ...
    const handoverDossier = {
      ...
      interestLevel: 'High',
      vehicleInterests: validatedData.relevantVehicles?.slice(0, 2).map(vehicle => ({
        make: vehicle.make,
        model: vehicle.model,
        year: vehicle.year,
        trim: vehicle.trim,
      ...
      objections: [
        {
          topic: "Price",
          description: "Customer has mentioned concern about the starting price",
          resolution: "Highlight the value features and available financing options"
        }
      ],
      nextSteps: {
    ...
    return res.status(500).json({ error: 'Failed to generate handover dossier' });
  }
});
