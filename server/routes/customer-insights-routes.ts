...
router.post(
  "/dealerships/:dealershipId/customer-profiles",
  async (req, res) => {
    try {
...
router.post("/customer-profiles/:profileId/interactions", async (req, res) => {
  try {
    ...
    if (
      !req.user ||
      (req.user.dealership_id !== profile.dealership_id &&
        req.user.role !== "super_admin")
    ) {
      return res.status(403).json({ error: "Unauthorized" });
    }
    ...
    if (!conversationId || !interactionType) {
      return res
        .status(400)
        .json({ error: "Conversation ID and interaction type are required" });
    }
...
router.get("/customer-profiles/:profileId/preferences", async (req, res) => {
  try {
    ...
    const preferences = await analyzeCustomerPreferences(
      profileId,
      conversationId,
    );
