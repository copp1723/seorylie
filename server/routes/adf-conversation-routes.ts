...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await conversationService.verifyConversationAccess(
        conversationId,
        dealershipId,
      );
      ...
      const result =
        await conversationService.getConversationMessagesWithCursor(
          conversationId,
          options,
        );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Get lead context
      const leadContext = await conversationService.getLeadContext(
        conversation.adfLeadId,
      );
