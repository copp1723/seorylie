...
export class SMSChannelHandler extends BaseChannelHandler {
  ...
  async sendMessage(message: ChannelMessage): Promise<ChannelDeliveryResult> {
    try {
      ...
      const customerPhone =
        message.metadata?.customerPhone ||
        (await this.getCustomerPhone(message.customerId));
      ...
      return this.handleError(error, {
        customerPhone: message.metadata?.customerPhone
          ? twilioSMSService.maskPhoneNumber(message.metadata.customerPhone)
          : "unknown",
      });
    }
  }
  ...
  validateMessage(message: ChannelMessage): boolean {
    ...
    if (message.content.length > maxLength) {
      this.log(
        "warn",
        `SMS content exceeds maximum length: ${message.content.length}/${maxLength}`,
      );
      return false;
    }
    ...
    if (
      message.metadata?.customerPhone &&
      !this.isValidPhoneNumber(message.metadata.customerPhone)
    ) {
  ...
  private formatSMSContent(message: ChannelMessage): string {
    const dealershipName =
      this.configuration.settings?.dealershipName || "Your Dealership";
  ...
  private async handleInboundReply(
    from: string,
    message: string,
    to: string,
  ): Promise<void> {
    try {
      ...
      if (optOutKeywords.some((keyword) => lowerMessage.includes(keyword))) {
        await twilioSMSService.handleOptOut(
          this.configuration.dealershipId,
          from,
          "user_request",
        );
  ...
  private async forwardToConversationSystem(
    ...
    try {
      ...
      this.log("error", "Failed to forward SMS to conversation system", {
        error,
      });
    }
  }
