...
export class EmailChannelHandler extends BaseChannelHandler {
  ...
  async sendMessage(message: ChannelMessage): Promise<ChannelDeliveryResult> {
    try {
      ...
      const customerEmail =
        message.metadata?.customerEmail ||
        (await this.getCustomerEmail(message.customerId));
      ...
      return this.handleError(error, {
        customerEmail: message.metadata?.customerEmail
          ? this.maskEmail(message.metadata.customerEmail)
          : "unknown",
      });
    }
  }
  ...
  private getFromAddress(): string {
    ...
    if (settings?.fromEmail) {
      const fromName =
        settings.fromName || settings.dealershipName || "Rylie AI";
