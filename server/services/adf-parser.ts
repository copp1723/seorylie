...
import type { AdfXmlStructure, InsertAdfLead } from "@shared/index";
...
export class AdfSchemaValidator {
   ...
   */
  static validateAdfStructure(parsedXml: any): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (requestDate && !this.isValidDateFormat(requestDate)) {
      errors.push(
        `Invalid requestdate format: ${requestDate}. Expected ISO 8601 or MM/DD/YYYY format`,
      );
    }
    ...
    if (prospect.customer?.contact) {
      ...
      if (contact.name && !this.validateNameStructure(contact.name)) {
        errors.push(
          "Invalid customer name structure. Expected name parts with type attributes",
        );
      }
  ...
  private static isValidDateFormat(dateStr: string): boolean {
    ...
    if (
      isoFormat.test(dateStr) ||
      usFormat.test(dateStr) ||
      shortIsoFormat.test(dateStr)
    ) {
  ...
  private static validatePhoneStructure(phone: any): boolean {
    ...
    if (phone.$ && phone._) {
      return (
        this.isValidPhoneNumber(phone._) &&
...
export class AdfParser {
  ...
  async parseAdfXml(xmlContent: string): Promise<{
    ...
    try {
      ...
      const schemaValidation =
        AdfSchemaValidator.validateAdfStructure(parsedXml);
  ...
  private extractLeadData(parsedXml: AdfXmlStructure): Partial<InsertAdfLead> {
    ...
    if (prospect.customer?.contact) {
      ...
      if (contact.address) {
        ...
        lead.customerAddress =
          [addr.street, addr.apartment].filter(Boolean).join(", ") || null;
    ...
    if (prospect.vehicle) {
      ...
      lead.vehiclePrice = vehicle.price
        ? Math.round(parseFloat(vehicle.price) * 100)
    ...
    if (prospect.trade?.vehicle) {
      ...
      lead.tradeInValue = trade.value
        ? Math.round(parseFloat(trade.value) * 100)
        : null; // Convert to cents
    }
  ...
  private extractNameFromContact(contact: any): {
    ...
    if (contact.name.part) {
      const parts = Array.isArray(contact.name.part)
        ? contact.name.part
        : [contact.name.part];
  ...
  private extractPhoneFromContact(contact: any): string | null {
    ...
    if (Array.isArray(contact.phone)) {
      const voicePhone = contact.phone.find(
        (p: any) => p.$?.type === "voice" || !p.$?.type,
      );
  ...
  private parseRequestDate(dateStr: string): Date {
    ...
    logger.warn("Could not parse request date, using current date", {
      dateStr,
    });
    return new Date();
  }
