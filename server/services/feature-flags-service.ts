...
const DEFAULT_FLAGS: Record<string, FeatureFlag> = {
  [FeatureFlagNames.SANDBOX_PAUSE_RESUME]: {
    ...
    description:
      "Enables the ability to pause and resume agent sandbox execution",
    enabled: false,
    rolloutPercentage: 0,
...
class FeatureFlagsService extends EventEmitter {
      ...
      this.pubSubClient.on("message", (channel, message) => {
        if (channel === "feature_flags:updates") {
          try {
            ...
            logger.error("Error processing feature flag update message", {
              error,
            });
          }
        }
  ...
  private async updateMetrics(): Promise<void> {
    try {
      ...
      monitoringService.setGauge("feature_flags_total", enabledCount, [
        "enabled",
      ]);
      monitoringService.setGauge("feature_flags_total", disabledCount, [
        "disabled",
      ]);
    } catch (error) {
  ...
  async getFlag(flagId: string): Promise<FeatureFlag | null> {
    ...
    try {
      ...
      if (cached) {
        logger.warn("Using expired cache for feature flag due to Redis error", {
          flagId,
        });
        return cached.flag;
      }
  ...
  async isEnabled(flagId: string, context: FlagContext = {}): Promise<boolean> {
    ...
    try {
      ...
      if (
        flag.abTestGroup &&
        context.abTestGroup &&
        flag.abTestGroup !== context.abTestGroup
      ) {
      ...
      if (flag.targetDealerships && flag.targetDealerships.length > 0) {
        if (
          ...
          !flag.targetDealerships.includes(context.dealershipId)
        ) {
      ...
      if (flag.rules && flag.rules.length > 0) {
        const allRulesPassed = flag.rules.every((rule) =>
  ...
  private recordEvaluation(
    ...
    try {
      ...
      monitoringService.incrementCounter("feature_flag_evaluations_total", 1, [
        flagId,
        result,
      ]);
      ...
      logger.error("Error recording feature flag evaluation metrics", {
        error,
      });
    }
  }
  ...
  private evaluateRule(rule: FeatureFlagRule, context: FlagContext): boolean {
    try {
      ...
      switch (rule.operator) {
        ...
        case "not_in":
          return (
            Array.isArray(rule.value) && !rule.value.includes(contextValue)
          );
        case "contains":
  ...
  async createRollbackVersion(flagId: string): Promise<FeatureFlag | null> {
    try {
      ...
      logger.info(`Created rollback version for flag: ${flagId}`, {
        rollbackId,
      });
  ...
  async restoreFromRollback(rollbackId: string): Promise<FeatureFlag | null> {
    try {
      ...
      logger.info(`Restored flag from rollback: ${originalFlagId}`, {
        rollbackId,
      });
  ...
  async getAuditTrail(flagId: string, limit = 100): Promise<FlagChangeEvent[]> {
    try {
      ...
      logger.error("Error retrieving feature flag audit trail", {
        flagId,
        error,
      });
      return [];
  ...
  async configureGradualRollout(
    ...
    try {
      ...
      if (!flag) {
        logger.warn(
          `Cannot configure rollout for non-existent flag: ${flagId}`,
        );
        return false;
      }
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      logger.warn(
        "Feature Flags Service used before initialization, initializing now",
      );
      this.initialize().catch((error) => {
        logger.error("Failed to initialize Feature Flags Service on demand", {
          error,
        });
      });
    }
