...
export class RylieRetriever implements Retriever {
  ...
  async retrieve(
    ...
    try {
      ...
      if (intent.includes("vehicle") || intent.includes("inventory")) {
        const vehicleDocs = await this.retrieveVehicleDocuments(
          keywords,
          query,
        );
      ...
      if (
        intent.includes("dealership") ||
        intent.includes("location") ||
      ...
      if (context?.conversationId) {
        const historyDocs = await this.retrieveConversationHistory(
  ...
  private analyzeIntent(query: string, keywords: string[]): string[] {
    ...
    if (
      keywords.some((k) =>
        ["car", "vehicle", "truck", "suv", "sedan"].includes(k),
      ) ||
    ...
    // Service intent
    if (
      keywords.some((k) => ["service", "maintenance", "repair"].includes(k)) ||
    ...
    if (
      keywords.some((k) =>
        ["financing", "lease", "loan", "payment"].includes(k),
      )
    ) {
