...
export class HandoffService {
  ...
  constructor(options: HandoffServiceOptions = {}) {
    this.messageThreshold =
      options.messageThreshold ||
      parseInt(process.env.HANDOFF_MESSAGE_COUNT || "5", 10);
  }
  ...
  async executeHandoff(conversationId: number): Promise<void> {
    ...
    const lead = await db.query.leads.findFirst({
      where: eq(leads.id, convo.leadId),
    });
    ...
    await sendHandoverEmail(
      process.env.SALES_TEAM_EMAIL || "sales@example.com",
      dossier,
    );
