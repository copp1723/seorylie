...
export class AIResponseService {
    ...
    try {
      ...
      if (escalationResult.shouldEscalate) {
        ...
        const handoverRequest: HandoverRequest = {
          conversationId: request.conversationId,
          reason: this.mapEscalationReasonToHandoverReason(
            escalationResult.reason || "ai_limitation",
          ),
          description:
            escalationResult.description ||
            `Automatic escalation: ${escalationResult.reason}`,
          urgency: this.determineUrgencyFromReason(
        ...
        if (handoverResult.success) {
          ...
          logger.warn(
            "Escalation creation failed, continuing with AI response",
            {
              conversationId: request.conversationId,
      ...
      if (postResponseEscalation.shouldEscalate) {
        ...
        const handoverRequest: HandoverRequest = {
          conversationId: request.conversationId,
          reason: this.mapEscalationReasonToHandoverReason(
            postResponseEscalation.reason || "ai_limitation",
          ),
          description:
            postResponseEscalation.description ||
            `Post-response escalation: ${postResponseEscalation.reason}`,
          urgency: this.determineUrgencyFromReason(
            postResponseEscalation.reason || "",
          ),
        };
        ...
        if (handoverResult.success) {
          ...
          await this.sendPostResponseEscalationMessage(
            request.dealershipId,
            request.conversationId,
          );
        }
      ...
      return {
        ...
        handoverId: postResponseEscalation.shouldEscalate
          ? undefined
          : undefined,
        errors,
      };
  ...
  private async generateAIResponse(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const conversationHistory = conversationDetails.messages
          .slice(-10)
          .map((msg) => ({
    ...
    return {
      ...
      content:
        "I'm having trouble processing your message right now. A human representative will be with you shortly.",
  ...
  private async sendEscalationMessage(
    ...
    try {
      const escalationMessage =
        "I'm connecting you with one of our team members who can better assist you. Someone will be with you shortly.";
   ...
   */
  private async sendPostResponseEscalationMessage(
    dealershipId: number,
    conversationId: string,
  ): Promise<void> {
    try {
      const escalationMessage =
        "I've also notified our team about your inquiry to ensure you get the best possible assistance.";
      ...
      logger.error("Failed to send post-response escalation message", {
        error,
        conversationId,
      });
    }
