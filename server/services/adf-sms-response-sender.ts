...
export class AdfSmsResponseSender extends EventEmitter {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.error("Failed to initialize ADF SMS Response Sender", {
        error: err.message,
      });
      throw err;
    }
  ...
  private async handleLeadResponse(data: {
    ...
    try {
      ...
      const isOptedOut = await twilioSMSService.checkOptOutStatus(
        dealershipId,
        phoneNumber,
      );
      if (isOptedOut) {
  ...
  public async handleDeliveryUpdate(event: AdfSmsDeliveryEvent): Promise<void> {
    try {
      const { leadId, messageSid, status, errorCode, errorMessage, timestamp } =
        event;
      ...
      if (status === "delivered") {
        ...
        if (smsResponse && smsResponse.sentAt) {
          const deliveryTimeMs =
            timestamp.getTime() - smsResponse.sentAt.getTime();

          SMS_METRICS.lastDeliveryTimeMs = deliveryTimeMs;
          SMS_METRICS.totalDeliveryTimeMs += deliveryTimeMs;
          SMS_METRICS.avgDeliveryTimeMs =
            SMS_METRICS.totalDeliveryTimeMs / SMS_METRICS.deliveredCount;
  ...
  public async handleOptOut(data: {
    ...
    try {
      ...
      const leads = await this.findLeadsByPhoneNumber(
        phoneNumber,
        dealershipId,
      );
  ...
  private scheduleRetry(leadId: number): void {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (
          smsResponse.status === "delivered" ||
          smsResponse.retryCount >= MAX_RETRIES
        ) {
          logger.info(
            "Skipping retry - already delivered or max retries reached",
            {
              leadId,
      ...
      async () => {
        ...
        `);
          ...
          if (!tableExists.rows[0]?.exists) {
            logger.warn(
              "adf_sms_responses table does not exist, skipping undelivered check",
            );
            return;
          }
    ...
    try {
      ...
      if (lead.customer) {
        ...
        if (lead.customer.phones) {
          const mobilePhone = lead.customer.phones.find(
            (p) =>
   ...
   */
  private async updateSmsWithSid(
    leadId: number,
    messageSid: string,
  ): Promise<void> {
    try {
   ...
   */
  private async getSmsResponseByLeadId(
    leadId: number,
  ): Promise<AdfSmsResponse | null> {
    try {
   ...
   */
  private async getSmsResponseByMessageSid(
    messageSid: string,
  ): Promise<AdfSmsResponse | null> {
    try {
      const results = await db.execute(sql`
  ...
  public async processWebhook(webhookData: any): Promise<void> {
    try {
      const { MessageSid, MessageStatus, ErrorCode, ErrorMessage, To } =
        webhookData;
   ...
   */
  private async findDealershipByPhoneNumber(
    phoneNumber: string,
  ): Promise<number | null> {
    try {
      // Normalize phone for matching
  ...
  private async createTestLead(
    ...
    try {
      ...
      const existingLeads = await this.findLeadsByPhoneNumber(
        phoneNumber,
        dealershipId,
      );
