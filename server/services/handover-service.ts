...
export class HandoverService extends EventEmitter {
  ...
  constructor(options: HandoverOptions = {}) {
    ...
    this.defaultSubject =
      options.defaultSubject || "Lead Handover Notification";
  ...
  async triggerHandover(data: HandoverData): Promise<HandoverResult> {
    ...
    try {
      ...
      const notificationResult = await this.sendHandoverNotification(
        handoverId,
        data,
        recipientEmails,
      );
  ...
  private async sendHandoverNotification(
    ...
    try {
      ...
      const templateData = {
        ...
        dealershipName:
          data.dealershipName || `Dealership ID: ${data.dealershipId}`,
  ...
  async shouldHandover(
    ...
    if (intents && intents.length > 0) {
      if (
        intents.includes("request_human") ||
        intents.includes("talk_to_agent")
      ) {
    ...
    if (
      lowerContent.includes("test drive") ||
      lowerContent.includes("drive it")
    ) {
      return { shouldHandover: true, reason: "test_drive_request" };
    }
