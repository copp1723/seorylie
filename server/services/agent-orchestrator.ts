...
import { AnalyticsClient, AnalyticsRequest, AnalyticsResponse } from './analytics-client';
...
export class AgentOrchestrator extends EventEmitter {
  ...
  constructor() {
    ...
    this.analyticsCircuitBreaker = new CircuitBreaker({
      ...
      onHalfOpen: () => logger.info('Analytics service circuit breaker half-open')
    });
    ...
    this.automationCircuitBreaker = new CircuitBreaker({
      ...
      onHalfOpen: () => logger.info('Automation service circuit breaker half-open')
    });
  ...
  public registerWorkflow(workflow: WorkflowDefinition): void {
    if (this.workflows.has(workflow.id)) {
      logger.warn(`Workflow with ID ${workflow.id} already exists, overwriting`);
    }
   ...
   */
  public async executeWorkflow(request: WorkflowExecutionRequest): Promise<WorkflowExecutionResponse> {
    const { workflowId, agentId, agentType, parameters, userId, dealershipId, options } = request;
    ...
    if (!workflow.supportedAgentTypes.includes(agentType)) {
      logger.error(`Agent type ${agentType} not supported for workflow ${workflowId}`);
      return {
        executionId: '',
        workflowId,
        status: 'rejected',
   ...
   */
  public getWorkflowStatus(executionId: string): WorkflowExecutionResponse | null {
    ...
    return {
      ...
      startTime: context.startTime,
      endTime: context.state === 'completed' || context.state === 'failed' ? new Date() : undefined,
      results: context.state === 'completed' ? context.results : undefined,
      error: context.state === 'failed' ? {
   ...
   */
  public listWorkflows(agentType?: AgentType): { id: string; name: string; description: string }[] {
   ...
   */
  public async callAnalyticsService(request: AnalyticsRequest): Promise<AnalyticsResponse> {
    try {
   ...
   */
  public async callAutomationService(operation: string, parameters: Record<string, any>): Promise<ServiceResponse> {
    try {
      return await this.automationCircuitBreaker.execute(async () => {
        logger.debug('Making automation service request', { operation, parameters });
        const startTime = Date.now();
        
        const response = await this.automationClient.post(`/api/v1/${operation}`, parameters);
   ...
   */
  public async executeServiceRequest(request: ServiceRequest): Promise<ServiceResponse> {
    ...
    try {
      switch (service) {
        case ServiceType.ANALYTICS:
          const analyticsResponse = await this.callAnalyticsService(parameters as AnalyticsRequest);
          return {
   ...
   */
  private async executeWorkflowSteps(executionId: string, workflow: WorkflowDefinition, context: WorkflowContext): Promise<void> {
    for (const step of workflow.steps) {
      // Skip step if condition is not met
      if (step.condition && !step.condition(context)) {
        logger.info(`Skipping workflow step: ${step.name} (condition not met)`, {
          executionId,
          stepId: step.id
        });
      ...
      try {
        ...
        const resolvedParameters = typeof step.parameters === 'function' 
        ...
        // Update step history
        const historyEntry = context.stepHistory.find(h => h.stepId === step.id);
        if (historyEntry) {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (attempt === maxRetries) {
          throw new Error(`Service request failed after ${maxRetries} attempts: ${response.error?.message}`);
        }
        
        // Log retry attempt
        logger.warn(`Service request failed, retrying (${attempt + 1}/${maxRetries})`, {
          service,
          operation,
        ...
        // Log retry attempt
        logger.warn(`Service request threw exception, retrying (${attempt + 1}/${maxRetries})`, {
          service,
          operation,
          error: error instanceof Error ? error.message : String(error)
        });
   ...
   */
  private handleStepCompletion(executionId: string, stepId: string, result: any): void {
   ...
   */
  private handleStepError(executionId: string, stepId: string, error: any): void {
   ...
   */
  private handleWorkflowCompletion(executionId: string, results: Record<string, any>): void {
    const context = this.activeWorkflows.get(executionId);
    if (!context) {
      return;
    }
   ...
   */
  private mapStateToStatus(state: string): 'accepted' | 'rejected' | 'completed' | 'failed' {
    switch (state) {
      case 'running':
      case 'paused':
        return 'accepted';
  ...
  private registerPredefinedWorkflows(): void {
    ...
    this.registerWorkflow({
      ...
      description: 'Analyzes sales data and updates VinSolutions if issues are found',
      supportedAgentTypes: [AgentType.SALES_ANALYTICS, AgentType.GENERAL_PURPOSE],
      steps: [
        {
          ...
          parameters: (context) => ({
            ...
            metrics: ['conversion_rate', 'sales_performance', 'lead_source_roi'],
          ...
          onSuccess: (result, context) => {
            ...
            const issues = result.insights?.filter(insight => 
              insight.score && insight.score < 0.4 // Low score indicates an issue
            );
    ...
    this.registerWorkflow({
      ...
      supportedAgentTypes: [AgentType.INVENTORY_AUTOMATION, AgentType.GENERAL_PURPOSE],
      steps: [
        {
          ...
          parameters: (context) => ({
            uploadId: context.parameters.uploadId,
            metrics: ['inventory_health', 'aging_analysis', 'price_competitiveness'],
        ...
        },
        {
          ...
          parameters: (context) => {
            ...
            const agingIssues = inventoryData.insights?.filter(i => 
              i.title.toLowerCase().includes('aging') && i.score && i.score < 0.5
            );
                ...
                actions: agingIssues?.length ? [
                  {
                    ...
                    vehicles: agingIssues.flatMap(i => i.metrics?.vehicles || []),
    ...
    this.registerWorkflow({
      ...
      supportedAgentTypes: [AgentType.LEAD_MANAGEMENT, AgentType.GENERAL_PURPOSE],
      steps: [
        {
          ...
          parameters: (context) => ({
            uploadId: context.parameters.uploadId,
            metrics: ['lead_volume', 'lead_sources', 'conversion_rates', 'response_time'],
            timeRange: context.parameters.timeRange || {
        ...
        },
        {
          ...
          parameters: (context) => ({
            uploadId: context.parameters.uploadId,
            query: 'Find leads that have not been contacted in the last 7 days with a high purchase likelihood score',
            filters: {
              lastContactDays: '>7',
        ...
        },
        {
          ...
          parameters: (context) => {
            const opportunities = context.results['identify-followup-opportunities'];
            return {
          ...
          condition: (context) => {
            const opportunities = context.results['identify-followup-opportunities'];
            return opportunities && opportunities.data && opportunities.data.length > 0;
          }
        },
        {
          ...
          parameters: (context) => ({
            ...
            followupSummary: {
              ...
              opportunities: context.results['identify-followup-opportunities']?.data?.length || 0
            }
          })
        }
      ]
...
export class CircuitBreaker {
  ...
  constructor(private options: {
    ...
    this.options.halfOpenSuccessThreshold = this.options.halfOpenSuccessThreshold || 1;
  }
