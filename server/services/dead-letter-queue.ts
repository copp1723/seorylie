...
export class DeadLetterQueueService extends EventEmitter {
  ...
  private retryHandlers: Map<string, (entry: DLQEntry) => Promise<void>> =
    new Map();
  ...
  private setupMetrics(): void {
    ...
    monitoring.registerCounter(
      "dlq_retries_attempted_total",
      "Total retry attempts",
    );
    monitoring.registerCounter(
      "dlq_retries_successful_total",
    ...
    monitoring.registerUpDownCounter(
      "dlq_pending_entries_count",
      "Number of pending DLQ entries",
    );
    monitoring.registerHistogram(
      "dlq_retry_duration_ms",
      "Time taken for DLQ retry operations",
    );
    monitoring.registerObservableGauge(
      "dlq_oldest_entry_age_seconds",
      "Age of oldest DLQ entry in seconds",
      (observerResult) => {
        const oldestEntry = this.getOldestEntry();
        if (oldestEntry) {
          const ageSeconds =
            (Date.now() - oldestEntry.timestamp.getTime()) / 1000;
  ...
  private async processEntries(): Promise<void> {
    ...
    const retryableEntries = Array.from(this.dlq.values())
      .filter(
        (entry) =>
          entry.nextRetryAt <= now && entry.attempts < entry.maxAttempts,
      )
      .sort((a, b) => {
  ...
  private calculateBackoffDelay(attempts: number): number {
    const delay =
      ...
      Math.pow(this.config.retryMultiplier, attempts - 1);
