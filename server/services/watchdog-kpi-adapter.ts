...
export class WatchdogKpiAdapter {
  ...
  constructor(baseUrl?: string, apiKey?: string) {
    this.baseUrl = baseUrl || process.env.WATCHDOG_API_URL || 'http://localhost:8000';
    this.cache = new NodeCache({ stdTTL: this.DEFAULT_CACHE_TTL, checkperiod: 120 });
    ...
    this.client.interceptors.response.use(
      (response) => {
        logger.debug(`Watchdog KPI API response from ${response.config.url} - Status: ${response.status}`);
        return response;
      },
      (error: AxiosError) => {
  ...
  private async request<T>(
    ...
    try {
      const response = await retry(
        ...
        },
        {
          ...
          onRetry: (error, attempt) => {
            logger.warn(`Retry attempt ${attempt} for ${endpoint} due to: ${error.message}`);
          }
        }
      );
      ...
      logger.error(`Watchdog KPI API error in ${method.toUpperCase()} ${endpoint}`, { error: error.message });
      
      if (error.response) {
        throw new Error(`Watchdog KPI API error: ${error.response.status} - ${JSON.stringify(error.response.data)}`);
   ...
   */
  async getAdsKpi(cid: string, filter: KpiFilter = {}): Promise<AdsKpiResponse> {
    ...
    try {
      ...
      const isHistorical = filter.endDate && new Date(filter.endDate).getTime() < Date.now() - 86400000;
      const cacheTtl = isHistorical ? this.LONG_CACHE_TTL : this.DEFAULT_CACHE_TTL;
      ...
      logger.error(`Error getting Ads KPI for ${cid}`, { error: error.message, filter });
  ...
  async getRoas(cid: string, filter: KpiFilter = {}): Promise<AdsKpiResponse> {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        ...
        const totalValue = response.data.reduce((sum, item) => sum + item.conversionValue, 0);
        ...
        return {
          ...
          data: {
            ...
            impressions: response.data.reduce((sum, item) => sum + item.impressions, 0),
            clicks: response.data.reduce((sum, item) => sum + item.clicks, 0),
            conversions: response.data.reduce((sum, item) => sum + item.conversions, 0),
            ctr: response.data.reduce((sum, item) => sum + item.ctr, 0) / response.data.length,
            cpc: response.data.reduce((sum, item) => sum + item.cpc, 0) / response.data.length,
            cpa: response.data.reduce((sum, item) => sum + item.cpa, 0) / response.data.length,
      ...
      logger.error(`Error getting ROAS for ${cid}`, { error: error.message, filter });
  ...
  async getCpa(cid: string, filter: KpiFilter = {}): Promise<AdsKpiResponse> {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        const totalCost = response.data.reduce((sum, item) => sum + item.cost, 0);
        const totalConversions = response.data.reduce((sum, item) => sum + item.conversions, 0);
        ...
        return {
          ...
          data: {
            ...
            impressions: response.data.reduce((sum, item) => sum + item.impressions, 0),
            clicks: response.data.reduce((sum, item) => sum + item.clicks, 0),
            conversionValue: response.data.reduce((sum, item) => sum + item.conversionValue, 0),
            ctr: response.data.reduce((sum, item) => sum + item.ctr, 0) / response.data.length,
            cpc: response.data.reduce((sum, item) => sum + item.cpc, 0) / response.data.length,
            roas: response.data.reduce((sum, item) => sum + item.roas, 0) / response.data.length,
            currencyCode: response.data[0]?.currencyCode || 'USD'
          },
          metadata: {
            ...response.metadata,
      ...
      logger.error(`Error getting CPA for ${cid}`, { error: error.message, filter });
      span.setStatus({ code: 2, message: error.message });
  ...
  async getCtr(cid: string, filter: KpiFilter = {}): Promise<AdsKpiResponse> {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        const totalImpressions = response.data.reduce((sum, item) => sum + item.impressions, 0);
        const totalClicks = response.data.reduce((sum, item) => sum + item.clicks, 0);
      ...
      logger.error(`Error getting CTR for ${cid}`, { error: error.message, filter });
      span.setStatus({ code: 2, message: error.message });
   ...
   */
  async getKpiForTimePeriod(cid: string, period: '7d' | '30d' | '90d' | 'ytd' | 'all', metric?: 'roas' | 'cpa' | 'ctr'): Promise<AdsKpiResponse> {
    ...
    try {
      ...
      logger.error(`Error getting KPI for period ${period}`, { error: error.message, cid });
      span.setStatus({ code: 2, message: error.message });
   ...
   */
  private async getKpiFromDatabase(cid: string, filter: KpiFilter = {}): Promise<AdsKpiResponse> {
    ...
    try {
      ...
      if (filter.startDate && filter.endDate) {
        ...
        query = query.where(gte(dailySpendLogs.date, new Date(filter.startDate)));
      ...
      if (filter.groupBy === 'campaign') {
        query = query.groupBy(dailySpendLogs.campaignId, dailySpendLogs.campaignName, dailySpendLogs.cid, dailySpendLogs.accountCurrencyCode);
      } else if (filter.groupBy === 'day') {
        query = query.groupBy(dailySpendLogs.date, dailySpendLogs.cid, dailySpendLogs.accountCurrencyCode);
      } else if (filter.groupBy === 'account') {
        query = query.groupBy(dailySpendLogs.cid, dailySpendLogs.accountCurrencyCode);
      }
      ...
      return {
        success: true,
        data: transformedResults.length === 1 ? transformedResults[0] : transformedResults,
      ...
      logger.error(`Error getting KPI from database for ${cid}`, { error: error.message, filter });
      span.setStatus({ code: 2, message: error.message });
   ...
   */
  private async getTimeSeriesFromDatabase(cid: string, filter: KpiFilter): Promise<AdsKpiTimeSeriesData | undefined> {
    if (!filter.startDate || !filter.endDate) {
      return undefined;
    }
    ...
    try {
      ...
      const timeSeries: AdsKpiTimeSeriesData = {
        ...
        metrics: {
          ...
          conversionValue: results.map(r => Number(r.conversionValueMicros) / 1000000 || 0),
      ...
      logger.error(`Error getting time series data for ${cid}`, { error: error.message });
      return undefined;
    }
  }
   ...
   */
  private transformApiResponse(apiResponse: any, filter: KpiFilter): AdsKpiResponse {
    ...
    try {
      ...
      if (Array.isArray(apiResponse.data)) {
        const transformedData: AdsKpiMetrics[] = apiResponse.data.map((item: any) => ({
        ...
        if (apiResponse.time_series) {
          timeSeriesData = {
            ...
            metrics: {
              ...
              conversionValue: apiResponse.time_series.metrics?.conversion_value || [],
