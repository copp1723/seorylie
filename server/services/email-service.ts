...
export async function sendEmail(
  ...
  try {
    ...
    // Determine provider
    const provider =
      emailOptions.provider || (getSendGridConfig() ? "sendgrid" : "smtp");
    ...
    if (provider === "sendgrid") {
      ...
      const mailOptions = {
        from:
          emailOptions.from || process.env.EMAIL_FROM || process.env.EMAIL_USER,
        to: emailOptions.to,
        subject: emailOptions.subject,
    ...
    logger.error("Failed to send email", {
      ...
      to:
        options?.to ||
        (typeof apiKeyOrOptions === "object" ? apiKeyOrOptions?.to : "unknown"),
    });
    return false;
  }
}
 ...
 */
export function getDeliveryStatusesByRecipient(
  recipient: string,
): EmailDeliveryStatus[] {
  return Array.from(deliveryTracking.values()).filter(
...
export class EmailService {
    ...
    switch (this.provider) {
      case "sendgrid":
        ...
        this.sendgrid = {
          ...
          fromEmail:
            process.env.SENDGRID_FROM_EMAIL || "noreply@cleanrylie.com",
        };
        break;
  ...
  async send(options: EmailOptions): Promise<{ messageId: string }> {
    try {
      ...
      if (options.text && options.text.length > 500000) {
        logger.warn("Email content exceeds recommended size", {
        ...
        try {
          ...
          if (attempt >= maxRetries || !isRetryableError) {
            ...
            if (attempt >= maxRetries) {
              throw new Error(
                `Failed to send email after ${maxRetries} retries`,
              );
            }
            break;
          }
      ...
      if (!success) {
        throw (
          lastError || new Error(`Failed to send email via ${this.provider}`)
        );
      }
  ...
  private async sendViaSendGrid(
    ...
    try {
      ...
      throw new Error(
        `Failed to send email via SendGrid: ${this.getErrorMessage(error)}`,
      );
    }
  }
  ...
  private async sendViaSmtp(
    ...
    try {
      ...
      throw new Error(
        `Failed to send email via SMTP: ${this.getErrorMessage(error)}`,
      );
    }
  }
  ...
  async sendTemplate(
    options: EmailOptions,
  ): Promise<{ messageId: string } | { success: boolean; errors?: string[] }> {
    try {
    ...
    try {
      ...
      if (!allSuccessful) {
        ...
        return {
          success: false,
          errors: [
            `Failed to send ${failedCount} out of ${results.length} emails`,
          ],
        };
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      logger.error("EmailService sendTemplateEmail failed", {
        error: errorMessage,
      });
      return {
        success: false,
  ...
  async renderTemplate(
    ...
    try {
      ...
      const malformedExpressions =
        templateContent.match(/{{[^}]*[^}][^}]*$/gm) ||
        templateContent.match(/{{[^{}]*[!@#$%^&*()+=\[\];':"\\|,.<>?]/g);
      if (malformedExpressions && malformedExpressions.length > 0) {
        throw new Error(
          "Invalid template syntax: malformed handlebars expressions",
        );
      }
