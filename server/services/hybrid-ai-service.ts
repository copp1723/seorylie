...
export class HybridAIService {
  ...
  constructor(config: HybridAIConfig = { 
    ...
    maxRetries: 2,
    preferAgentSquadFor: ['inventory', 'vehicle_search', 'pricing', 'availability']
  }) {
    this.config = {
      timeoutMs: 30000,
      enablePerformanceTracking: true,
    ...
    logger.info('HybridAIService initialized with enhanced configuration', { config: this.config });
  }
  ...
  async generateResponse(request: AIResponseRequest): Promise<HybridAIResponse> {
    ...
    if (this.config.useAgentSquad && isAgentSquadReady() && shouldPreferAgentSquad) {
      for (retryAttempt = 0; retryAttempt < (this.config.maxRetries || 2); retryAttempt++) {
        try {
          agentSquadStartTime = Date.now();
          const agentSquadResult = await this.tryAgentSquadWithTimeout(request, retryAttempt);
          ...
          if (agentSquadResult.success && !agentSquadResult.fallbackRequired) {
            ...
            logger.warn(`Agent Squad attempt ${retryAttempt + 1} failed or requested fallback`, {
              conversationId: request.conversationId,
          ...
          const agentSquadTimeMs = agentSquadStartTime ? Date.now() - agentSquadStartTime : 0;
          logger.error(`Agent Squad processing failed on attempt ${retryAttempt + 1}`, {
          ...
          if (error instanceof Error && (error.message.includes('timeout') || error.message.includes('rate limit'))) {
            break;
          }
        }
    ...
    if (this.config.fallbackToOriginal) {
      ...
      logger.info('Using original AI service as fallback', {
        ...
        reason: shouldPreferAgentSquad ? 'agent_squad_failed' : 'agent_squad_not_preferred'
      });
      ...
      try {
        ...
        return {
          ...
          fallbackReason: this.config.useAgentSquad ? 
            (shouldPreferAgentSquad ? 'agent_squad_failed' : 'agent_squad_not_preferred') : 
        ...
        return {
          success: false,
          errors: [`All AI systems failed: ${error instanceof Error ? error.message : String(error)}`],
          usedAgentSquad: false,
  ...
  private shouldUseAgentSquad(request: AIResponseRequest): boolean {
    ...
    const hasInventoryKeywords = [
      'inventory', 'vehicle', 'car', 'truck', 'suv', 'sedan', 'search', 'find',
      'available', 'price', 'cost', 'financing', 'lease', 'buy', 'purchase',
      'model', 'make', 'year', 'mileage', 'features', 'specs', 'details'
    ...
    const hasPreferredIntent = preferredTypes.some(type => {
      switch (type) {
        ...
        case 'vehicle_search':
          return hasInventoryKeywords || prompt.includes('looking for') || prompt.includes('need a');
        case 'pricing':
          return prompt.includes('price') || prompt.includes('cost') || prompt.includes('payment');
        case 'availability':
          return prompt.includes('available') || prompt.includes('in stock') || prompt.includes('test drive');
        default:
          return false;
      }
    });
  ...
  private async tryAgentSquadWithTimeout(
    ...
    try {
      ...
      const timeoutPromise = new Promise<AgentSquadResponse>((_, reject) => {
        setTimeout(() => {
          reject(new Error(`Agent Squad timeout after ${this.config.timeoutMs}ms`));
      ...
      const agentSquadResult = await Promise.race([agentSquadPromise, timeoutPromise]);
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Agent Squad processing failed',
   ...
   */
  async getPerformanceAnalytics(timeRange?: { start: Date; end: Date }): Promise<{
...
export const hybridAIService = new HybridAIService({
  ...
  preferAgentSquadFor: ['inventory', 'vehicle_search', 'pricing', 'availability'],
  agentSquadThreshold: parseFloat(process.env.AGENT_SQUAD_CONFIDENCE_THRESHOLD || '0.7')
});