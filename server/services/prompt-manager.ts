...
export class PromptManager {
  ...
  private setupHelpers(): void {
    ...
    this.handlebars.registerHelper('formatDate', (date: Date | string, format = 'MM/dd/yyyy') => {
    ...
    this.handlebars.registerHelper('customerGreeting', (name: string, timeOfDay?: string) => {
      const greeting = this.getTimeBasedGreeting(timeOfDay);
      return name ? `${greeting}, ${name}` : greeting;
    });

    // Previous message summary helper
    this.handlebars.registerHelper('previousMessageSummary', (messages: any[], maxLength = 100) => {
   ...
   */
  private async loadPromptsFromDirectory(dirPath: string, category = ''): Promise<void> {
   ...
   */
  private async loadPromptFromFile(filePath: string, category: string): Promise<void> {
    try {
   ...
   */
  private parseFrontMatter(content: string): { metadata: any; template: string } {
  ...
  private async setupFileWatchers(): Promise<void> {
    try {
      ...
      watcher.on('change', async (filePath) => {
        ...
        await this.loadPromptFromFile(filePath, category === '.' ? '' : category);
      });
   ...
   */
  async selectPrompt(context: ConversationContext, turnNumber: number): Promise<PromptTemplate> {
   ...
   */
  private async findBestMatch(criteria: PromptSelectionCriteria): Promise<PromptTemplate | null> {
   ...
   */
  private calculateMatchScore(template: PromptTemplate, criteria: PromptSelectionCriteria): number {
    let score = 0;
    ...
    if (template.metadata.vehicleType && criteria.vehicleInterest) {
      if (template.metadata.vehicleType.toLowerCase() === criteria.vehicleInterest.toLowerCase()) {
        score += 25;
      }
    }
   ...
   */
  private async getDefaultPrompt(turnNumber: number): Promise<PromptTemplate | null> {
    // Look for default turn-specific prompts
    const defaultPrompts = Array.from(this.templateCache.values()).filter(template => 
      (template.category === 'default' || template.category === 'adf') &&
      (template.metadata.turnNumber === turnNumber || !template.metadata.turnNumber)
    );

    if (defaultPrompts.length > 0) {
      // Prefer turn-specific, fall back to generic
      const turnSpecific = defaultPrompts.find(p => p.metadata.turnNumber === turnNumber);
  ...
  private getTimeBasedGreeting(timeOfDay?: string): string {
    ...
    if (timeOfDay) {
      return timeOfDay.toLowerCase().includes('morning') ? 'Good morning' :
             timeOfDay.toLowerCase().includes('afternoon') ? 'Good afternoon' :
             timeOfDay.toLowerCase().includes('evening') ? 'Good evening' : 'Hello';
    }
