...
import {
  handoverDossierService,
  type HandoverDossier,
} from "./handover-dossier-service";
...
export class HandoverOrchestrator extends EventEmitter {
  ...
  constructor() {
    ...
    try {
      ...
      const htmlTemplate = fs.readFileSync(
        path.join(templateDir, "handover-dossier.html"),
        "utf8",
      );
      const textTemplate = fs.readFileSync(
        path.join(templateDir, "handover-dossier.txt"),
        "utf8",
      );
  ...
  init(): void {
    if (!this.isInitialized) {
      logger.error(
        "Cannot initialize HandoverOrchestrator: templates not loaded",
      );
      return;
    }
    ...
    eventBus.on(
      ...
      this.handleReadyForHandover.bind(this),
    );
   ...
   */
  private async checkExistingHandover(
    conversationId: string,
   ...
   * Update handover record with dossier
   */
  private async updateHandoverWithDossier(
    handoverId: string,
  ...
  private async sendHandoverEmail(
    ...
    try {
      ...
      if (!settings || !settings.handoverEmail) {
        throw new Error(
          `No handover email configured for dealership ${dealershipId}`,
        );
      }
      ...
      const urgencyPrefix =
        dossier.urgency === "high" || dossier.urgency === "urgent"
   ...
   */
  private async handleEmailDelivered(data: {
    messageId: string;
  }): Promise<void> {
    try {
      ...
      const results = await db
        ...
        .from(handovers)
        .where(
          db.sql`${handovers.context}->>'emailMessageId' = ${data.messageId}`,
        )
        .limit(1);
   ...
   */
  private async handleEmailFailed(data: {
    messageId: string;
    reason: string;
  }): Promise<void> {
    try {
   ...
   */
  private async getDealershipIdForHandover(
    handoverId: string,
  ): Promise<number | null> {
    try {
      const results = await db
  ...
  private scheduleEmailRetry(handoverId: string, delayMs: number): void {
    logger.info(`Scheduling handover email retry in ${delayMs}ms`, {
      handoverId,
    });
