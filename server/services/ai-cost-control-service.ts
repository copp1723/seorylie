...
export class AiCostControlService {
  ...
  constructor(options: {
    ...
    this.templatePath =
      ...
      path.join(process.cwd(), "server", "templates", "ai-responses");
    ...
    try {
      ...
      for (const file of jsonFiles) {
        try {
          ...
          logger.debug(
            `Loaded template: ${template.id} for intent: ${template.intent}`,
          );
        } catch (error) {
  ...
  private getRateLimiter(dealershipId: number | string): Bottleneck {
    ...
    if (!this.limiter[dealershipIdStr]) {
      ...
      const dailyLimit =
        this.defaultRateLimitConfig.dealershipOverrides?.[Number(dealershipId)]
      ...
      this.limiter[dealershipIdStr].on("dropped", () => {
        logger.warn(
          `Request dropped due to rate limit for dealership ${dealershipId}`,
        );
        prometheusMetrics.incrementAiRateLimitEvents({
          dealership_id: dealershipIdStr,
          event_type: "dropped",
  ...
  private findMatchingTemplate(
    ...
    const matchingTemplates = this.templates.filter(
      (t) => t.intent.toLowerCase() === intent.toLowerCase(),
    );
    ...
    const genericTemplate = matchingTemplates.find(
      (t) => !t.vehicleTypes || t.vehicleTypes.length === 0,
    );
    ...
    try {
      ...
      return template
        .replace(
          ...
          lead.customerName?.split(" ")[0] || "there",
        )
  ...
  private async storeInCache(
    ...
    try {
      ...
      await this.redis.set(cacheKey, JSON.stringify(cachedResponse), {
        EX: ttl,
      });
  ...
  async generateResponse(
    ...
    try {
      ...
      if (!forceFresh) {
        ...
        if (template) {
          const personalizedText = this.personalizeTemplate(
            template.template,
            lead,
            dealership,
          );
      ...
      if (reservoir !== null && reservoir <= 0) {
        ...
        const fallbackTemplate = this.templates.find(
          (t) => t.intent === "rate_limit_exceeded",
        );
        if (fallbackTemplate) {
          const fallbackText = this.personalizeTemplate(
      ...
      return await limiter.schedule(async () => {
        ...
        // Call OpenAI
        const aiResponse = await aiService.getCompletion(prompt, {
...
`;
  ...
  updateRateLimitConfig(config: Partial<RateLimitConfig>): void {
    ...
    logger.info("Updated rate limit configuration", {
      maxConcurrent: this.defaultRateLimitConfig.maxConcurrent,
      dailyLimit: this.defaultRateLimitConfig.dailyLimit,
      dealershipOverridesCount: Object.keys(
        this.defaultRateLimitConfig.dealershipOverrides || {},
      ).length,
    });
  }
...
export const initializeAiCostControlService = async (
  redis: Redis,
  options?: {
    rateLimitConfig?: Partial<RateLimitConfig>;
    cacheEnabled?: boolean;
...
export const getAiCostControlService = (): AiCostControlService => {
  if (!aiCostControlService) {
    throw new Error(
      "AI Cost Control Service not initialized. Call initializeAiCostControlService first.",
    );
  }
  return aiCostControlService;
};
