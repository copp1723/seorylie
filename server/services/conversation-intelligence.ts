...
export interface ConversationIntent {
  primary: 'browsing' | 'specific_vehicle' | 'pricing' | 'financing' | 'trade_in' | 'scheduling' | 'support';
  confidence: number;
...
export interface CustomerNeed {
  ...
  timeline: 'immediate' | 'within_month' | 'within_3months' | 'within_6months' | 'exploring';
...
export class ConversationIntelligence {
   ...
   */
  async getConversationContext(conversationId: string, dealershipId: number): Promise<ConversationContext> {
   ...
   */
  async analyzeMessage(conversationId: string, message: string, isFromCustomer: boolean): Promise<ConversationContext> {
   ...
   */
  private async extractCustomerInsights(context: ConversationContext, message: string): Promise<void> {
    ...
    if (nameMatch && !context.customerName) {
      context.customerName = (nameMatch[1] || nameMatch[2] || nameMatch[3]).trim();
    ...
    const phoneMatch = message.match(/(\d{3}[-.]?\d{3}[-.]?\d{4}|\(\d{3}\)\s?\d{3}[-.]?\d{4})/);
   ...
   */
  private async extractVehicleInterests(context: ConversationContext, message: string): Promise<void> {
    const lowerMessage = message.toLowerCase();
    
    // Extract makes and models
    const makes = ['toyota', 'honda', 'ford', 'chevrolet', 'nissan', 'hyundai', 'kia', 'volkswagen', 'bmw', 'mercedes', 'audi', 'lexus', 'acura'];
    const bodyStyles = ['sedan', 'suv', 'truck', 'coupe', 'convertible', 'hatchback', 'wagon', 'crossover'];
    
    for (const make of makes) {
      if (lowerMessage.includes(make)) {
        const existingInterest = context.previousInterests.find(i => i.make?.toLowerCase() === make);
        if (existingInterest) {
          existingInterest.mentions++;
          existingInterest.lastMentioned = new Date();
          existingInterest.interestLevel = Math.min(10, existingInterest.interestLevel + 1);
        } else {
    ...
    // Extract price range
    const priceMatch = message.match(/(\$|under |around |about |between )[\d,]+(\s*and\s*[\d,]+)?/);
    if (priceMatch && !context.currentNeed?.budget) {
      if (!context.currentNeed) context.currentNeed = { primary: 'replacement', timeline: 'exploring', financing: 'undecided' };
   ...
   */
  private async extractCustomerNeeds(context: ConversationContext, message: string): Promise<void> {
    const lowerMessage = message.toLowerCase();
    
    if (!context.currentNeed) {
      context.currentNeed = { primary: 'replacement', timeline: 'exploring', financing: 'undecided' };
    }

    // Timeline detection
    if (lowerMessage.includes('asap') || lowerMessage.includes('immediately') || lowerMessage.includes('urgent')) {
      context.currentNeed.timeline = 'immediate';
    } else if (lowerMessage.includes('this month') || lowerMessage.includes('soon')) {
      context.currentNeed.timeline = 'within_month';
    } else if (lowerMessage.includes('few months') || lowerMessage.includes('3 months')) {
    ...
    } else if (lowerMessage.includes('cash') || lowerMessage.includes('pay full')) {
      context.currentNeed.financing = 'cash';
    }

    // Trade-in detection
    if (lowerMessage.includes('trade') || lowerMessage.includes('current car') || lowerMessage.includes('existing vehicle')) {
   ...
   */
  private async detectIntentAndStage(context: ConversationContext, message: string): Promise<void> {
    const lowerMessage = message.toLowerCase();
    
    // Intent detection
    if (lowerMessage.includes('price') || lowerMessage.includes('cost') || lowerMessage.includes('payment')) {
      context.currentIntent = { primary: 'pricing', confidence: 0.8, details: {} };
    } else if (lowerMessage.includes('schedule') || lowerMessage.includes('appointment') || lowerMessage.includes('test drive')) {
      context.currentIntent = { primary: 'scheduling', confidence: 0.9, details: {} };
    } else if (lowerMessage.includes('financing') || lowerMessage.includes('loan') || lowerMessage.includes('lease')) {
      context.currentIntent = { primary: 'financing', confidence: 0.8, details: {} };
    } else if (lowerMessage.includes('trade') || lowerMessage.includes('trade-in')) {
      context.currentIntent = { primary: 'trade_in', confidence: 0.8, details: {} };
    }

    // Stage progression
    if (context.conversationStage === 'greeting' && context.totalMessages > 2) {
      context.conversationStage = 'discovery';
    } else if (context.conversationStage === 'discovery' && context.previousInterests.length > 0) {
      context.conversationStage = 'exploration';
    } else if (context.conversationStage === 'exploration' && context.currentIntent?.primary === 'pricing') {
  ...
  private detectSentiment(message: string): 'positive' | 'neutral' | 'negative' {
    ...
    const positiveWords = ['great', 'excellent', 'love', 'perfect', 'amazing', 'fantastic', 'interested', 'excited'];
    const negativeWords = ['terrible', 'awful', 'hate', 'disappointed', 'frustrated', 'annoyed', 'expensive'];
  ...
  private detectUrgency(message: string): 'low' | 'medium' | 'high' {
    ...
    if (lowerMessage.includes('soon') || lowerMessage.includes('this week') || lowerMessage.includes('need quickly')) {
      return 'medium';
    }
    return 'low';
  }
   ...
   * Update conversation state
   */
  private async updateConversationState(context: ConversationContext, message: string): Promise<void> {
    // Update conversation summary
    if (context.conversationSummary.length > 500) {
      // Compress summary if getting too long
      context.conversationSummary = await this.compressConversationSummary(context.conversationSummary, message);
    } else {
   ...
   */
  private async compressConversationSummary(currentSummary: string, newMessage: string): Promise<string> {
    // Extract key points and compress
    const keyPoints = currentSummary.split('.').filter(point => 
   ...
   */
  private async loadConversationContext(conversationId: string): Promise<ConversationContext | null> {
    try {
      ...
      return {
        ...
        currentIntent: row.current_intent ? JSON.parse(row.current_intent) : undefined,
        customerSentiment: row.customer_sentiment,
        urgencyLevel: row.urgency_level,
        previousInterests: row.previous_interests ? JSON.parse(row.previous_interests) : [],
        currentNeed: row.current_need ? JSON.parse(row.current_need) : undefined,
