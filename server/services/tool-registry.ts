...
import { db } from '../db';
import { tools, agent_tools, Tool, AgentTool, InsertTool, InsertAgentTool } from '../../shared/schema';
...
class ToolRegistryService extends EventEmitter {
  ...
  private initializeBuiltInTools(): void {
    ...
    this.registerInMemoryTool({
      ...
      description: 'Analyzes data and answers questions using the Watchdog analytics engine',
      version: '1.0.0',
      category: 'analytics',
      requiresAuth: true,
  ...
  private async handleVinAgentTask(
    parameters: { taskType: string; parameters: Record<string, any>; priority?: 'low' | 'medium' | 'high' },
    context: ToolContext
  ): Promise<any> {
  ...
  public async registerTool(toolData: InsertTool): Promise<Tool> {
    try {
      ...
      const [insertedTool] = await db.insert(tools).values(toolData).returning();
   ...
   * Update a tool
   */
  public async updateTool(id: number, toolData: Partial<InsertTool>): Promise<Tool | null> {
    try {
   ...
   */
  public async assignToolToAgent(agentId: string, toolId: number, config?: Partial<InsertAgentTool>): Promise<AgentTool> {
    try {
      ...
      if (existingAssignment) {
        ...
        const [updatedAssignment] = await db.update(agent_tools)
          .set({
            ...config,
      ...
      const [newAssignment] = await db.insert(agent_tools)
        .values({
          agent_id: agentId,
          tool_id: toolId,
   ...
   */
  public async unassignToolFromAgent(agentId: string, toolId: number): Promise<boolean> {
    try {
      const [deletedAssignment] = await db.delete(agent_tools)
        .where(and(
          eq(agent_tools.agent_id, agentId),
          eq(agent_tools.tool_id, toolId)
        ))
   ...
   */
  public async agentHasToolAccess(agentId: string, toolName: string): Promise<boolean> {
    try {
      // Get the tool ID
   ...
   */
  private handleToolExecution(data: { toolName: string; processingTime: number; context: ToolContext }): void {
    // Log the event
  ...
  private logMetrics(): void {
    ...
    for (const [toolName, times] of Object.entries(this.metricsCollector.executionTimes)) {
      if (times.length > 0) {
    ...
    logger.info('Tool registry metrics', {
      ...
      successRate: this.metricsCollector.totalExecutions > 0
        ? (this.metricsCollector.successfulExecutions / this.metricsCollector.totalExecutions) * 100
        : 100,
