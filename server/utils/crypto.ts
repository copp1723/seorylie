...
export async function decrypt(encryptedBuffer: Buffer): Promise<string> {
  try {
    ...
    if (error instanceof Error && error.message.includes('Unsupported state or unable to authenticate data')) {
      logger.error('Data integrity error during decryption - possible tampering', {
        error: error.message
      });
 ...
 */
export async function encryptField(value: string | null | undefined): Promise<Buffer | null> {
 ...
 */
export async function decryptField(value: Buffer | null | undefined): Promise<string | null> {
  if (value === null || value === undefined) {
    return null;
  }
 ...
 */
export function maskPII(value: string | null | undefined, type: 'email' | 'phone' | 'name' = 'email'): string {
