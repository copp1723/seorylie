...
function validateCriticalEnvVars(): void {
  ...
  const missingVars = CRITICAL_ENV_VARS.filter(
...
function validateSecretStrength(): void {
  ...
  if (
    ...
    process.env.SESSION_SECRET.length < MIN_SECRET_LENGTH
  ) {
  ...
  try {
    ...
    if (isProduction && !connectionString.includes("ssl=true")) {
      logger.warn("Database connection may not be using SSL encryption", {
        action:
          "Update DATABASE_URL to include ssl=true for secure connections",
      });
    }
...
async function validateRowLevelSecurity(): Promise<void> {
  ...
  try {
    ...
    const rlsFunctionCount = parseInt(
      String(rlsFunctionResult?.[0]?.count || "0"),
      10,
    );
    ...
    console.error(
      ...
      "WARNING: Unable to validate Row Level Security configuration",
    );
  }
}
...
function logFatalError(message: string, code: string): never {
  logger.error(
    `FATAL ERROR [${code}]: Production safety check failed`,
    new Error(message),
  );
 ...
 */
export async function validateConfigurationAspect(
  aspect: "env" | "database" | "rls" | "auth",
): Promise<boolean> {
  try {
    switch (aspect) {
      case "env":
