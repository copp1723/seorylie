...
const HEALTH_LOG_INTERVAL = parseInt(process.env.HEALTH_LOG_INTERVAL || '300000'); // 5 minutes
...
function createHealthApp(): express.Application {
  ...
  app.get('/healthz', async (req, res) => {
    try {
      ...
      const healthStatus = {
        status: dbHealth.isHealthy && servicesHealth.status === 'healthy' ? 'healthy' : 'unhealthy',
      ...
      if (healthStatus.status === 'healthy') {
        logger.debug('ADF Worker health check passed', { responseTime: `${responseTime}ms` });
      } else {
