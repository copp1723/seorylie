...
const DEFAULT_SENSITIVE_FIELDS = [
  ...
  'password', 'passwd', 'secret', 'token', 'api_key', 'apikey', 'api_secret', 'apisecret',
  'access_token', 'accesstoken', 'refresh_token', 'refreshtoken', 'auth_token', 'authtoken',
  'session_token', 'sessiontoken', 'jwt', 'private_key', 'privatekey', 'secret_key', 'secretkey',
  
  // Personal information
  'ssn', 'social_security', 'socialsecurity', 'social_security_number', 'socialsecuritynumber',
  'dob', 'date_of_birth', 'dateofbirth', 'birth_date', 'birthdate',
  'dl_number', 'dlnumber', 'drivers_license', 'driverslicense', 'license_number', 'licensenumber',
  'passport', 'passport_number', 'passportnumber',
  
  // Contact information
  'email', 'email_address', 'emailaddress', 'phone', 'phone_number', 'phonenumber',
  'mobile', 'mobile_number', 'mobilenumber', 'cell', 'cell_number', 'cellnumber',
  'address', 'street_address', 'streetaddress', 'home_address', 'homeaddress',
  'zip', 'zip_code', 'zipcode', 'postal_code', 'postalcode',
  
  // Financial information
  'cc', 'cc_number', 'ccnumber', 'credit_card', 'creditcard', 'card_number', 'cardnumber',
  'cvv', 'cvc', 'security_code', 'securitycode', 'card_security', 'cardsecurity',
  'expiry', 'expiration', 'expiry_date', 'expirydate', 'expiration_date', 'expirationdate',
  'bank_account', 'bankaccount', 'account_number', 'accountnumber', 'routing_number', 'routingnumber',
  'tax_id', 'taxid', 'ein', 'employer_id', 'employerid',
  
  // Health information
  'medical_record', 'medicalrecord', 'health_id', 'healthid', 'insurance_id', 'insuranceid',
  'patient_id', 'patientid', 'diagnosis', 'treatment', 'medication'
];
 ...
 */
export function redactSensitivePatterns(input: string, options: LogRedactionOptions = DEFAULT_OPTIONS): string {
  ...
  if (options.customPatterns) {
    Object.entries(options.customPatterns).forEach(([_, pattern]) => {
      redacted = redacted.replace(pattern, options.replacementString || '[REDACTED]');
    });
  }
 ...
 */
function isSensitiveField(fieldName: string, options: LogRedactionOptions): boolean {
...
export function redactSensitiveInfo(
  ...
  if (typeof obj === 'string') {
    ...
    // Handle arrays
    return obj.map(item => redactSensitiveInfo(item, options, currentDepth + 1));
  }
 ...
 */
function redactHeaders(headers: Record<string, any>, options: LogRedactionOptions): Record<string, any> {
  const result: Record<string, any> = {};
  
  for (const [key, value] of Object.entries(headers)) {
    // Skip safe headers
    if (options.safeHeaders?.some(h => h.toLowerCase() === key.toLowerCase())) {
      result[key] = value;
      continue;
    }
 ...
 */
function redactQueryParams(query: Record<string, any>, options: LogRedactionOptions): Record<string, any> {
 ...
 */
function createRedactedRequest(req: Request, options: LogRedactionOptions): Record<string, any> {
  ...
  if (options.redactHeaders && req.headers) {
    redacted.headers = redactHeaders(req.headers as Record<string, any>, options);
  }
  ...
  if (options.redactPathParams && req.params && Object.keys(req.params).length > 0) {
 ...
 */
function createRedactedResponse(res: Response, options: LogRedactionOptions): Record<string, any> {
  const redacted: Record<string, any> = {
  ...
  if (options.redactHeaders && res.getHeaders) {
    redacted.headers = redactHeaders(res.getHeaders() as Record<string, any>, options);
  }
 ...
 */
export function logRedactionMiddleware(customOptions: Partial<LogRedactionOptions> = {}): (req: Request, res: Response, next: NextFunction) => void {
    ...
    const errorHandler = (err: Error) => {
      try {
        ...
        const redactedError = {
          ...
          stack: process.env.NODE_ENV === 'production' ? undefined : redactSensitivePatterns(err.stack || '', options),
          name: err.name
        };
 ...
 */
export function setupLoggerRedaction(customOptions: Partial<LogRedactionOptions> = {}): void {
  const options: LogRedactionOptions = {
    ...DEFAULT_OPTIONS,
    ...customOptions
  };
  ...
  logger.info('Logger redaction configured', {
    sensitiveFieldsCount: DEFAULT_SENSITIVE_FIELDS.length + (options.additionalFields?.length || 0),
    patternsCount: Object.keys(PII_PATTERNS).length + Object.keys(options.customPatterns || {}).length
  });
}
