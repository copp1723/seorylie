...
export const DATABASE_CONFIG = {
  CONNECTION_STRING:
    ...
    "postgresql://user:password@localhost:5432/cleanrylie",
...
export const WEBSOCKET_CONFIG = {
  ...
  CONNECTION_TIMEOUT: parseInt(
    process.env.WS_CONNECTION_TIMEOUT || "300000",
    10,
  ), // 5 minutes
...
export function validateRequiredEnvVars(): void {
  const requiredInProduction = ["SESSION_SECRET", "JWT_SECRET", "DATABASE_URL"];

  if (SERVER_CONFIG.NODE_ENV === "production") {
    for (const envVar of requiredInProduction) {
      if (!process.env[envVar]) {
        throw new Error(
          `Required environment variable ${envVar} is not set in production`,
        );
      }
    }
  }
