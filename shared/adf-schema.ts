...
export type AdfProcessingStatus =
  | "pending"
  | "processing"
  | "processed"
  | "failed"
  | "retrying";
export type AdfLeadStatus =
  | "new"
  | "contacted"
  | "qualified"
  | "converted"
...
export const adfLeads = pgTable(
  ...
  (table) => ({
    ...
    deduplicationIdx: index("adf_leads_deduplication_idx").on(
      table.deduplicationHash,
    ),
    dealershipIdx: index("adf_leads_dealership_idx").on(table.dealershipId),
    customerEmailIdx: index("adf_leads_customer_email_idx").on(
      table.customerEmail,
    ),
    customerPhoneIdx: index("adf_leads_customer_phone_idx").on(
      table.customerPhone,
    ),
    ...
    processingStatusIdx: index("adf_leads_processing_status_idx").on(
      table.processingStatus,
    ),

    // Unique constraint for deduplication
    uniqueDeduplication: unique("adf_leads_unique_deduplication").on(
      table.deduplicationHash,
    ),
  }),
);
...
export const adfEmailQueue = pgTable(
  "adf_email_queue",
  {
    id: serial("id").primaryKey(),
    ...
    attachmentInfo: json("attachment_info")
      .$type<
        Array<{
          filename: string;
          contentType: string;
  ...
  (table) => ({
    // Indexes
    emailMessageIdIdx: index("adf_email_queue_message_id_idx").on(
      table.emailMessageId,
    ),
    processingStatusIdx: index("adf_email_queue_processing_status_idx").on(
      table.processingStatus,
    ),
    ...
    resultingLeadIdx: index("adf_email_queue_resulting_lead_idx").on(
      table.resultingLeadId,
    ),

    // Unique constraint for email message ID
    uniqueEmailMessage: unique("adf_email_queue_unique_message").on(
      table.emailMessageId,
    ),
  }),
);
...
export const adfProcessingLogs = pgTable(
  "adf_processing_logs",
  {
    id: serial("id").primaryKey(),
  ...
  (table) => ({
    ...
    emailQueueIdx: index("adf_processing_logs_queue_idx").on(
      table.emailQueueId,
    ),
    ...
    createdAtIdx: index("adf_processing_logs_created_at_idx").on(
      table.createdAt,
    ),
  }),
);
...
export const adfEmailQueueRelations = relations(
  adfEmailQueue,
  ({ one, many }) => ({
...
export const adfProcessingLogsRelations = relations(
  adfProcessingLogs,
  ({ one }) => ({
...
export const insertAdfProcessingLogSchema = createInsertSchema(
  adfProcessingLogs,
).omit({
  id: true,
  createdAt: true,
...
export type InsertAdfProcessingLog = z.infer<
  typeof insertAdfProcessingLogSchema
>;
