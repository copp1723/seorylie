...
export const leadSourcesTable = pgTable(
  ...
  (table) => ({
    ...
    uniqueName: unique("lead_sources_unique_name").on(
      table.dealershipId,
      table.name,
    ),
  }),
...
export const dealershipHandoverSettings = pgTable(
  "dealership_handover_settings",
  {
    ...
    dossierTemplate: varchar("dossier_template", { length: 100 }).default(
      "default",
    ),
  ...
  (table) => ({
    dealershipIdx: index("dealership_handover_settings_dealership_idx").on(
      table.dealershipId,
    ),
    uniqueDealership: unique("unique_dealership_handover_settings").on(
      table.dealershipId,
    ),
  }),
);
...
export const customers = pgTable(
  "customers",
  {
    ...
    preferredLanguage: varchar("preferred_language", { length: 20 }).default(
      "en",
    ),
  ...
  (table) => ({
    ...
    uniqueDedup: unique("customers_unique_dedup").on(
      table.dealershipId,
...
export const vehicleInterests = pgTable(
  ...
  (table) => ({
    makeModelIdx: index("vehicle_interests_make_model_idx").on(
      table.make,
      table.model,
    ),
...
export const leads = pgTable(
  "leads",
  {
    ...
    vehicleInterestId: uuid("vehicle_interest_id").references(
      () => vehicleInterests.id,
    ),
  ...
  (table) => ({
    ...
    uniqueLeadNumber: unique("leads_unique_lead_number").on(
      table.dealershipId,
      table.leadNumber,
    ),
    uniqueDedup: unique("leads_unique_dedup").on(
      table.dealershipId,
      table.deduplicationHash,
    ),
  }),
...
export const conversations = pgTable(
  ...
  (table) => ({
    ...
    assignedUserIdx: index("conversations_assigned_user_idx").on(
...
export const messages = pgTable(
  ...
  (table) => ({
    ...
    externalIdIdx: index("messages_external_id_idx").on(
...
export const handovers = pgTable(
  ...
  (table) => ({
    conversationIdx: index("handovers_conversation_idx").on(
      table.conversationId,
    ),
...
export const leadSourcesRelations = relations(
  leadSourcesTable,
  ({ one, many }) => ({
...
export const dealershipHandoverSettingsRelations = relations(
...
export const vehicleInterestsRelations = relations(
  vehicleInterests,
  ({ many }) => ({
    leads: many(leads),
  }),
...
export type DealershipHandoverSettings =
  typeof dealershipHandoverSettings.$inferSelect;
export type InsertDealershipHandoverSettings = z.infer<
  typeof insertDealershipHandoverSettingsSchema
>;
