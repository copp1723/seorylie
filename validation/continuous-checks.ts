...
class ContinuousValidator {
  ...
  private async validateApiHealth(): Promise<void> {
    ...
    for (const endpoint of CONFIG.endpoints) {
      ...
      try {
        ...
        if (response.status >= 200 && response.status < 300) {
          if (duration > CONFIG.thresholds.apiResponseTime) {
            ...
            this.violations.push(
              `API endpoint ${endpoint.path} responding slowly (${duration}ms)`,
            );
          } else {
            this.addCheck({
              name: checkName,
          ...
          this.violations.push(
            `API endpoint ${endpoint.path} returned HTTP ${response.status}`,
          );
        }
      } catch (error) {
        const duration = Date.now() - checkStart;
        ...
        this.violations.push(
          `API endpoint ${endpoint.path} failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
  }

  private async makeApiCall(endpoint: {
    path: string;
    method: string;
    timeout: number;
  ...
  private async validateDatabaseSchema(): Promise<void> {
    ...
    try {
      ...
      this.violations.push(
        `Database schema validation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async checkDatabaseConnection(): Promise<void> {
    ...
    try {
      ...
      const result = await execAsync('echo "SELECT 1;" | head -1', {
        timeout: CONFIG.thresholds.dbConnectionTime,
      });
      ...
      if (connectionTime > CONFIG.thresholds.dbConnectionTime) {
        ...
        this.violations.push(
          `Database connection slower than expected (${connectionTime}ms)`,
        );
      }
    } catch (error) {
      this.addCheck({
    ...
    try {
      ...
      for (const migration of migrationFiles) {
        ...
        if (!rollbackFiles.includes(rollbackName)) {
          this.violations.push(
            `Migration ${migration} missing rollback file ${rollbackName}`,
          );
          this.addCheck({
  ...
  private async checkMigrationStatus(): Promise<void> {
    try {
      ...
      if (pendingMigrations > 0) {
        ...
        this.violations.push(
          `${pendingMigrations} pending database migrations detected`,
        );
      }
    } catch (error) {
  ...
  private async validatePerformanceBaselines(): Promise<void> {
    ...
    try {
      ...
      if (metrics.memoryUsage > CONFIG.thresholds.memoryUsage) {
        ...
        this.violations.push(
          `Memory usage above threshold: ${metrics.memoryUsage}%`,
        );
      } else {
        this.addCheck({
          name: "Memory Usage",
    ...
    try {
      ...
      if (errorCount > CONFIG.thresholds.typeScriptErrors) {
        this.addCheck({
          ...
          details: {
            errorCount,
            threshold: CONFIG.thresholds.typeScriptErrors,
          },
          severity: "high",
        });
        this.violations.push(
          `${errorCount} TypeScript errors detected (threshold: ${CONFIG.thresholds.typeScriptErrors})`,
        );
      } else {
        this.addCheck({
          name: "TypeScript Errors",
    ...
    try {
      ...
      if (circularCount > CONFIG.thresholds.circularDependencies) {
        this.addCheck({
          ...
          details: {
            circularCount,
            threshold: CONFIG.thresholds.circularDependencies,
          },
          severity: "medium",
        });
        this.violations.push(
          `${circularCount} circular dependencies detected (threshold: ${CONFIG.thresholds.circularDependencies})`,
        );
      } else {
        this.addCheck({
          name: "Circular Dependencies",
  ...
  private determineExitCode(result: ValidationResult): number {
    if (result.status === "fail") {
      console.log(
        `❌ Validation failed: ${result.violations.length} violations found`,
      );
      result.violations.forEach((violation) => console.log(`  - ${violation}`));
      return 1;
    } else if (result.status === "warning") {
      console.log(
        `⚠️  Validation passed with warnings: ${result.violations.length} issues found`,
      );
      result.violations.forEach((violation) => console.log(`  - ${violation}`));
      return 0; // Warnings don't fail the check
    } else {
      console.log(
        `✅ All validations passed (${result.summary.passed}/${result.summary.total} checks)`,
      );
      return 0;
    }
  }
