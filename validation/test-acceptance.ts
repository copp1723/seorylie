...
class AcceptanceTest {
  ...
  private async testFirstRunGeneratesOutput(): Promise<void> {
    ...
    try {
      ...
      try {
        await execAsync("NODE_ENV=test npm run validation:run", {
          timeout: 120000,
        });
      } catch (error) {
      ...
      console.log(
        `   ${fileExists && meetsSize ? "✅" : "❌"} File: ${fileExists ? `${fileSizeKB} KB` : "Not found"}\n`,
      );
    } catch (error) {
      this.results.push({
        name: "First run generates output ≥1 KB",
  ...
  private async testSecondRunWithNoNewViolations(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   ${noNewViolations ? "✅" : "❌"} Violations: ${initialViolations} → ${newViolations}\n`,
      );
    } catch (error) {
      this.results.push({
        name: "Second run behavior with no new violations",
  ...
  private async testFakeViolationDetection(): Promise<void> {
    ...
    try {
      ...
      const originalContent = await fs.readFile(
        "validation/continuous-checks.ts",
        "utf-8",
      );
      ...
      try {
        ...
        const hasFakeViolation =
          data.violations?.includes("Fake violation for testing") || false;
        ...
        console.log(
          `   ${exitCode === 1 && hasFakeViolation ? "✅" : "❌"} Exit: ${exitCode}, Violations: ${violationsFound}\n`,
        );
      } catch (error) {
        this.results.push({
          name: "Fake violation detection",
