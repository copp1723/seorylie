...
class ValidationDaemon {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `🏥 Health check: http://localhost:${CONFIG.healthCheckPort}/health`,
      );
      console.log(
        `📊 Status: http://localhost:${CONFIG.healthCheckPort}/status`,
      );
  ...
  private async runValidation(): Promise<void> {
    ...
    console.log(
      `🔍 Starting validation run #${this.status.runCount + 1} at ${new Date().toISOString()}`,
    );

    try {
      // Execute the continuous validation script
      const { stdout, stderr } = await execAsync(
        `npx tsx ${CONFIG.validationScript}`,
        {
      ...
      console.log(
        `✅ Validation run #${this.status.runCount} completed in ${duration}ms`,
      );
      ...
      console.error(
        `❌ Validation run #${this.status.runCount + 1} failed after ${duration}ms:`,
        errorMessage,
      );
    }
  }
  ...
  private async validateOutput(): Promise<void> {
    try {
      ...
      if (stats.size >= 1024) {
        // At least 1KB as per acceptance criteria
        console.log(
          `📊 Validation output: ${CONFIG.outputFile} (${fileSizeKB} KB)`,
        );
      } else {
        throw new Error(
          `Validation output file too small: ${fileSizeKB} KB (expected ≥1 KB)`,
        );
      }
    } catch (error) {
