...
export async function createTestCustomer(dealershipId: number = 1) {
  ...
  const customerData = {
    ...
    totalPurchases: 0,
    customFields: {},
    deduplicationHash: crypto
      .createHash("sha256")
      .update(
        `test.customer.${Date.now()}@example.com|5551234567|Test Customer`,
      )
      .digest("hex"),
  };
...
export async function cleanupTestData() {
  ...
  await db
    .delete(customers)
    .where(like(customers.email, "test.customer.%@example.com"));
  await db
    .delete(leadSourcesTable)
    .where(eq(leadSourcesTable.name, "Test API Source"));
}
