...
import { searchInventory, getVehicleDetails } from '../../server/services/agentSquad/inventory-functions';
...
describe('Agent Squad System Integration', () => {
  ...
  describe('End-to-End Customer Journey Scenarios', () => {
    it('should handle complete new customer vehicle shopping journey', async () => {
      ...
      expect(greetingResult.selectedAgent).toMatch(/general-agent|inventory-agent/);
    ...
    it('should handle frustrated customer escalation scenario', async () => {
      ...
      expect(angryResult.response).toContain('connect you with one of our team members');
    });
  ...
  describe('Analytics and Tracking', () => {
    ...
    it('should provide detailed routing decisions', async () => {
      ...
      if (result.sentiment) {
        expect(['neutral', 'curious', 'happy', 'excited']).toContain(result.sentiment);
      }
    });
  });
