...
describe("STAB-307: Agent Squad System Validation", () => {
  ...
  describe("Advanced Routing Engine", () => {
    ...
    it("should perform complete routing analysis", async () => {
      ...
      expect(["low", "medium", "high", "urgent"]).toContain(
        analysis.routingDecision.priority,
      );
  ...
  describe("Inventory Functions Validation", () => {
    ...
    it("should provide inventory summary statistics", async () => {
      ...
      if (summary.success && summary.summary) {
        ...
        expect(summary.summary.priceRange.min).toBeLessThanOrEqual(
          summary.summary.priceRange.max,
        );
      }
    });
    ...
    it("should create enhanced inventory handlers", () => {
      ...
      expect(typeof handlers.searchInventoryWithRecommendations).toBe(
        "function",
      );
  ...
  describe("System Integration and Performance", () => {
    ...
    it("should maintain conversation history", async () => {
      ...
      await agentSquad.routeMessage("First message", "test-user", sessionId, {
        dealershipId: 1,
      });

      await agentSquad.routeMessage("Second message", "test-user", sessionId, {
        dealershipId: 1,
      });
  ...
  describe("Agent Specialization Verification", () => {
    it("should have different agents handle different types of requests", async () => {
      const agentMappings = [
        {
          message: "Show me Honda Accords",
          expectedPattern: /inventory|general/,
        },
        {
          message: "What financing do you offer?",
          expectedPattern: /finance|general/,
        },
        {
          message: "I need service for my car",
          expectedPattern: /service|general/,
        },
