            ...
            message: {
              ...
              function_call: {
                ...
                arguments: JSON.stringify({
                  ...
                  intentType: 'purchase',
                  confidence: 0.92,
                  reasoning: 'Customer is asking about purchasing the vehicle'
                })
              }
            }
          }]
...
describe('Intent Detection System', () => {
  ...
  describe('IntentOrchestrator', () => {
    ...
    test('should initialize with all components', () => {
      ...
      expect(monitoringService.registerMetric).toHaveBeenCalledWith('intent_detection_total', 'counter');
      expect(monitoringService.registerMetric).toHaveBeenCalledWith('intent_detection_latency_ms', 'histogram');
    });
    ...
    test('should detect intent from rule engine', async () => {
      ...
      // Execute
      await orchestrator.processMessage(message, conversation, dealershipConfig);
      ...
      expect(emitSpy).toHaveBeenCalledWith('handover.intent.triggered', expect.objectContaining({
      ...
      expect(monitoringService.incrementMetric).toHaveBeenCalledWith(
        ...
        { family: 'rule', dealership_id: conversation.dealershipId, outcome: 'triggered' }
      );
    ...
    test('should detect intent from ML classifier when rule engine finds none', async () => {
      ...
      expect(monitoringService.incrementMetric).toHaveBeenCalledWith(
        'intent_detection_total',
        { family: 'ml', dealership_id: conversation.dealershipId, outcome: 'triggered' }
      );
    });
    ...
    test('should not emit event when no intent is detected', async () => {
      ...
      expect(emitSpy).not.toHaveBeenCalledWith('handover.intent.triggered', expect.anything());
      
      // Verify metrics were still recorded
      expect(monitoringService.incrementMetric).toHaveBeenCalledWith(
        'intent_detection_total',
        { family: 'rule', dealership_id: conversation.dealershipId, outcome: 'no_trigger' }
      );
    });
    ...
    test('should debounce multiple intent detections within 10 seconds', async () => {
      ...
      await orchestrator.processMessage(message1, conversation, dealershipConfig);
      
      // Execute second message immediately after
      await orchestrator.processMessage(message2, conversation, dealershipConfig);
    ...
    test('should handle errors gracefully', async () => {
      ...
      ruleEngine.evaluateMessage = jest.fn().mockRejectedValue(new Error('Test error'));
  ...
  describe('RuleEngine', () => {
    ...
    test('should detect intent from exact phrase match', async () => {
      ...
      // Execute
      const result = await ruleEngine.evaluateMessage(message, conversation, rules);
    ...
    test('should complete evaluation in under 50ms for performance', async () => {
      ...
      const message = { 
        ...mockMessage,
        content: 'I want to buy this car and also schedule a test drive and discuss financing options and trade-in value and check availability'
      };
      const conversation = { ...mockConversation };
      const rules = {
  ...
  describe('MLClassifier', () => {
    ...
    test('should classify intent using OpenAI function calls', async () => {
      ...
      // Execute
      const result = await mlClassifier.classifyIntent(message, conversation, threshold);
    ...
    test('should cache results for the same message content', async () => {
      ...
      // Mock the OpenAI API
      const openaiSpy = jest.spyOn(mlClassifier['openai'], 'createChatCompletion');
    ...
    test('should handle OpenAI errors gracefully', async () => {
      ...
      mlClassifier['openai'].createChatCompletion = jest.fn().mockRejectedValue(
    ...
    test('should extract function call parameters correctly', async () => {
      ...
      mlClassifier['openai'].createChatCompletion = jest.fn().mockResolvedValue({
        data: {
          choices: [{
            message: {
              content: null,
  ...
  describe('BehaviouralMonitor', () => {
      ...
      db.execute.mockImplementation((query) => {
        ...
        if (query.toString().includes('SELECT')) {
          return Promise.resolve([
            { id: 1, content: 'Hello', is_from_customer: true, created_at: new Date(Date.now() - 1000 * 60 * 10) },
            { id: 2, content: 'How can I help?', is_from_customer: false, created_at: new Date(Date.now() - 1000 * 60 * 9) },
            { id: 3, content: 'I want to know more', is_from_customer: true, created_at: new Date(Date.now() - 1000 * 60 * 5) }
          ]);
        }
    ...
    test('should detect engaged conversation based on message count', async () => {
      ...
      // Execute
      const result = await behaviouralMonitor.evaluateEngagement(message, conversation, config);
    ...
    test('should respect time window configuration', async () => {
      ...
      db.execute.mockImplementation((query) => {
        ...
        if (query.toString().includes('SELECT')) {
          return Promise.resolve([
            { id: 3, content: 'I want to know more', is_from_customer: true, created_at: new Date(Date.now() - 1000 * 60 * 3) }
          ]);
        }
      ...
      db.execute.mockImplementation((query) => {
        ...
        if (query.toString().includes('SELECT')) {
          return Promise.resolve([
            { id: 1, content: 'Hello', is_from_customer: true, created_at: new Date(Date.now() - 1000 * 60 * 10) },
            { id: 2, content: 'How can I help?', is_from_customer: false, created_at: new Date(Date.now() - 1000 * 60 * 9) },
            { id: 3, content: 'I am very interested in this vehicle and would like to discuss financing options and possibly schedule a test drive this weekend. What kind of deals are you offering currently?', is_from_customer: true, created_at: new Date(Date.now() - 1000 * 60 * 5) }
          ]);
        }
    ...
    beforeEach(() => {
      ...
      db.execute.mockImplementation((query) => {
        ...
        if (query.toString().includes('last_customer_message_at')) {
          return Promise.resolve([{
            last_customer_message_at: new Date(Date.now() - 1000 * 60 * 60 * 24) // 24 hours ago
          }]);
        }
  ...
  describe('UnifiedConfigService', () => {
    ...
    test('should get dealership configuration', async () => {
      ...
      configService.getDealershipConfig = jest.fn().mockResolvedValue(mockDealershipConfig);
    ...
    test('should check feature flag status', () => {
      ...
      configService.isFeatureEnabled = jest.fn().mockImplementation((flagName, dealershipId) => {
      ...
      // Execute
      const isEnabled = configService.isFeatureEnabled('INTENT_DETECTION_V2', 456);
    ...
    test('should get A/B test variant', () => {
      ...
      configService.getABTestVariant = jest.fn().mockImplementation((testName, dealershipId) => {
    ...
    test('should emit events on configuration changes', async () => {
      ...
      // Verify
      expect(emitSpy).toHaveBeenCalledWith('config.global.changed', expect.any(String));
    });
    ...
    test('should apply A/B test variants to configuration', async () => {
      ...
      // Execute
      const result = await configService['applyABTestVariants'](baseConfig, 456);
    ...
    test('should gracefully handle configuration errors', async () => {
      // Setup
      configService.getDealershipConfig = jest.fn().mockRejectedValue(new Error('Config error'));
      ...
      expect(monitoringService.incrementMetric).toHaveBeenCalledWith('config_service_errors_total');
    });
  });
  ...
  describe('Integration Tests', () => {
    ...
    test('should achieve ≥90% precision on test dataset', async () => {
      ...
      for (const testCase of testMessages) {
        ...
        // Process message
        await orchestrator.processMessage(message, mockConversation, mockDealershipConfig);
    ...
    test('should meet end-to-end latency requirement of ≤2s', async () => {
      ...
      // Get config
      const config = await configService.getDealershipConfig(mockConversation.dealershipId);
    ...
    test('should support configuration hot-reloading without restart', async () => {
      ...
      const newConfig = await configService.getDealershipConfig(mockConversation.dealershipId);
