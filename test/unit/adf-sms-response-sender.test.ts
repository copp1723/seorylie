...
describe("AdfSmsResponseSender", () => {
  ...
  describe("initialization", () => {
    test("should initialize and register metrics", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "Initializing ADF SMS Response Sender",
      );
      expect(logger.info).toHaveBeenCalledWith(
        "ADF SMS Response Sender initialized successfully",
      );
    });
    ...
    test("should handle initialization errors", async () => {
      ...
      await expect(adfSmsResponseSender.initialize()).rejects.toThrow(
        "Registration failed",
      );
      expect(logger.error).toHaveBeenCalledWith(
        "Failed to initialize ADF SMS Response Sender",
        expect.any(Object),
      );
    });
  });
  ...
  describe("event handling", () => {
    ...
    test("should handle lead.response.ready event", async () => {
      ...
      jest
        .spyOn(adfSmsResponseSender as any, "extractPhoneNumber")
        .mockReturnValue(mockPhoneNumber);

      // Mock sendSms to resolve successfully
      const sendSmsSpy = jest
        .spyOn(adfSmsResponseSender as any, "sendSms")
        .mockResolvedValue(undefined);

      // Mock checkOptOutStatus to return false (not opted out)
      (twilioSMSService.checkOptOutStatus as jest.Mock).mockResolvedValue(
        false,
      );
      ...
      expect(adfSmsResponseSender["extractPhoneNumber"]).toHaveBeenCalledWith(
        eventData.lead,
      );
      expect(twilioSMSService.checkOptOutStatus).toHaveBeenCalledWith(
        mockDealershipId,
        mockPhoneNumber,
      );
    ...
    test("should skip sending if customer has opted out", async () => {
      ...
      expect(db.execute).toHaveBeenCalledWith(
        expect.stringContaining("UPDATE adf_leads"),
      );
  ...
  describe("SMS sending", () => {
    test("should send SMS successfully", async () => {
      ...
      const recordAttemptSpy = jest
        .spyOn(adfSmsResponseSender as any, "recordSmsAttempt")
        .mockResolvedValue(undefined);
      const updateSidSpy = jest
        .spyOn(adfSmsResponseSender as any, "updateSmsWithSid")
        .mockResolvedValue(undefined);
      const setTimeoutSpy = jest
        .spyOn(adfSmsResponseSender as any, "setDeliveryTimeout")
        .mockImplementation(() => {});
    ...
    test("should handle SMS sending failure", async () => {
      ...
      const recordAttemptSpy = jest
        .spyOn(adfSmsResponseSender as any, "recordSmsAttempt")
        .mockResolvedValue(undefined);
      const updateLeadStatusSpy = jest
        .spyOn(adfSmsResponseSender as any, "updateLeadSmsStatus")
        .mockResolvedValue(undefined);
      const scheduleRetrySpy = jest
        .spyOn(adfSmsResponseSender as any, "scheduleRetry")
        .mockImplementation(() => {});
      ...
      expect(updateLeadStatusSpy).toHaveBeenCalledWith(
        mockLeadId,
        "failed",
        "Failed to send SMS",
      );
    ...
    test("should handle exceptions during SMS sending", async () => {
      ...
      await expect(
        (adfSmsResponseSender as any).sendSms(
          mockLeadId,
          mockDealershipId,
          mockPhoneNumber,
          mockMessage,
  ...
  describe("webhook processing", () => {
    test("should process delivery status webhook for delivered status", async () => {
      ...
      jest
        .spyOn(adfSmsResponseSender as any, "getSmsResponseByMessageSid")
        .mockResolvedValue({
          leadId: mockLeadId,
          messageSid: mockMessageSid,
      ...
      const clearTimeoutSpy = jest
        .spyOn(adfSmsResponseSender as any, "clearDeliveryTimeout")
        .mockImplementation(() => {});
      ...
      expect(db.execute).toHaveBeenCalledWith(
        expect.stringContaining("UPDATE adf_sms_responses"),
      );
    ...
    test("should handle webhook processing errors", async () => {
      ...
      // Verify error was logged
      expect(logger.error).toHaveBeenCalledWith(
        "Failed to handle delivery update",
        expect.objectContaining({
          error: "Database error",
          messageSid: mockMessageSid,
        }),
    ...
    test("should process inbound SMS webhook for opt-out", async () => {
      ...
      jest
        .spyOn(adfSmsResponseSender as any, "findDealershipByPhoneNumber")
        .mockResolvedValue(mockDealershipId);

      // Mock handleOptOut
      const handleOptOutSpy = jest
        .spyOn(adfSmsResponseSender as any, "handleOptOut")
        .mockResolvedValue(undefined);
    ...
    test("should ignore inbound SMS without opt-out keywords", async () => {
      // Mock handleOptOut
      const handleOptOutSpy = jest.spyOn(
        adfSmsResponseSender as any,
        "handleOptOut",
      );
  ...
  describe("opt-out handling", () => {
    test("should handle opt-out requests", async () => {
      ...
      jest
        .spyOn(adfSmsResponseSender as any, "findLeadsByPhoneNumber")
        .mockResolvedValue([{ id: mockLeadId }, { id: mockLeadId + 1 }]);
  ...
  describe("retry logic", () => {
    ...
    test("should schedule retry for failed messages", async () => {
      ...
      jest
        .spyOn(adfSmsResponseSender as any, "getSmsResponseByLeadId")
        .mockResolvedValue({
          leadId: mockLeadId,
          dealershipId: mockDealershipId,
          phoneNumber: mockPhoneNumber,
  ...
  describe("delivery timeout handling", () => {
    ...
    test("should not update already delivered messages", async () => {
      ...
      // Mock updateLeadSmsStatus
      const updateLeadStatusSpy = jest.spyOn(
        adfSmsResponseSender as any,
        "updateLeadSmsStatus",
      );
    ...
    test("should handle errors during timeout handling", async () => {
      ...
      // Verify error was logged
      expect(logger.error).toHaveBeenCalledWith(
        "Error in delivery timeout handler",
        expect.objectContaining({
          error: "Database error",
          leadId: mockLeadId,
        }),
  ...
  describe("phone number and message formatting", () => {
    test("should extract phone number from lead data", () => {
      ...
      expect((adfSmsResponseSender as any).extractPhoneNumber(lead1)).toBe(
        mockPhoneNumber,
      );
      ...
      expect((adfSmsResponseSender as any).extractPhoneNumber(lead2)).toBe(
        mockPhoneNumber,
      );
      ...
      expect((adfSmsResponseSender as any).extractPhoneNumber(lead3)).toBe(
        mockPhoneNumber,
      );
      ...
      expect(
        (adfSmsResponseSender as any).extractPhoneNumber(lead4),
      ).toBeNull();
    });
    ...
    test("should format message with personalization and opt-out footer", () => {
      ...
      const formatted1 = (adfSmsResponseSender as any).formatMessage(
        message1,
        lead1,
      );
      ...
      const formatted2 = (adfSmsResponseSender as any).formatMessage(
        message2,
        lead2,
      );
      ...
      const formatted3 = (adfSmsResponseSender as any).formatMessage(
        longMessage,
        lead1,
      );
    ...
    test("should handle errors in message formatting", () => {
      ...
      const formatted = (adfSmsResponseSender as any).formatMessage(
        message,
        problematicLead,
      );
  ...
  describe("database operations", () => {
    test("should record SMS attempt in database", async () => {
      ...
      expect(db.execute).toHaveBeenCalledWith(
        expect.stringContaining("SELECT id FROM adf_sms_responses"),
      );
    ...
    test("should update SMS with SID", async () => {
      ...
      expect(db.execute).toHaveBeenCalledWith(
        expect.stringContaining("message_sid = "),
      );
      expect(db.execute).toHaveBeenCalledWith(
    ...
    test("should update lead SMS status", async () => {
      ...
      expect(db.execute).toHaveBeenCalledWith(
        expect.stringContaining("sms_status = "),
      );
      expect(db.execute).toHaveBeenCalledWith(
        expect.stringContaining("sms_error = "),
      );
    });
    ...
    test("should handle database errors", async () => {
      ...
      // Verify error was logged
      expect(logger.error).toHaveBeenCalledWith(
        "Failed to update lead SMS status",
        expect.objectContaining({
          error: "Database error",
          leadId: mockLeadId,
        }),
  ...
  describe("webhook processing", () => {
    test("should process Twilio webhook", async () => {
      ...
      const handleDeliveryUpdateSpy = jest
        .spyOn(adfSmsResponseSender, "handleDeliveryUpdate")
        .mockResolvedValue(undefined);
      ...
      expect(handleDeliveryUpdateSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          leadId: mockLeadId,
          messageSid: mockMessageSid,
          status: "delivered",
    ...
    test("should handle unknown message SID in webhook", async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        "Received webhook for unknown message SID",
        expect.any(Object),
      );
    });
  ...
  describe("test SMS sending", () => {
    test("should send test SMS", async () => {
      // Mock createTestLead
      jest
        .spyOn(adfSmsResponseSender as any, "createTestLead")
        .mockResolvedValue(mockLeadId);
