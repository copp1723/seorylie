...
import { sendInvitation, verifyInvitation, cleanupExpiredInvitations } from '../../server/services/magic-link-auth';
...
describe('Magic Link Authentication', () => {
  ...
  beforeEach(async () => {
    ...
    await db.delete(magicLinkInvitations).where(eq(magicLinkInvitations.email, testEmail));
  ...
  test('should handle different roles correctly', async () => {
    ...
    for (const role of roles) {
      ...
      // Clean up
      await db.delete(magicLinkInvitations).where(eq(magicLinkInvitations.email, email));
    }
  });
});