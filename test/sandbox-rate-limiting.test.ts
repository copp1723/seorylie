...
import { orchestrator, SandboxOperationType, RateLimitExceededError } from '../server/services/orchestrator';
...
describe('Sandbox Rate Limiting', () => {
  ...
  beforeAll(async () => {
    ...
    jest.spyOn(db, 'insert').mockImplementation((...args) => testTx.insert(...args));
    jest.spyOn(db, 'select').mockImplementation((...args) => testTx.select(...args));
    jest.spyOn(db, 'update').mockImplementation((...args) => testTx.update(...args));
    jest.spyOn(db, 'delete').mockImplementation((...args) => testTx.delete(...args));
    jest.spyOn(db, 'execute').mockImplementation((...args) => testTx.execute(...args));
    jest.spyOn(db.query, 'sandboxes').mockImplementation(() => ({
      ...
      findMany: jest.fn().mockImplementation(async ({ where, orderBy }) => {
        return await testTx.select().from(sandboxes).where(where).orderBy(orderBy);
      })
    }));
    jest.spyOn(db.query, 'sandbox_sessions').mockImplementation(() => ({
      findFirst: jest.fn().mockImplementation(async ({ where }) => {
        const results = await testTx.select().from(sandbox_sessions).where(where);
        return results[0] || null;
      }),
      findMany: jest.fn().mockImplementation(async ({ where, orderBy }) => {
        return await testTx.select().from(sandbox_sessions).where(where).orderBy(orderBy);
      })
    }));
  });
  ...
  test('should create a sandbox session', async () => {
    ...
    expect(sessionContext.websocketChannel).toContain(`ws/sandbox/${testSandboxId}/`);
    
    // Verify we can retrieve the session
    const retrievedSession = await orchestrator.getSandboxSession(sessionContext.sessionId);
  ...
  test('should track token usage correctly', async () => {
    ...
    const usageLogs = await testTx.select().from(token_usage_logs)
    ...
    expect(usageLogs[0].operation_type).toBe(SandboxOperationType.AGENT_MESSAGE);
  ...
  test('should enforce rate limits and return 429 for a 5k token burst', async () => {
    ...
    if (currentHourlyUsage + tokensToUse > TEST_HOURLY_LIMIT) {
      await expect(orchestrator.trackTokenUsage({
        sandboxId: testSandboxId,
        sessionId,
        operationType: SandboxOperationType.AGENT_MESSAGE,
        tokensUsed: tokensToUse,
      ...
      await expect(orchestrator.executeToolInSandbox(
        sessionId,
        {
          toolName: 'test_tool',
    ...
    const rateCheck = await orchestrator.checkRateLimit(testSandboxId, tokensToUse);
  ...
  test('should handle multiple concurrent sessions in a sandbox', async () => {
    ...
    for (const sessionId of [session2Context.sessionId, session3Context.sessionId]) {
      try {
        await orchestrator.trackTokenUsage({
          sandboxId: testSandboxId,
          sessionId,
    ...
    if (usageStats.usageBySession) {
      ...
      const hasSession2Usage = Object.keys(usageStats.usageBySession).includes(session2Context.sessionId);
      const hasSession3Usage = Object.keys(usageStats.usageBySession).includes(session3Context.sessionId);
