...
import { gadsAccounts, gadsCampaigns, tools, sandboxes, users, dealerships } from '../../shared/schema';
...
process.env.GOOGLE_ADS_REDIRECT_URI = 'http://localhost:3000/api/ads/auth/callback';
process.env.GOOGLE_ADS_DEVELOPER_TOKEN = 'test-developer-token';
process.env.ENCRYPTION_KEY = 'test-encryption-key-12345678901234567890123456789012';
...
describe('Google Ads API Integration', () => {
  ...
  beforeAll(async () => {
    ...
    // Mock Google Ads API
    (GoogleAdsApi.prototype.listAccessibleCustomers as jest.Mock).mockResolvedValue(mockAccessibleCustomers);
  ...
  describe('OAuth Flow', () => {
    ...
    it('should handle OAuth callback and store account', async () => {
      ...
      expect(OAuth2Client.prototype.getToken).toHaveBeenCalledWith('test-auth-code');
      
      // Verify account was stored/updated in database
      const accounts = await db.select().from(gadsAccounts).where(eq(gadsAccounts.userId, testUser.id));
      expect(accounts.length).toBeGreaterThan(0);
    });

    it('should handle OAuth errors gracefully', async () => {
      // Mock an error
      (OAuth2Client.prototype.getToken as jest.Mock).mockRejectedValueOnce(new Error('Invalid code'));
  ...
  describe('Campaign Management', () => {
    it('should create a search campaign (dry-run)', async () => {
      ...
      adsApiService.createSearchCampaign = jest.fn().mockResolvedValue(testCampaign);
      ...
      expect(response.body).toHaveProperty('campaignName', testCampaign.campaignName);
      expect(response.body).toHaveProperty('isDryRun', true);
      expect(response.body).toHaveProperty('status');
      expect(adsApiService.createSearchCampaign).toHaveBeenCalledWith(campaignData);
    ...
    it('should list campaigns for an account', async () => {
      ...
      expect(response.body.campaigns[0]).toHaveProperty('campaignId', testCampaign.campaignId);
    ...
    it('should get campaign details', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty('campaignId', testCampaign.campaignId);
      expect(response.body).toHaveProperty('campaignName', testCampaign.campaignName);
  ...
  describe('Acceptance Criteria Verification', () => {
    it('should meet all acceptance criteria', async () => {
      ...
      expect(authUrlResponse.body.url).toContain('accounts.google.com/o/oauth2/auth');
      ...
      expect(campaignResponse.body).toHaveProperty('campaignName', campaignData.campaignName);
