...
import {
  adsApiService,
  AdsApiService,
} from "../../server/services/ads-api-service";
import {
  tools,
  gadsAccounts,
  gadsCampaigns,
...
beforeAll(async () => {
  ...
  (GoogleAdsApi as jest.Mock).mockImplementation(() => ({
    ...
    listAccessibleCustomers: jest
      .fn()
      .mockResolvedValue(mockGoogleAdsApiResponses.listAccessibleCustomers),
    Customer: jest.fn().mockReturnValue({
      query: jest
        .fn()
        .mockResolvedValue(mockGoogleAdsApiResponses.customerQuery),
      campaignBudgets: {
        create: jest
          .fn()
          .mockResolvedValue(mockGoogleAdsApiResponses.budgetCreate),
      },
      campaigns: {
        create: jest
          .fn()
          .mockResolvedValue(mockGoogleAdsApiResponses.campaignCreate),
      },
    }),
  }));
...
async function cleanupTestData() {
  ...
  await db
    ...
    .where(eq(gadsCampaigns.campaignName, mockCampaignRequest.campaignName));

  await db.delete(gadsAccounts).where(eq(gadsAccounts.cid, TEST_ACCOUNT_CID));
}
...
describe("Google Ads API Integration", () => {
  ...
  describe("OAuth Flow", () => {
    test("should generate OAuth URL", async () => {
      const response = await request.post("/api/ads/auth/url").send({
        userId: TEST_USER_ID,
        dealershipId: TEST_DEALERSHIP_ID,
        sandboxId: TEST_SANDBOX_ID,
      });
    ...
    test("should handle OAuth callback", async () => {
      ...
      const handleCallbackSpy = jest
        .spyOn(adsApiService, "handleCallback")
        .mockResolvedValueOnce(mockGoogleAdsAccount);

      const response = await request.get("/api/ads/auth/callback").query({
        code: "mock-auth-code",
        state: Buffer.from(
          JSON.stringify({
            userId: TEST_USER_ID,
  ...
  describe("Account Management", () => {
    ...
    test("should list Google Ads accounts", async () => {
      ...
      const testAccount = response.body.accounts.find(
        (acc) => acc.cid === TEST_ACCOUNT_CID,
      );
    ...
    test("should get account details", async () => {
      ...
      const testAccount = listResponse.body.accounts.find(
        (acc) => acc.cid === TEST_ACCOUNT_CID,
      );
      expect(testAccount).toBeTruthy();

      // Mock getAccountDetails
      const getAccountDetailsSpy = jest
        .spyOn(adsApiService, "getAccountDetails")
        .mockResolvedValueOnce(mockGoogleAdsAccount);

      const response = await request.get(`/api/ads/accounts/${testAccount.id}`);
  ...
  describe("Campaign Management", () => {
    ...
    test("should create a search campaign with dry-run flag", async () => {
      ...
      const createSearchCampaignSpy = jest
        .spyOn(adsApiService, "createSearchCampaign")
      ...
      const response = await request.post("/api/ads/campaigns").send({
        ...mockCampaignRequest,
        isDryRun: true,
      });
      ...
      expect(response.body).toHaveProperty(
        "campaignName",
        mockCampaignRequest.campaignName,
      );
      expect(createSearchCampaignSpy).toHaveBeenCalledWith(
    ...
    test("should list campaigns", async () => {
      ...
      await request.post("/api/ads/campaigns").send(mockCampaignRequest);
  ...
  describe("Tool Registry Integration", () => {
    ...
    test("should execute google_ads.createCampaign tool via tool registry", async () => {
      ...
      const executeSpy = jest
        .spyOn(toolRegistryService, "executeTool")
