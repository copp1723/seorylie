...
async function createWebSocketClients(count: number): Promise<WebSocket[]> {
  ...
  for (let i = 0; i < count; i++) {
    const ws = new WebSocket(
      `ws://localhost:${port}/ws/sandbox/${testSandboxId}`,
    );
    clients.push(ws);
...
async function runLoadTest(
  ...
  duration: number,
  rps: number,
): Promise<{ avgResponseTime: number; errorRate: number }> {
  const startTime = Date.now();
  let requestCount = 0;
  let errorCount = 0;
  ...
  test("should log errors with OTLP structured format", async () => {
    ...
    if (errorLogFile) {
      const logContent = fs.readFileSync(
        path.join(process.cwd(), "logs", errorLogFile),
        "utf8",
      );
...
describe("INT-007 H4: Redis WebSocket Scaling", () => {
  test(
    "should support 300+ concurrent WebSocket connections",
    async () => {
      ...
      const openConnections = clients.filter(
      ...
      return new Promise<number>((resolve) => {
        client.on("message", (data) => {
          ...
          if (
            ...
            message.payload.id === testMessage.payload.id
          ) {
  ...
  test("should queue messages for offline clients", async () => {
    ...
    const port = (server.address() as any).port;
    const ws = new WebSocket(
      `ws://localhost:${port}/ws/sandbox/${testSandboxId}?clientId=${offlineClientId}`,
    );
    wsClients.push(ws);
...
describe("Cross-Feature Integration", () => {
  ...
  test("should handle errors in paused sandbox correctly", async () => {
    ...
    expect(toolResponse.body.userFriendlyMessage).toContain(
      "sandbox is paused",
    );
  });
});
...
describe("Performance Testing", () => {
  test(
    "should handle 100 RPS with <1% error rate",
    async () => {
  ...
  test(
    "should maintain <50ms response times for cached endpoints under load",
    async () => {
      // Cache the result first
      await request
        .get("/api/kpi/dashboard")
...
describe("Memory Leak Detection", () => {
  test(
    "should not have memory leaks during WebSocket operations",
    async () => {
  ...
  test(
    "should not have memory leaks during cache operations",
    async () => {
...
describe("Security Boundary Testing", () => {
  test("should enforce authentication on all protected endpoints", async () => {
    ...
    const response = await request.get("/api/kpi/dashboard");
...
describe("Comprehensive Platform Validation", () => {
  test(
    "should validate all integrated systems working together",
    async () => {
