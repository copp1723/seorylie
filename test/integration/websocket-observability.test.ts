...
describe("WebSocket Observability Integration", () => {
  ...
  test("should update connection metrics when WebSocket connects/disconnects", async () => {
    ...
    expect(connectedMetrics.data).toContain(
      'websocket_connections_total{status="active"}',
    );
    expect(connectedMetrics.data).toContain(
      'websocket_connections_total{status="total"}',
    );
    ...
    expect(disconnectedMetrics.data).toContain(
  ...
  test("should update message metrics when WebSocket messages are sent", async () => {
    ...
    expect(metrics.data).toContain(
      'websocket_messages_total{type="ping",direction="received"}',
    );
    expect(metrics.data).toContain(
      'websocket_messages_total{type="pong",direction="sent"}',
    );
    expect(metrics.data).toContain(
  ...
  test("should handle multiple concurrent WebSocket connections", async () => {
    ...
    expect(healthResponse.data.websocket.connections).toBeGreaterThanOrEqual(
      connectionCount,
    );
    ...
    const messagePromises = connections.map((ws, index) => {
      ...
      return new Promise<void>((resolve, reject) => {
        ws.on("message", (data) => {
          ...
          if (
            response.type === "echo" &&
            response.message.includes(`Message from connection ${index}`)
          ) {
            resolve();
          }
        });
        setTimeout(
          () => reject(new Error(`Echo timeout for connection ${index}`)),
          3000,
        );
      });
    });
