...
import {
  adsAutomationWorker,
  AdsTaskType,
} from "../../server/services/ads-automation-worker";
...
jest.mock("../../server/services/analytics-client", () => ({
  analyticsClient: {
    answerQuestion: jest
      .fn()
      .mockImplementation(async (uploadId, question, options) => {
      ...
      const messageHandler = (message: any) => {
        if (predicate(message)) {
          this.eventHandlers
            .get("message")!
            .splice(
              this.eventHandlers.get("message")!.indexOf(messageHandler),
              1,
            );
          clearTimeout(timer);
          resolve(message);
...
async function createTestSession(sandboxId: number) {
  ...
  await db
    ...
    .where(eq(sandboxSessions.sessionId, TEST_SESSION_ID));
...
beforeAll(async () => {
  ...
  testWsClient = new TestWebSocketClient(
    `ws://localhost:${TEST_PORT}/ws/sandbox/${TEST_SANDBOX_ID}/${TEST_SESSION_ID}`,
  );
...
afterAll(async () => {
  ...
  await db
    ...
    .where(eq(tokenUsageLogs.sandboxId, TEST_SANDBOX_ID));
}, TEST_TIMEOUT);
...
describe("Cross-Service Orchestration", () => {
  ...
  describe("Sequential Workflow Execution", () => {
    it(
      "should execute a sequential workflow from Watchdog to Google Ads",
      async () => {
        ...
        const replayableLogs =
          orchestratorService.getReplayableLogs(correlationId);
        ...
        const workflowStartEvent = wsMessages.find(
          (m) => m.type === "workflow_start",
        );
        ...
        const workflowCompleteEvent = wsMessages.find(
          (m) => m.type === "workflow_complete",
        );
        ...
        const toolStartEvents = wsMessages.filter(
          (m) => m.type === "tool_start",
        );
        expect(toolStartEvents.length).toBe(2);

        const toolCompleteEvents = wsMessages.filter(
          (m) => m.type === "tool_complete",
        );
    ...
    it(
      "should handle errors in workflow steps and perform rollback",
      async () => {
        // Mock analytics client to throw an error
        analyticsClient.answerQuestion = jest
          .fn()
        ...
        const wsMessages = testWsClient.getMessages();
        const workflowErrorEvent = wsMessages.find(
          (m) => m.type === "workflow_step_error",
        );
        ...
        const replayableLogs =
          orchestratorService.getReplayableLogs(correlationId);
        const errorLog = replayableLogs.find(
          (log) => log.type === "tool_error",
        );
        expect(errorLog).toBeDefined();
        expect(errorLog.error.message).toContain(
          "Analytics service unavailable",
        );
  ...
  describe("Event Publishing and Consumption", () => {
    it(
      "should publish and consume events across services",
      async () => {
    ...
    it(
      "should handle task queue processing and completion events",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
  ...
  describe("WebSocket Event Streaming", () => {
    it(
      "should stream events to WebSocket clients",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
        ...
        const toolStartEvent = await testWsClient.waitForMessage(
          (m) =>
            m.type === "tool_start" && m.data.correlationId === correlationId,
          5000,
        );
        ...
        const toolCompleteEvent = await testWsClient.waitForMessage(
          (m) =>
            m.type === "tool_complete" &&
            m.data.correlationId === correlationId,
          10000,
        );
    ...
    it(
      "should stream workflow events to WebSocket clients",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
    ...
    it(
      "should handle rate limit exceeded events",
      async () => {
  ...
  describe("Replayable Logs", () => {
    it(
      "should capture replayable logs for tool execution",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
        ...
        const toolCompleteLog = logs.find(
          (log) => log.type === "tool_complete",
        );
    ...
    it(
      "should capture replayable logs for workflow execution",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
        ...
        const workflowStartLog = logs.find(
          (log) => log.type === "workflow_start",
        );
        ...
        const workflowCompleteLog = logs.find(
          (log) => log.type === "workflow_complete",
        );
  ...
  describe("Performance and Timing", () => {
    it(
      "should execute tools within reasonable time limits",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
        ...
        expect(toolCompleteLog.executionTime).toBeLessThanOrEqual(
          executionTime,
        );
      },
      TEST_TIMEOUT,
    ...
    it(
      "should execute workflows with parallel steps faster than sequential",
      async () => {
        ...
        // Get replayable logs
        const sequentialLogs = orchestratorService.getReplayableLogs(
          sequentialCorrelationId,
        );
        const sequentialCompleteLog = sequentialLogs.find(
          (log) => log.type === "workflow_complete",
        );

        const parallelLogs = orchestratorService.getReplayableLogs(
          parallelCorrelationId,
        );
        const parallelCompleteLog = parallelLogs.find(
          (log) => log.type === "workflow_complete",
        );
  ...
  describe("End-to-End Integration", () => {
    it(
      "should demonstrate full cross-service orchestration capability",
      async () => {
        // Create a correlation ID for tracking
        const correlationId = uuidv4();
        ...
        // Verify WebSocket events were sent
        const wsMessages = testWsClient
          .getMessages()
          .filter((m) => m.data && m.data.correlationId === correlationId);
        expect(wsMessages.length).toBeGreaterThan(0);
        ...
        expect(
          steps[1].result.taskId || steps[1].result.campaignId,
        ).toBeDefined();
      },
      TEST_TIMEOUT,
    );
