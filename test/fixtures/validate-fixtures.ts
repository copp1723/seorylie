...
const options = {
  reportFile: args
    .find((arg) => arg.startsWith("--report-file="))
    ?.split("=")[1],
...
async function validateFixtures() {
  try {
    ...
    if (!fs.existsSync(ADF_FIXTURES_DIR)) {
      ...
      console.log(
        colors.yellow(
          `Created missing ADF fixtures directory: ${ADF_FIXTURES_DIR}`,
        ),
      );
    }
    ...
    if (!fs.existsSync(MOCK_DATA_DIR)) {
      ...
      console.log(
        colors.yellow(`Created missing mock data directory: ${MOCK_DATA_DIR}`),
      );
    }
...
async function validateAdfFixtures() {
  ...
  try {
    ...
    console.log(
      colors.green(`✅ Validated ${xmlFiles.length} ADF XML fixtures`),
    );
  } catch (error) {
...
async function validateAdfXmlFile(filePath: string) {
  ...
  try {
    ...
    if (xmlValidationResult !== true) {
      const error = xmlValidationResult as {
        err: { code: string; msg: string; line: number };
      };
      throw new Error(
        `Invalid XML: ${error.err.msg} at line ${error.err.line}`,
      );
    }
    ...
    if (piiResults.detected) {
      console.log(
        colors.yellow(
          `⚠️ ${fileName}: Possible PII detected: ${piiResults.matches.join(", ")}`,
        ),
      );
      stats.piiDetections++;
      stats.warnings++;
    ...
    if (parseTime > PERFORMANCE_THRESHOLD_MS) {
      console.log(
        colors.yellow(
          `⚠️ ${fileName}: Slow parsing (${parseTime.toFixed(2)}ms > ${PERFORMANCE_THRESHOLD_MS}ms threshold)`,
        ),
      );
      stats.warnings++;
    }
    ...
    if (options.verbose) {
      console.log(
        colors.green(
          `✅ ${fileName}: Valid ADF XML (${parseTime.toFixed(2)}ms)`,
        ),
      );
    }
  } catch (error) {
 ...
 */
async function validateFixtureMetadata(
  metadataPath: string,
  ...
  try {
    ...
    if (!fs.existsSync(metadataPath)) {
      if (options.fix) {
        ...
        console.log(
          colors.yellow(
            `⚠️ Created missing metadata file: ${path.basename(metadataPath)}`,
          ),
        );
        stats.warnings++;
        stats.fixed++;
        ...
        console.log(
          colors.yellow(
            `⚠️ Missing metadata file: ${path.basename(metadataPath)}`,
          ),
        );
        stats.warnings++;
        return;
    ...
    if (!validationResult.success) {
      console.log(
        colors.yellow(
          `⚠️ Invalid metadata in ${path.basename(metadataPath)}: ${validationResult.error.message}`,
        ),
      );
      stats.warnings++;
      return;
    ...
    if (options.verbose) {
      console.log(
        colors.green(`✅ ${path.basename(metadataPath)}: Valid metadata`),
      );
    }
  } catch (error) {
    console.log(
      colors.yellow(
        `⚠️ Error validating metadata for ${originalFileName}: ${error.message}`,
      ),
    );
    stats.warnings++;
  }
 ...
 */
function checkForPII(content: string): {
  detected: boolean;
  matches: string[];
} {
  ...
  for (const pattern of PII_PATTERNS) {
    ...
    if (match) {
      matches.push(
        `${match[0].substring(0, 3)}***${match[0].substring(match[0].length - 3)}`,
      );
    }
  }
...
async function attemptToFixXml(filePath: string): Promise<boolean> {
  try {
    ...
    console.log(
      colors.red(
        `❌ Failed to fix ${path.basename(filePath)}: ${error.message}`,
      ),
    );
    return false;
  }
...
function validateCoverage() {
  ...
  console.log(
    `Total scenarios covered: ${stats.coverage.scenarios.size}/${requiredScenarios.length}`,
  );
  console.log(
    `Edge cases covered: ${stats.coverage.edgeCases.size}/${requiredEdgeCases.length}`,
  );

  if (stats.coverage.missingScenarios.length > 0) {
    console.log(
      colors.yellow(
        `⚠️ Missing scenarios: ${stats.coverage.missingScenarios.join(", ")}`,
      ),
    );
  ...
  const missingEdgeCases = requiredEdgeCases.filter(
    (edgeCase) => !stats.coverage.edgeCases.has(edgeCase),
  );
  if (missingEdgeCases.length > 0) {
    console.log(
      colors.yellow(`⚠️ Missing edge cases: ${missingEdgeCases.join(", ")}`),
    );
...
async function generateReport() {
  ...
  const reportFile =
    ...
    path.join(
      REPORT_DIR,
      `fixture-validation-${new Date().toISOString().replace(/:/g, "-")}.json`,
    );
    ...
    coverage: {
      scenarios: {
        ...
        percentage: Math.round(
          (stats.coverage.scenarios.size / requiredScenarios.length) * 100,
        ),
      },
      edgeCases: {
        covered: Array.from(stats.coverage.edgeCases),
        missing: requiredEdgeCases.filter(
          (edgeCase) => !stats.coverage.edgeCases.has(edgeCase),
        ),
        total: requiredEdgeCases.length,
        percentage: Math.round(
          (stats.coverage.edgeCases.size / requiredEdgeCases.length) * 100,
        ),
      },
    },
...
function generateRecommendations(): string[] {
  ...
  if (stats.coverage.missingScenarios.length > 0) {
    recommendations.push(
      `Add fixtures for missing scenarios: ${stats.coverage.missingScenarios.join(", ")}`,
    );
  }
  ...
  if (missingEdgeCases.length > 0) {
    recommendations.push(
      `Add fixtures for missing edge cases: ${missingEdgeCases.join(", ")}`,
    );
  }
  ...
  if (stats.performance.max > PERFORMANCE_THRESHOLD_MS) {
    recommendations.push(
      `Optimize slow fixtures (max parse time: ${stats.performance.max.toFixed(2)}ms)`,
    );
  }
  ...
  if (stats.piiDetections > 0) {
    recommendations.push(
      `Remove or anonymize PII from ${stats.piiDetections} fixtures`,
    );
  }
...
function printSummary() {
  ...
  console.log(
    `Invalid: ${stats.invalid > 0 ? colors.red(stats.invalid.toString()) : stats.invalid}`,
  );
  console.log(
    `Warnings: ${stats.warnings > 0 ? colors.yellow(stats.warnings.toString()) : stats.warnings}`,
  );
  console.log(
    `Fixed: ${stats.fixed > 0 ? colors.green(stats.fixed.toString()) : stats.fixed}`,
  );
  ...
  console.log(colors.cyan("\nCoverage Metrics"));
  console.log(
    `Scenarios: ${stats.coverage.scenarios.size}/${requiredScenarios.length} (${Math.round((stats.coverage.scenarios.size / requiredScenarios.length) * 100)}%)`,
  );
  console.log(
    `Edge cases: ${stats.coverage.edgeCases.size}/${requiredEdgeCases.length} (${Math.round((stats.coverage.edgeCases.size / requiredEdgeCases.length) * 100)}%)`,
  );
