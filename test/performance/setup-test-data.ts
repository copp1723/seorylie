...
import { dealerships, users, vehicles, conversations, messages } from '../../shared/schema';
...
class TestDataSetup {
  ...
  private async cleanTestData(): Promise<void> {
    ...
    try {
      ...
      await db.delete(conversations).where(eq(conversations.subject, 'Test Conversation'));
      ...
      await db.delete(dealerships).where(eq(dealerships.subdomain, 'test-dealership-1'));
      await db.delete(dealerships).where(eq(dealerships.subdomain, 'test-dealership-2'));
      await db.delete(dealerships).where(eq(dealerships.subdomain, 'test-dealership-3'));
  ...
  private async createTestDealerships(): Promise<any[]> {
    ...
    for (let i = 1; i <= this.config.dealerships; i++) {
      ...
      const [created] = await db.insert(dealerships).values(dealership).returning();
  ...
  private async createTestVehicles(testDealerships: any[]): Promise<void> {
    ...
    for (const dealership of testDealerships) {
      ...
      console.log(`✅ Created ${this.config.vehiclesPerDealership} vehicles for ${dealership.name}`);
    }
  }
  ...
  private async createTestConversations(testDealerships: any[]): Promise<void> {
    ...
    for (const dealership of testDealerships) {
      ...
      for (let i = 1; i <= this.config.conversationsPerDealership; i++) {
        ...
        for (let j = 1; j <= this.config.messagesPerConversation; j++) {
          const message = {
            ...
            sender_name: j % 2 === 1 ? conversation.customer_name : 'AI Assistant',
      ...
      console.log(`✅ Created ${this.config.conversationsPerDealership} conversations for ${dealership.name}`);
    }
  }
  ...
  async verifyTestData(): Promise<void> {
    ...
    try {
      const dealershipCount = await db.select().from(dealerships).where(eq(dealerships.name, 'Test Dealership 1'));
      const userCount = await db.select().from(users).where(eq(users.username, 'testuser1'));
      const vehicleCount = await db.select().from(vehicles).where(eq(vehicles.make, 'Toyota'));
