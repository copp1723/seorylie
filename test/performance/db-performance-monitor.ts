...
class DatabasePerformanceMonitor {
  ...
  private generateReport(): PerformanceReport {
    ...
    const averageQueryTime = validMetrics.length > 0 
      ? validMetrics.reduce((sum, m) => sum + m.duration, 0) / validMetrics.length 
      : 0;

    // Sort queries by duration
    const sortedByDuration = [...validMetrics].sort((a, b) => b.duration - a.duration);
  ...
  private getQueryType(query: string): string {
    ...
    if (lowerQuery.includes('vehicle') || lowerQuery.includes('inventory')) return 'INVENTORY_QUERY';
  ...
  private generateRecommendations(metrics: QueryMetrics[]): string[] {
    ...
    if (recommendations.length === 0) {
      recommendations.push('Database performance looks good! No major issues detected.');
    }
  ...
  private printSummary(report: PerformanceReport): void {
    ...
    console.log('================================');
    console.log(`Test Duration: ${((report.testEndTime.getTime() - report.testStartTime.getTime()) / 1000).toFixed(2)}s`);
    ...
    Object.entries(report.queryTypeBreakdown).forEach(([type, stats]) => {
      console.log(`${type}: ${stats.count} queries, avg ${stats.avgDuration.toFixed(2)}ms`);
    });
