...
const commonFields = {
  ...
  description: z.string().min(20).max(2000).describe('Detailed description of what is needed'),
  priority: z.enum(['low', 'medium', 'high']).default('medium').describe('Priority level for this request'),
  targetKeywords: z.array(z.string()).optional().describe('Target keywords to focus on'),
  deadline: z.string().optional().describe('Requested completion date (YYYY-MM-DD)'),
...
export const AssetSchema = z.object({
  type: z.enum(['image', 'document', 'video', 'link']).describe('Type of asset'),
  url: z.string().url().optional().describe('URL to the asset if already hosted'),
  ...
  s3Key: z.string().optional().describe('S3 key for assets stored in the neutral CDN'),
});
...
export const PageRequestSchema = z.object({
  ...
  template: z.enum(['landing', 'about', 'service', 'contact', 'custom']).default('custom')
  ...
  targetAudience: z.string().optional().describe('Target audience for this page'),
  callToAction: z.string().optional().describe('Primary call-to-action for the page'),
  assets: z.array(AssetSchema).optional().describe('Images, documents, or other assets for the page'),
  ...commonFields,
});
...
export const BlogRequestSchema = z.object({
  ...
  wordCount: z.number().min(300).max(5000).default(800)
    .describe('Target word count for the blog post'),
  audience: z.string().optional().describe('Target audience for this blog post'),
  focusKeyword: z.string().optional().describe('Primary keyword to focus on'),
  contentStructure: z.array(z.string()).optional()
    .describe('Suggested sections or structure for the blog post'),
  assets: z.array(AssetSchema).optional().describe('Images or other assets for the blog post'),
  ...commonFields,
});
...
export const GBPRequestSchema = z.object({
  type: z.literal('gbp').describe('Request type: Google Business Profile update'),
  updateType: z.enum([
    'post', 
    'hours', 
    'photos', 
  ...
  businessName: z.string().optional().describe('Business name if different from account name'),
  locationId: z.string().optional().describe('Location ID if multiple locations exist'),
  content: z.string().min(20).max(1500).describe('Content for the update'),
  assets: z.array(AssetSchema).optional().describe('Images or other assets for the GBP update'),
  responseToReview: z.object({
    reviewId: z.string(),
    rating: z.number().min(1).max(5),
    reviewContent: z.string(),
    responseContent: z.string().min(10).max(500)
  }).optional().describe('Details for responding to a review'),
  ...commonFields,
});
...
export const MaintenanceRequestSchema = z.object({
  ...
  urls: z.array(z.string().url()).min(1).describe('URLs affected by this maintenance task'),
  issue: z.string().min(20).describe('Description of the issue that needs maintenance'),
  currentState: z.string().optional().describe('Current state of the affected pages/elements'),
  desiredState: z.string().optional().describe('Desired state after maintenance'),
  assets: z.array(AssetSchema).optional().describe('Screenshots or other assets related to the issue'),
  ...commonFields,
});
...
export const VendorResponseSchema = z.object({
  ...
  status: z.enum(['received', 'in_progress', 'completed', 'rejected', 'needs_info'])
    .describe('Status of the request'),
  message: z.string().optional().describe('Message from the vendor'),
  completedUrl: z.string().url().optional().describe('URL of the completed work'),
  completedAssets: z.array(AssetSchema).optional().describe('Assets produced by the vendor'),
  reportUrl: z.string().url().optional().describe('URL to the detailed report'),
  reportS3Key: z.string().optional().describe('S3 key for the transformed report'),
  completedDate: z.string().optional().describe('Date when the request was completed'),
  metrics: z.object({
...
export const InstallProfileSchema = z.object({
  ...
  additionalLocations: z.array(z.object({
    ...
    zipCode: z.string(),
    country: z.string().default('USA'),
  })).optional().describe('Additional business locations'),
  ...
  })).optional().describe('Business hours'),
  googleBusinessProfile: z.object({
    profileId: z.string().optional(),
    profileUrl: z.string().url().optional(),
    hasAccess: z.boolean().default(false),
  }).optional().describe('Google Business Profile information'),
  ...
  created_at: z.date().default(() => new Date()).describe('Creation timestamp'),
  updated_at: z.date().default(() => new Date()).describe('Last update timestamp'),
});
...
export const GA4ReportSchema = z.object({
  ...
  topPages: z.array(z.object({
    ...
    entrances: z.number(),
    bounceRate: z.number(),
  })).max(10).describe('Top performing pages'),
  topKeywords: z.array(z.object({
    ...
    ctr: z.number(),
    position: z.number(),
  })).max(10).optional().describe('Top performing keywords'),
  ...
  sandbox_id: z.string().uuid().describe('Sandbox ID for tenant isolation'),
  created_at: z.date().default(() => new Date()).describe('Report generation timestamp'),
});
