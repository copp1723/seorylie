...
export class SEOAgent {
  ...
  async processSEORequest(userMessage: string): Promise<{
    ...
    try {
      ...
      if (functionCall && functionCall.name && functionCall.arguments) {
        ...
        if (validatedRequest) {
          ...
          return {
            ...
            response: assistantMessage || `Your ${requestType} request has been submitted successfully. We'll start working on it right away!`,
            success: true,
          };
        }
      }
      ...
      return {
        seoRequest: null,
        response: assistantMessage || "I'm having trouble understanding your request. Could you provide more details about what SEO service you need?",
        success: false,
      };
    } catch (error) {
      logger.error({
      ...
      return {
        seoRequest: null,
        response: "I apologize, but I encountered an error processing your request. Please try again or provide more specific details about your SEO needs.",
        success: false,
      };
    }
  }
   ...
   * @returns Job ID
   */
  private async queueSEOTask(seoRequest: SeoRequest & { id: string, sandbox_id: string }): Promise<string> {
  ...
  async processInstallForm(userMessage: string = ''): Promise<{
    ...
    try {
      ...
      // If function was called, parse the arguments
      if (functionCall && functionCall.name === 'collectInstallProfile' && functionCall.arguments) {
        const args = JSON.parse(functionCall.arguments);
        ...
        if (profileResult.success) {
          ...
          return {
            installProfile,
            response: assistantMessage || "Thank you for completing the SEO onboarding form! We'll start setting up your SEO services right away.",
            success: true,
            completed: true,
          };
        } else {
          ...
          return {
            installProfile: null,
            response: assistantMessage || "Let's continue setting up your SEO services. I need a few more details to complete your profile.",
            success: true,
            completed: false,
          };
        }
      ...
      return {
        installProfile: null,
        response: assistantMessage || "Let's get started with setting up your SEO services. I'll guide you through the process step by step.",
        success: true,
        completed: false,
      };
    } catch (error) {
      ...
      return {
        installProfile: null,
        response: "I apologize, but I encountered an error processing your onboarding information. Let's try again with the basic details.",
        success: false,
        completed: false,
      };
    }
   ...
   * @returns Job ID
   */
  private async queueInstallProfile(installProfile: InstallProfile): Promise<string> {
  ...
  async processGA4Report(reportData: any): Promise<{
    ...
    try {
      ...
      return {
        response: assistantMessage || "I've analyzed your GA4 data and found some interesting insights. Would you like me to focus on any specific aspect of your website performance?",
        success: true,
      };
    } catch (error) {
      logger.error({
      ...
      return {
        response: "I apologize, but I encountered an error analyzing your GA4 data. Please try again later or provide a different date range.",
        success: false,
      };
    }
  }
  ...
  async processPublishNotification(publishData: any): Promise<{
    ...
    try {
      ...
      return {
        response: assistantMessage || `Great news! Your ${publishData.publish_type} has been published and is now live at ${publishData.published_url}.`,
        success: true,
      };
    } catch (error) {
      logger.error({
      ...
      return {
        response: "I'm pleased to inform you that your content has been published successfully!",
        success: false,
      };
    }
  }
