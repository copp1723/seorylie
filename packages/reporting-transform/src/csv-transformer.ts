...
export class CSVTransformer {
  ...
  constructor(options: CSVTransformOptions) {
    ...
    const defaultOptions = {
      ...
      vendorEmails: ['support@customerscout.com', 'reports@cs-seo.com', 'analytics@cs-analytics.com'],
  ...
  public async transform(): Promise<CSVTransformResult> {
    ...
    try {
      ...
      // Process the CSV
      const { transformedCsvBuffer, stats } = await this.processCsv(inputCsvBuffer, result);
      ...
      logger.error({ error, transformId: this.transformId }, 'Error transforming CSV');
  ...
  private async processCsv(
  ...
  ): Promise<{ transformedCsvBuffer: Buffer; stats: { rowCount: number; columnCount: number } }> {
    // Create a temporary file path for processing
    const tempInputPath = path.join(this.tempDir, `${this.transformId}_input.csv`);
    const tempOutputPath = path.join(this.tempDir, `${this.transformId}_output.csv`);
    ...
    if (this.options.columnMappings) {
      headers = headers.map(header => 
        this.options.columnMappings?.[header] || header
      );
    }
   ...
   */
  private sanitizeHeaders(headers: string[], result: CSVTransformResult): string[] {
    const sanitizedHeaders = headers.map(header => {
      ...
      this.options.vendorNames.forEach(vendorName => {
        if (header.includes(vendorName)) {
          ...
          result.detectedVendorReferences.push(`Header contains vendor name: ${vendorName}`);
          replacedHeader = true;
        }
      });
      ...
      this.options.vendorDomains.forEach(vendorDomain => {
        if (header.includes(vendorDomain)) {
          ...
          result.detectedVendorReferences.push(`Header contains vendor domain: ${vendorDomain}`);
          replacedHeader = true;
        }
      });
   ...
   */
  private sanitizeRowData(row: Record<string, string>, result: CSVTransformResult): Record<string, string> {
    ...
    Object.entries(row).forEach(([key, value]) => {
      ...
      this.options.vendorNames.forEach(vendorName => {
        if (value.includes(vendorName)) {
          ...
          result.detectedVendorReferences.push(`Data contains vendor name: ${vendorName}`);
          replacedValue = true;
        }
      });
      ...
      this.options.vendorDomains.forEach(vendorDomain => {
        if (value.includes(vendorDomain)) {
          ...
          result.detectedVendorReferences.push(`Data contains vendor domain: ${vendorDomain}`);
          replacedValue = true;
        }
      });
      ...
      this.options.vendorEmails.forEach(vendorEmail => {
        if (value.includes(vendorEmail)) {
          ...
          result.detectedVendorReferences.push(`Data contains vendor email: ${vendorEmail}`);
          replacedValue = true;
        }
      });
      ...
      if (this.options.vendorPhones && this.options.whiteLabelPhone) {
        this.options.vendorPhones.forEach(vendorPhone => {
          if (value.includes(vendorPhone)) {
            ...
            result.detectedVendorReferences.push(`Data contains vendor phone: ${vendorPhone}`);
            replacedValue = true;
          }
        });
      }
  ...
  private async getInputCsvBuffer(): Promise<Buffer> {
    ...
    if (this.options.inputCsvS3Key) {
      ...
      if (!s3Response.Body) {
        throw new Error(`Failed to get CSV from S3: ${this.options.inputCsvS3Key}`);
      }
      
      return s3Response.Body as Buffer;
    }
    
    throw new Error('No input CSV provided. Please provide inputCsvBuffer, inputCsvPath, or inputCsvS3Key.');
  }
   ...
   * @param result Result object to update with transformation details
   */
  private async handleOutput(transformedCsvBuffer: Buffer, result: CSVTransformResult): Promise<void> {
    ...
    if (this.options.outputCsvS3Key) {
      const s3Bucket = this.options.outputCsvS3Bucket || process.env.S3_BUCKET_NAME || 'rylie-seo-reports';
  ...
  private async cleanup(): Promise<void> {
    ...
    try {
      ...
      logger.warn({ error, transformId: this.transformId }, 'Error cleaning up temporary files');
    }
  }
}
 ...
 */
export function createCSVTransformer(options: CSVTransformOptions): CSVTransformer {
