...
export class PDFTransformer {
  ...
  public async transform(): Promise<PDFTransformResult> {
    ...
    try {
      ...
      logger.error({ error, transformId: this.transformId }, 'Error transforming PDF');
   ...
   */
  private async processPdf(pdfDoc: PDFDocument, result: PDFTransformResult): Promise<PDFDocument> {
    ...
    const checkAndReplace = (value: string | undefined): string | undefined => {
      ...
      this.options.vendorNames.forEach(vendorName => {
        if (value.includes(vendorName)) {
          ...
          newValue = newValue.replace(new RegExp(vendorName, 'gi'), this.options.whiteLabelName);
          result.detectedVendorReferences.push(`Metadata contains vendor name: ${vendorName}`);
      ...
      this.options.vendorDomains.forEach(vendorDomain => {
        if (value.includes(vendorDomain)) {
          containsVendorRef = true;
          newValue = newValue.replace(new RegExp(vendorDomain, 'gi'), 'rylie-seo.com');
          result.detectedVendorReferences.push(`Metadata contains vendor domain: ${vendorDomain}`);
   ...
   * @param result Result object to update with transformation details
   */
  private async replaceText(pdfDoc: PDFDocument, result: PDFTransformResult): Promise<void> {
    ...
    logger.info('Text replacement in PDFs requires custom content extraction and recreation');
    logger.info('This would be implemented with pdfjs-dist in a production environment');
    ...
    this.options.vendorNames.forEach(vendorName => {
      result.detectedVendorReferences.push(`Assumed text contains vendor name: ${vendorName}`);
    });
  }
   ...
   * @param result Result object to update with transformation details
   */
  private async replaceImages(pdfDoc: PDFDocument, result: PDFTransformResult): Promise<void> {
    ...
    for (const page of pages) {
      ...
      const logoHeight = (logoWidth / whiteLabelLogoImage.width) * whiteLabelLogoImage.height;
  ...
  private async addFooter(pdfDoc: PDFDocument): Promise<void> {
    ...
    // Get footer text
    const footerText = this.options.whiteLabelFooterText || 
  ...
  private async addHeader(pdfDoc: PDFDocument): Promise<void> {
    ...
    // Get header text
    const headerText = this.options.whiteLabelHeaderText || this.options.whiteLabelName;
  ...
  private async addCoverPage(pdfDoc: PDFDocument): Promise<void> {
    ...
    // Draw footer
    firstPage.drawText(`© ${new Date().getFullYear()} ${this.options.whiteLabelName} | www.rylie-seo.com`, {
      x: 50,
      y: 50,
      size: 10,
      font: subtitleFont,
  ...
  private async getInputPdfBuffer(): Promise<Buffer> {
    ...
    if (this.options.inputPdfS3Key) {
      ...
      if (!s3Response.Body) {
        throw new Error(`Failed to get PDF from S3: ${this.options.inputPdfS3Key}`);
      }
      
      return s3Response.Body as Buffer;
    }
    
    throw new Error('No input PDF provided. Please provide inputPdfBuffer, inputPdfPath, or inputPdfS3Key.');
  }
  ...
  private async getWhiteLabelLogo(): Promise<Buffer> {
    ...
    if (this.options.whiteLabelLogoS3Key && this.options.whiteLabelLogoS3Bucket) {
      const s3Response = await this.s3.getObject({
      ...
      if (!s3Response.Body) {
        throw new Error(`Failed to get logo from S3: ${this.options.whiteLabelLogoS3Key}`);
      }
   ...
   * @param result Result object to update with transformation details
   */
  private async handleOutput(transformedPdfBuffer: Buffer, result: PDFTransformResult): Promise<void> {
    ...
    if (this.options.outputPdfS3Key) {
      const s3Bucket = this.options.outputPdfS3Bucket || process.env.S3_BUCKET_NAME || 'rylie-seo-reports';
 ...
 */
export function createPDFTransformer(options: PDFTransformOptions): PDFTransformer {
