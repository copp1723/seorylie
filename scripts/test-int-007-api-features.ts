...
const ADMIN_AUTH_TOKEN = process.env.ADMIN_AUTH_TOKEN || 'your-admin-auth-token'; // Replace with actual token
const NUM_CONCURRENT_WS_CONNECTIONS = parseInt(process.env.NUM_CONCURRENT_WS_CONNECTIONS || '300', 10);
const MESSAGE_LAG_THRESHOLD_MS = parseInt(process.env.MESSAGE_LAG_THRESHOLD_MS || '1000', 10);
...
const log = (level: 'info' | 'warn' | 'error' | 'debug', message: string, data?: any) => {
  console[level](`[${new Date().toISOString()}] [${level.toUpperCase()}] ${currentTestSuiteName ? `[${currentTestSuiteName}] ` : ''}${message}`, data || '');
};
...
const runTest = async (name: string, testFn: () => Promise<void>): Promise<void> => {
  ...
  try {
    ...
    testResults.push({ name, passed: false, details: errorDetails, duration, error });
    log('error', `FAILED: ${name} (Duration: ${duration.toFixed(2)}ms) - ${errorDetails}`);
  }
};
...
const createWebSocketClient = (clientId: string, traceId: string): Promise<WebSocket> => {
...
const waitForMessage = (ws: WebSocket, timeoutMs: number = 5000, filterFn?: (data: any) => boolean): Promise<any> => {
  return new Promise((resolve, reject) => {
...
const simulateCreateSandbox = async (name: string = `test-sandbox-${uuidv4()}`): Promise<{ id: string, name: string, userId: number, dealershipId: number, state: string }> => {
  ...
  await apiClient.post(`/sandbox/debug/create`, sandboxData).catch(e => {
    ...
    log('warn', `Debug sandbox creation endpoint failed or not available, proceeding with mock: ${e.message}`);
  });
...
const setFeatureFlag = async (flagName: string, enabled: boolean, rolloutPercentage: number = 100): Promise<void> => {
  try {
    ...
    log('info', `Feature flag ${flagName} set to: enabled=${enabled}, rollout=${rolloutPercentage}%`);
  } catch (error: any) {
    log('warn', `Failed to set feature flag ${flagName} via API. Assuming manual setup or test limitations. Error: ${error.message}`);
...
async function runSandboxApiTests() {
  ...
  await runTest('List sandboxes and find the new sandbox', async () => {
    ...
    if (response.status !== 200) throw new Error(`Failed to list sandboxes: ${response.status}`);
    const sandboxes = response.data.data;
    if (!Array.isArray(sandboxes)) throw new Error('Sandbox list is not an array');
    const found = sandboxes.some((sb: any) => sb.id === sandboxId);
    if (!found) throw new Error(`Created sandbox ${sandboxId} not found in list`);
  });

  await runTest('Get status of the new sandbox', async () => {
    const response = await apiClient.get(`/sandbox/${sandboxId}/status`);
    if (response.status !== 200) throw new Error(`Failed to get sandbox status: ${response.status}`);
    if (response.data.data.id !== sandboxId) throw new Error('Incorrect sandbox status returned');
    if (response.data.data.state !== 'active') throw new Error(`Sandbox initial state is not active: ${response.data.data.state}`);
  });

  await runTest('Pause the sandbox', async () => {
    const response = await apiClient.post(`/sandbox/pause/${sandboxId}`, { reason: 'Automated test pause' });
    if (response.status !== 200) throw new Error(`Failed to pause sandbox: ${response.status}`);
    if (response.data.data.state !== 'paused') throw new Error('Sandbox state not updated to paused');
  });

  await runTest('Attempt tool execution on paused sandbox (expect 423)', async () => {
    try {
      await apiClient.post(`/sandbox/${sandboxId}/tools/execute`, { toolId: 'test-tool' });
      throw new Error('Tool execution did not fail on paused sandbox');
    } catch (error: any) {
      if (!error.isAxiosError || error.response?.status !== 423) {
        throw new Error(`Expected 423 status, got ${error.response?.status || error.message}`);
      }
      if (error.response.data.error.code !== 'RESOURCE_LOCKED') {
        throw new Error(`Expected error code RESOURCE_LOCKED, got ${error.response.data.error.code}`);
      }
      log('info', 'Correctly received 423 Locked for tool execution on paused sandbox.');
    }
  });

  await runTest('Resume the sandbox', async () => {
    const response = await apiClient.post(`/sandbox/resume/${sandboxId}`);
    if (response.status !== 200) throw new Error(`Failed to resume sandbox: ${response.status}`);
    if (response.data.data.state !== 'active') throw new Error('Sandbox state not updated to active');
  });

  await runTest('Attempt tool execution on active sandbox (expect success)', async () => {
    const response = await apiClient.post(`/sandbox/${sandboxId}/tools/execute`, { toolId: 'test-tool' });
    if (response.status !== 200) throw new Error(`Tool execution failed on active sandbox: ${response.status}`);
    if (!response.data.data.executionId) throw new Error('Tool execution response missing executionId');
  });

  await runTest('Check pause history', async () => {
    const response = await apiClient.get(`/sandbox/${sandboxId}/pause-history`);
    if (response.status !== 200) throw new Error(`Failed to get pause history: ${response.status}`);
    const history = response.data.data.history;
    if (!Array.isArray(history) || history.length < 2) throw new Error('Pause history incomplete');
    const pauseEntry = history.find((e: any) => e.action === 'pause');
    const resumeEntry = history.find((e: any) => e.action === 'resume');
    if (!pauseEntry || !resumeEntry) throw new Error('Pause or resume entry missing in history');
    if (pauseEntry.reason !== 'Automated test pause') throw new Error('Pause reason incorrect in history');
  });
  ...
  await runTest('Configure and verify auto-pause settings', async () => {
    ...
    const response = await apiClient.put(`/sandbox/${sandboxId}/pause-settings`, settings);
    if (response.status !== 200) throw new Error(`Failed to update auto-pause settings: ${response.status}`);
  ...
  await runTest('Bulk pause sandboxes', async () => {
    ...
    const response = await apiClient.post('/sandbox/bulk/pause', { sandboxIds: idsToPause, reason: 'Bulk test pause' });
    if (response.status !== 200) throw new Error(`Bulk pause failed: ${response.status}`);
    const results = response.data.data;
    idsToPause.forEach(id => {
      if (!results[id] || !results[id].success) throw new Error(`Failed to pause sandbox ${id} in bulk`);
    });
    const status1 = (await apiClient.get(`/sandbox/${sandboxId}/status`)).data.data;
    const status2 = (await apiClient.get(`/sandbox/${sandboxId2}/status`)).data.data;
    if (status1.state !== 'paused' || status2.state !== 'paused') throw new Error('Sandboxes not paused after bulk operation');
  });

  await runTest('Bulk resume sandboxes', async () => {
    const idsToResume = [sandboxId, createdSandboxIds[createdSandboxIds.length -1]]; // Use last created one
    const response = await apiClient.post('/sandbox/bulk/resume', { sandboxIds: idsToResume });
    if (response.status !== 200) throw new Error(`Bulk resume failed: ${response.status}`);
    const results = response.data.data;
    idsToResume.forEach(id => {
      if (!results[id] || !results[id].success) throw new Error(`Failed to resume sandbox ${id} in bulk`);
    });
    const status1 = (await apiClient.get(`/sandbox/${sandboxId}/status`)).data.data;
    const status2 = (await apiClient.get(`/sandbox/${idsToResume[1]}/status`)).data.data;
    if (status1.state !== 'active' || status2.state !== 'active') throw new Error('Sandboxes not active after bulk operation');
  });
...
async function runWebSocketScalingTests() {
  ...
  await runTest(`Establish ${NUM_CONCURRENT_WS_CONNECTIONS} concurrent WebSocket connections`, async () => {
    ...
    for (let i = 0; i < NUM_CONCURRENT_WS_CONNECTIONS; i++) {
      ...
      connectionPromises.push(limit(() => createWebSocketClient(clientId, `trace-${clientId}`)));
    }
    ...
    if (clients.length !== NUM_CONCURRENT_WS_CONNECTIONS) {
      throw new Error(`Failed to establish all ${NUM_CONCURRENT_WS_CONNECTIONS} connections. Established: ${clients.length}`);
    }
    log('info', `Successfully established ${clients.length} WebSocket connections.`);
  });

  await runTest('Verify initial CONNECT message for all clients', async () => {
    const connectMessagePromises = clients.map(wsClient =>
      waitForMessage(wsClient, 5000, data => data.type === 'connect' && data.metadata?.clientId)
    );
    const connectMessages = await Promise.all(connectMessagePromises);
    if (connectMessages.length !== clients.length) {
        throw new Error(`Did not receive CONNECT messages for all clients. Received: ${connectMessages.length}`);
    }
    log('info', 'All clients received CONNECT message.');
  });

  await runTest('Test message delivery lag (<1s) among concurrent connections', async () => {
    if (clients.length < 2) throw new Error('Not enough clients for lag test');
    ...
    const receivePromises = receivers.map(receiver =>
      waitForMessage(receiver, MESSAGE_LAG_THRESHOLD_MS + 1000, data =>
        data.type === 'chat_message' && data.message === testMessageId && data.dealershipId === dealershipIdForTest
      )
    );
    ...
    if (receivedMessages.length !== receivers.length) {
      throw new Error(`Not all receivers got the message. Expected: ${receivers.length}, Got: ${receivedMessages.length}`);
    }
    if (lag > MESSAGE_LAG_THRESHOLD_MS) {
      throw new Error(`Message lag exceeded threshold. Lag: ${lag.toFixed(2)}ms, Threshold: ${MESSAGE_LAG_THRESHOLD_MS}ms`);
    }
    log('info', `Message delivery lag test passed. Lag: ${lag.toFixed(2)}ms`);
  });

  await runTest('Test Redis Pub/Sub for broadcast functionality (simulated instances)', async () => {
    if (clients.length < 4) throw new Error('Not enough clients for Pub/Sub test (need at least 4)');
    ...
    const promises = [
      waitForMessage(receiverInstanceA, 5000, data => data.type === 'chat_message' && data.message === broadcastMessageId && data.dealershipId === broadcastDealershipId),
      waitForMessage(receiverInstanceB1, 5000, data => data.type === 'chat_message' && data.message === broadcastMessageId && data.dealershipId === broadcastDealershipId),
      waitForMessage(receiverInstanceB2, 5000, data => data.type === 'chat_message' && data.message === broadcastMessageId && data.dealershipId === broadcastDealershipId),
    ];
    ...
    if (results.length !== 3 || results.some(r => !r)) {
      throw new Error('Not all simulated instances/clients received the broadcast message via Redis Pub/Sub.');
    }
    log('info', 'Redis Pub/Sub broadcast test passed.');
  });

  await runTest('Test WebSocket health checking (ping/pong and stale connection closure)', async () => {
    if (clients.length === 0) throw new Error('No clients to test health checking.');
    ...
    sendMessage(testClient, { type: 'health_check', message: healthCheckMsgId, traceId: `trace-health-${uuidv4()}` });
    const response = await waitForMessage(testClient, 3000, data => data.type === 'health_check' && data.message === 'Health check response');
    if (!response) throw new Error('Client did not receive health check response.');

    log('info', `WebSocket health check (client side) passed for ${clientId}. Server-side stale closure is harder to verify here.`);
  ...
  await runTest('Test WebSocket Rate Limiting', async () => {
    if (clients.length === 0) throw new Error('No clients for rate limit test.');
    ...
    for (let i = 0; i < 70; i++) {
        sendMessage(rateLimitClient, { type: 'chat_message', message: `Rate limit test ${i}`, dealershipId: 1, traceId });
    ...
    try {
        ...
        const errMessage = await waitForMessage(rateLimitClient, 5000, data => data.type === 'error' && data.metadata?.reason === 'RATE_LIMIT_EXCEEDED');
        ...
        log('warn', `Did not receive explicit RATE_LIMIT_EXCEEDED error message, but rate limiting might still be active: ${e}`);
    }
    if (!errorReceived) {
        log('warn', 'Rate limit error message not explicitly received. Further validation might be needed.');
  ...
  await runTest('Test WebSocket Connection Migration (Simulated)', async () => {
    ...
    log('info', `Simulating server sending MIGRATION message to client ${clientId}.`);
    ...
    const closePromise = new Promise<void>(resolve => migrationClient.once('close', (code, reason) => {
        log('info', `Client ${clientId} closed with code ${code}, reason: ${reason}. Simulating reconnection attempt.`);
        resolve();
    }));
    ...
    const newConnection = await createWebSocketClient(clientId + "-reconnected", traceId + "-reconnected");
    if (!newConnection || newConnection.readyState !== WebSocket.OPEN) {
        throw new Error('Failed to re-establish WebSocket connection after simulated migration.');
    }
    log('info', 'Successfully re-established WebSocket connection after simulated migration.');
...
async function runMonitoringAndPerformanceTests() {
  ...
  await runTest('Verify monitoring metrics integration (Conceptual)', async () => {
    ...
    await apiClient.post(`/sandbox/pause/${sandbox.id}`, { reason: 'Monitoring test' });
    ...
    try {
        ...
        if (metricsResponse.status === 200 && metricsResponse.data) {
            log('info', `Database pool metrics retrieved: ${JSON.stringify(metricsResponse.data)}`);
            // Add assertions here based on expected metric values or changes
        } else {
            log('warn', 'Could not retrieve specific database pool metrics for validation.');
        }
    } catch (e: any) {
        log('warn', `Metrics endpoint for database pool not available or failed: ${e.message}`);
    }
    // Similarly for WebSocket metrics, if an endpoint exists.
    log('info', 'Conceptual monitoring metrics test: Actions performed. Manual or specific metrics endpoint check needed.');
  });
  ...
  await runTest('Performance benchmark: Sandbox API operations', async () => {
    ...
    for (let i = 0; i < operations; i++) {
      ...
      await apiClient.post(`/sandbox/pause/${sandboxId}`, { reason: `Perf test ${i}` });
    ...
    log('info', `Sandbox API avg operation duration: ${avgDuration.toFixed(2)}ms over ${operations * 3} operations.`);
    if (avgDuration > 200) { // Example threshold
      log('warn', `Sandbox API performance may be slow (avg > 200ms): ${avgDuration.toFixed(2)}ms`);
    }
  });

  await runTest('Performance benchmark: WebSocket message throughput (100 messages)', async () => {
    const wsClient = await createWebSocketClient(`perf-ws-${uuidv4()}`, `trace-perf-ws-${uuidv4()}`);
    const numMessages = 100;
    let messagesReceived = 0;

    wsClient.on('message', (data) => {
        const parsed = JSON.parse(data.toString());
        if(parsed.type === 'chat_message' && parsed.metadata?.source === 'perf-test-echo') {
            messagesReceived++;
        }
    });
    ...
    await new Promise<void>((resolve) => {
        const interval = setInterval(() => {
            if (messagesReceived >= numMessages || (performance.now() - startTime > 10000)) {
    ...
    log('info', `WebSocket: Sent ${numMessages} messages, received ${messagesReceived} echo replies in ${duration.toFixed(2)}ms.`);
    if (messagesReceived < numMessages * 0.9) { // Allow for some loss in a basic test
        throw new Error(`Significant message loss in WebSocket performance test. Sent: ${numMessages}, Received: ${messagesReceived}`);
    }
    const throughput = (messagesReceived / duration) * 1000; // messages per second
    log('info', `WebSocket message throughput: ~${throughput.toFixed(2)} msgs/sec.`);
  });
...
async function main() {
  ...
  testResults.forEach(result => {
    if (result.passed) {
      log('info', `✅ PASSED: ${result.name} (Duration: ${result.duration?.toFixed(2)}ms)`);
    } else {
      log('error', `❌ FAILED: ${result.name} (Duration: ${result.duration?.toFixed(2)}ms) - Details: ${result.details}`);
    }
  });
  ...
  if (createdSandboxIds.length > 0) {
    ...
    createdSandboxIds.forEach(id => log('debug', `Simulated deletion of sandbox: ${id}`));
  }
