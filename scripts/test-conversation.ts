  ...
  try {
    const response = await fetch(`${BASE_URL}/api/inbound`, {
      ...
      body: JSON.stringify({
        ...
        customerMessage:
          "Hi, I'm interested in the Toyota RAV4. Can you tell me more about it?",
        channel: "sms",
...
async function runTests() {
  ...
  if (!apiKey) {
    console.error(
      "Failed to retrieve an API key. Make sure the server is running and the database is seeded.",
    );
    return;
  }
