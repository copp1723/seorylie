...
class BuildOptimizationVerifier {
  ...
  private verifyPackageJson(): void {
    ...
    try {
      ...
      if (missingScripts.length === 0) {
        ...
        this.addResult("Package Scripts", "fail", "Missing required scripts", {
          missingScripts,
          presentScripts,
        });
      }
  ...
  private async testBuildScripts(): Promise<void> {
    ...
    try {
      ...
      this.addResult(
        "Clean",
        "warning",
        "Clean script failed (may be expected)",
      );
    }
    ...
    const buildScripts = [
      ...
      {
        name: "build:server",
        description: "Server build (includes ADF worker)",
      },
    ];
    ...
    for (const { name, description } of buildScripts) {
      try {
        ...
        this.addResult(description, "pass", `${name} completed successfully`, {
          duration: `${duration}ms`,
        });
      } catch (error) {
        this.addResult(description, "fail", `${name} failed`, {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }
  ...
  private verifyBuildOutputs(): void {
    ...
    for (const { path, description } of expectedFiles) {
      if (existsSync(path)) {
        ...
        this.addResult(description, "pass", `${path} exists`, {
          size: `${Math.round(stats.size / 1024)}KB`,
          modified: stats.mtime,
        });
      } else {
        this.addResult(description, "fail", `${path} not found`);
      }
    }
  }
  ...
  private verifyAdfWorker(): void {
    ...
    if (existsSync(workerPath)) {
      ...
      this.addResult("ADF Worker File", "fail", "ADF worker file not found");
    }
  }
  ...
  private testMaintenanceScripts(): void {
    ...
    for (const { name, description } of maintenanceScripts) {
      try {
        execSync(`npm run ${name}`, { stdio: "pipe" });
        this.addResult(description, "pass", `${name} executed successfully`);
      } catch (error) {
        // pkg:audit might fail if there are vulnerabilities, which is expected
        const status = name === "pkg:audit" ? "warning" : "fail";
        this.addResult(description, status, `${name} completed with issues`, {
          note:
            name === "pkg:audit"
              ? "Audit failures may indicate security vulnerabilities"
              : undefined,
        });
      }
    }
