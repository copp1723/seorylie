...
async function testConnectionPooling(): Promise<boolean> {
  try {
      ...
      .map(async (_, index) => {
        return executeQuery(async () => {
          const result = await db.execute(
            sql`SELECT ${index} as query_id, NOW() as timestamp`,
          );
          return result[0];
        });
      });
    ...
    logger.info(
      `Executed ${concurrentQueries} concurrent queries in ${endTime - startTime}ms`,
    );
  ...
  for (const [tableName, table] of Object.entries(ALL_TABLES)) {
    ...
    try {
      ...
      if (tableInfo.exists) {
        ...
        logger.info(
          `Table ${tableName}: ✅ EXISTS (${tableInfo.count} rows, ${tableInfo.indexes?.length || 0} indexes, ${tableInfo.foreignKeys?.length || 0} FK constraints)`,
        );
      } else {
