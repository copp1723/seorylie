...
import { AiCostControlService, initializeAiCostControlService } from '../server/services/ai-cost-control-service';
...
jest.mock('../server/services/enhanced-ai-service', () => {
  const originalModule = jest.requireActual('../server/services/enhanced-ai-service');
  
  // Mock the getCompletion method to return a fixed response
  const mockGetCompletion = jest.fn().mockImplementation(async (prompt, options) => {
...
const createTestLead = (id: number, vehicleMake: string, intent: string): Partial<AdfLead> => ({
  id,
  externalId: `test-${id}`,
...
async function ensureTestTemplates() {
  ...
  const templatesDir = path.join(process.cwd(), 'server', 'templates', 'ai-responses');
  ...
  if (missingTemplates) {
    console.log('ℹ️ Please create the missing templates before running this test');
...
async function testTemplateFirstGeneration(service: AiCostControlService) {
  ...
  const pricingResponse = await service.generateResponse(pricingLead, 'pricing_inquiry', testDealership);
  ...
  const testDriveResponse = await service.generateResponse(testDriveLead, 'test_drive_request', testDealership);
  ...
  const unknownResponse = await service.generateResponse(unknownLead, 'unknown_intent', testDealership);
...
async function testRedisCaching(service: AiCostControlService) {
  ...
  const startTime1 = performance.now();
  const response1 = await service.generateResponse(cacheLead, 'feature_inquiry', testDealership);
  const duration1 = performance.now() - startTime1;
  ...
  const startTime2 = performance.now();
  const response2 = await service.generateResponse(cacheLead, 'feature_inquiry', testDealership);
  const duration2 = performance.now() - startTime2;
  ...
  if (duration2 < duration1) {
    console.log(`✅ Cache is faster: ${duration1.toFixed(2)}ms vs ${duration2.toFixed(2)}ms`);
  } else {
    console.log(`⚠️ Cache is not faster: ${duration1.toFixed(2)}ms vs ${duration2.toFixed(2)}ms`);
  }
  ...
  const startTime3 = performance.now();
  const response3 = await service.generateResponse(duplicateLead, 'feature_inquiry', testDealership);
  const duration3 = performance.now() - startTime3;
  ...
  if (response3.model.includes('cache') && response3.totalTokens === 0) {
    ...
    throw new Error('❌ Duplicate lead should have hit cache with 0 OpenAI calls');
  }
...
async function testRateLimiting(service: AiCostControlService) {
  ...
  console.log(`ℹ️ Initial rate limit status: ${initialStatus.remaining}/${initialStatus.total} remaining`);
  ...
  console.log(`🔄 Making ${totalRequests} requests (limit: ${initialStatus.total})...`);
  ...
  const rateLimitedResponses = results.filter(r => r.model === 'rate_limited');
  
  if (rateLimitedResponses.length > 0) {
    console.log(`✅ Rate limiting correctly applied after ${results.length - rateLimitedResponses.length} requests`);
  } else {
    console.log(`⚠️ Rate limiting was not triggered after ${results.length} requests`);
  }
  
  // Get updated rate limit status
  const finalStatus = await service.getRateLimitStatus(TEST_DEALERSHIP_ID);
  console.log(`ℹ️ Final rate limit status: ${finalStatus.remaining}/${finalStatus.total} remaining`);
  
  if (finalStatus.remaining <= 0) {
    console.log('✅ Rate limit correctly depleted');
  } else {
    console.log(`⚠️ Rate limit not depleted: ${finalStatus.remaining} remaining`);
  }
...
async function testCostTracking() {
  ...
  const tokenMetric = metrics.find((m: any) => m.name === 'openai_tokens_total');
  ...
  const sourceMetric = metrics.find((m: any) => m.name === 'ai_responses_generated_total');
  ...
  const cacheMetric = metrics.find((m: any) => m.name === 'ai_cache_events_total');
  ...
  const rateLimitMetric = metrics.find((m: any) => m.name === 'ai_rate_limit_events_total');
...
async function testCacheInvalidation(service: AiCostControlService) {
  ...
  const response1 = await service.generateResponse(cacheLead, 'cache_invalidation_test', testDealership);
  ...
  console.log('\n🔄 Second request (should hit cache)...');
  const response2 = await service.generateResponse(cacheLead, 'cache_invalidation_test', testDealership);
  ...
  const invalidated = await service.invalidateCache(cacheLead, 'cache_invalidation_test');
  ...
  console.log('\n🔄 Third request after invalidation (should not hit cache)...');
  const response3 = await service.generateResponse(cacheLead, 'cache_invalidation_test', testDealership);
  
  console.log(`🔍 Response model: ${response3.model}`);
  console.log(`🔢 Tokens used: ${response3.totalTokens}`);
  
  if (response3.model.includes('cache')) {
    throw new Error('❌ Third request should not have hit cache after invalidation');
  } else {
