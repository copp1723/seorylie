...
async function testKunesSetup() {
  try {
    ...
    const kunesDealerships = await db.select().from(dealerships)
      .where(like(dealerships.name, '%Kunes%'));
    
    console.log(`✅ Found ${kunesDealerships.length} Kunes dealerships\n`);
    
    if (kunesDealerships.length === 0) {
      console.log('❌ No Kunes dealerships found. Run setup-kunes-dealerships.ts first.');
      return;
    }
    ...
    for (const dealership of kunesDealerships) {
      ...
      const validations = [
        ...
        { field: 'contactEmail', value: dealership.contactEmail, required: true },
        { field: 'contactPhone', value: dealership.contactPhone, required: true },
      ...
      for (const validation of validations) {
        if (validation.required && (!validation.value || validation.value.trim() === '')) {
      ...
      if (dealership.settings) {
        ...
        console.log(`   ✅ Primary Persona: ${settings.primaryPersona || 'Not set'}`);
      }
    ...
    const kunesPersonas = await db.select({
      name: personas.name,
      role: personas.role,
    ...
    kunesPersonas.forEach(persona => {
      console.log(`   👤 ${persona.name} - ${persona.role} at ${persona.dealershipName} (${persona.isActive ? 'Active' : 'Inactive'})`);
    });
    ...
    const duplicateEmails = Array.from(emailCounts.entries()).filter(([email, count]) => count > 1);
    ...
    console.log('============================');
    console.log(`Total Kunes dealerships: ${kunesDealerships.length}/${expectedCount}`);
    ...
    if (kunesDealerships.length === expectedCount && validationErrors === 0) {
      ...
      console.log('Name,Subdomain,Address,Phone,Website,ADF Email,Persona,Active');
      csvData.forEach(row => {
        console.log(`"${row.name}","${row.subdomain}",${row.address},"${row.phone}","${row.website}","${row.adfEmail}","${row.persona}","${row.isActive}"`);
      });
      
    } else {
      console.log('❌ Setup validation failed. Please review and fix the errors above.');
      if (kunesDealerships.length !== expectedCount) {
        console.log(`   Expected ${expectedCount} dealerships, found ${kunesDealerships.length}`);
      }
      process.exit(1);
    }
