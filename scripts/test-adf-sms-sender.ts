 ...
 */
const createMockWebhook = (status: string, messageSid: string = TEST_CONFIG.mockSid) => ({
...
async function runTest(
  ...
  try {
    ...
    logger.error(`❌ FAILED: ${name} (${duration}ms)`, { error: err.message, stack: err.stack });
    TEST_RESULTS.failed++;
    TEST_RESULTS.tests.push({
      name,
      result: 'failed',
...
function printTestResults(): void {
  ...
  console.log(`Passed: ${TEST_RESULTS.passed} (${Math.round(TEST_RESULTS.passed / TEST_RESULTS.total * 100)}%)`);
  ...
  if (TEST_RESULTS.failed > 0) {
    ...
    TEST_RESULTS.tests
      ...
      .forEach(test => {
        console.log(`  - ${test.name} (${test.duration}ms): ${test.error?.message}`);
      });
  }
...
async function runTests() {
  try {
    ...
    await runTest('2. Test lead data extraction', async () => {
      ...
      const phoneNumber = (adfSmsResponseSender as any).extractPhoneNumber(mockLead);
      
      if (phoneNumber !== TEST_CONFIG.phoneNumber) {
        throw new Error(`Phone number extraction failed. Expected: ${TEST_CONFIG.phoneNumber}, Got: ${phoneNumber}`);
      }
      ...
      const phoneNumber2 = (adfSmsResponseSender as any).extractPhoneNumber(mockLead2);
      
      if (phoneNumber2 !== TEST_CONFIG.phoneNumber) {
        throw new Error(`Phone number extraction failed for string phone. Expected: ${TEST_CONFIG.phoneNumber}, Got: ${phoneNumber2}`);
      }
      ...
      const phoneNumber3 = (adfSmsResponseSender as any).extractPhoneNumber(mockLead3);
      
      if (phoneNumber3 !== TEST_CONFIG.phoneNumber) {
        throw new Error(`Phone number extraction failed for phones array. Expected: ${TEST_CONFIG.phoneNumber}, Got: ${phoneNumber3}`);
      }
      
      logger.info('Lead data extraction tests passed');
    });
    
    await runTest('3. Test message formatting with opt-out footer', async () => {
      // Create mock lead data
      const mockLead = {
        id: TEST_CONFIG.testLeadId,
        customer: {
      ...
      const response = 'Thank you for your interest in our dealership. We have received your inquiry and will get back to you shortly.';
      
      // Access the private method using any type assertion
      const formattedMessage = (adfSmsResponseSender as any).formatMessage(response, mockLead);
      ...
      const formattedMessage2 = (adfSmsResponseSender as any).formatMessage(response, mockLead2);
      
      if (!formattedMessage2.includes('John')) {
        throw new Error('Message does not include personalization for string name');
      }
      
      // Test with very long message (should be truncated)
      const longResponse = 'This is a very long message that should be truncated because SMS messages have a limit of 160 characters. We need to ensure that the message is properly truncated and the opt-out footer is still included. This message is definitely longer than 160 characters.';
      
      const formattedMessage3 = (adfSmsResponseSender as any).formatMessage(longResponse, mockLead);
      
      if (formattedMessage3.length > 160) {
        throw new Error(`Message is too long: ${formattedMessage3.length} characters`);
      }
    ...
    await runTest('4. Test SMS response recording in database', async () => {
      ...
      const message = 'Thank you for your interest in our dealership. We will contact you shortly.';
      ...
      if (smsResponse.status !== 'pending') {
        throw new Error(`Unexpected status: ${smsResponse.status}, expected: pending`);
      }
    ...
    await runTest('5. Test SID update and status tracking', async () => {
      ...
      if (smsResponse.message_sid !== TEST_CONFIG.mockSid) {
        throw new Error(`SID not updated. Expected: ${TEST_CONFIG.mockSid}, Got: ${smsResponse.message_sid}`);
      }
      
      if (smsResponse.status !== 'sent') {
        throw new Error(`Status not updated. Expected: sent, Got: ${smsResponse.status}`);
      }
    ...
    await runTest('6. Test webhook processing for delivery status updates', async () => {
      ...
      if (smsResponse.status !== 'delivered') {
        throw new Error(`Status not updated. Expected: delivered, Got: ${smsResponse.status}`);
      }
    ...
    await runTest('7. Test webhook processing for failed delivery', async () => {
      ...
      if (smsResponse.status !== 'failed') {
        throw new Error(`Status not updated. Expected: failed, Got: ${smsResponse.status}`);
      }
    ...
    await runTest('10. Test metrics tracking', async () => {
      ...
      const requiredMetrics = [
        ...
        'retryCount', 'optOutCount', 'avgDeliveryTimeMs'
      ];
    ...
    await runTest('11. Test complete SMS flow with lead.response.ready event', async () => {
      ...
      try {
        ...
        (adfSmsResponseSender as any).sendSms = async (
          ...
          if (leadId === newLeadId) {
            ...
            await (adfSmsResponseSender as any).recordSmsAttempt(
              leadId, dealershipId, phoneNumber, message
            );
            
            await (adfSmsResponseSender as any).updateSmsWithSid(
              leadId, 'SM' + crypto.randomUUID().replace(/-/g, '').substring(0, 32)
            );
        ...
        const sendSuccessPromise = waitForEvent(adfSmsResponseSender, 'sms.send.success', 5000);
        
        // Emit lead.response.ready event
        mockLeadResponseEvent(newLeadId, 'Thank you for your interest. We will contact you shortly.');
        ...
        if (!smsSent) {
          throw new Error('SMS not sent in response to lead.response.ready event');
        }
        ...
        if (smsResponse.status !== 'sent') {
          throw new Error(`Unexpected status: ${smsResponse.status}, expected: sent`);
        }
    ...
    await runTest('12. Test error handling for invalid phone numbers', async () => {
      // Create a new test lead for this test
      const newLeadId = await createTestLead();
      ...
      try {
        ...
        const sendFailedPromise = waitForEvent(adfSmsResponseSender, 'sms.send.failed', 5000);
    ...
    await runTest('13. Test delivery timeout handling', async () => {
      ...
      try {
        ...
        const originalSetDeliveryTimeout = (adfSmsResponseSender as any).setDeliveryTimeout;
        let timeoutSet = false;
        
        (adfSmsResponseSender as any).setDeliveryTimeout = (leadId: number, messageSid: string) => {
          if (leadId === newLeadId) {
            timeoutSet = true;
            ...
            setTimeout(async () => {
              try {
                ...
                // Update lead status
                await (adfSmsResponseSender as any).updateLeadSmsStatus(leadId, 'undelivered');
        ...
        (adfSmsResponseSender as any).sendSms = async (
          ...
          if (leadId === newLeadId) {
            ...
            const mockSid = 'SM' + crypto.randomUUID().replace(/-/g, '').substring(0, 32);
            await (adfSmsResponseSender as any).updateSmsWithSid(leadId, mockSid);
        ...
        (adfSmsResponseSender as any).setDeliveryTimeout = originalSetDeliveryTimeout;
        (adfSmsResponseSender as any).sendSms = originalSendSms;
        ...
        if (smsResponse.status !== 'undelivered') {
          throw new Error(`Unexpected status: ${smsResponse.status}, expected: undelivered`);
        }
    ...
    await runTest('14. Live SMS sending test (requires real phone number)', async () => {
      ...
      console.log('\n✅ SMS sent successfully to', process.env.TEST_PHONE_NUMBER);
