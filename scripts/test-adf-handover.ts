...
const TEST_CONFIG = {
  ...
  webhookUrl:
    process.env.WEBHOOK_URL || "http://localhost:3000/webhooks/sendgrid",
...
async function runTest() {
  ...
  try {
    ...
    // Success!
    logger.success(
      "ADF-08 Handover System Integration Test completed successfully!",
    );
  } catch (error) {
  ...
  if (emailProvider === "mailhog") {
    ...
    try {
      const response = await axios.get(
        `http://${TEST_CONFIG.mailhogHost}:${TEST_CONFIG.mailhogApiPort}/api/v2/messages`,
      );
      ...
      logger.warn(
        "MailHog API not accessible, email verification will be skipped",
      );
    }
...
async function createTestData() {
  ...
  const existingSettings = await db
    ...
    .where(
      eq(dealershipHandoverSettings.dealershipId, TEST_CONFIG.dealershipId),
    )
    .limit(1)
    .then((results) => results[0]);
  ...
  if (existingSettings) {
    ...
    logger.info(
      `Updated dealership handover settings (ID: ${existingSettings.id})`,
    );
  } else {
    // Create new settings
  ...
  const messageContents = [
    {
      sender: "customer",
      content: "Hi, I'm interested in a new SUV with good fuel economy.",
    },
    {
      sender: "ai",
      content:
        "Hello! I'd be happy to help you find an SUV with good fuel economy. Are you looking for a specific make or model?",
    },
    {
      sender: "customer",
      content:
        "I'm considering a Toyota RAV4 or Honda CR-V. Do you have any in stock?",
    },
    {
      sender: "ai",
      content:
        "Yes, we have several Toyota RAV4 and Honda CR-V models in stock. The RAV4 Hybrid gets excellent fuel economy at around 40 MPG combined. Would you like more information about pricing and availability?",
    },
    {
      sender: "customer",
      content:
        "Yes, I'd like to know the price range for the RAV4 Hybrid and if you offer test drives.",
    },
    {
      sender: "ai",
      content:
        "Our Toyota RAV4 Hybrid models range from $29,500 to $37,000 depending on trim level and options. And yes, we absolutely offer test drives! Would you like to schedule one?",
    },
    {
      sender: "customer",
      content:
        "That sounds good. I'd like to schedule a test drive for this weekend if possible.",
    },
  ];
...
async function testDossierGeneration() {
  ...
  try {
    ...
    throw new Error(
      `Dossier generation test failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
async function testHandoverOrchestration() {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const handoverInitiatedListener = (data: any) => {
      logger.info("Handover initiated event received", {
    ...
    const handoverEmailSentListener = (data: any) => {
      ...
      // Remove listeners
      handoverOrchestrator.removeListener(
        "handover:initiated",
        handoverInitiatedListener,
      );
      handoverOrchestrator.removeListener(
        "handover_email:sent",
        handoverEmailSentListener,
      );
      handoverOrchestrator.removeListener(
        "handover:failed",
        handoverFailedListener,
      );
      handoverOrchestrator.removeListener(
        "handover_email:failed",
        handoverEmailFailedListener,
      );
...
async function verifyEmailDelivery() {
  ...
  if (handover.status !== "email_sent") {
    throw new Error(
      `Unexpected handover status: ${handover.status} (expected: email_sent)`,
    );
  }
  ...
  if (TEST_CONFIG.useMailhog) {
    try {
      ...
      if (response.status !== 200) {
        logger.warn(
          "MailHog API returned non-200 status code, skipping email verification",
        );
        return;
      }
      ...
      if (!toAddress.includes(TEST_CONFIG.handoverEmail)) {
        throw new Error(
          `Email sent to wrong address: ${toAddress} (expected: ${TEST_CONFIG.handoverEmail})`,
        );
      }
      ...
      const hasBothParts =
        ...
        ourEmail.MIME.Parts.some((part: any) =>
      ...
      if (
        ...
        error.message.includes("Handover email not found")
      ) {
        throw error;
      }
...
async function testWebhook() {
  ...
  try {
    ...
    if (response.status !== 200) {
      throw new Error(
        `Webhook returned non-200 status code: ${response.status}`,
      );
    }
...
async function validateMetrics() {
  ...
  const hasDossierGenerationMetric = metrics.includes(
  ...
  if (
    hasHandoverTriggerMetric &&
    hasHandoverEmailMetric &&
    hasDossierGenerationMetric
  ) {
...
async function cleanup() {
  ...
  try {
    ...
    if (testIds.handoverId) {
      await db.delete(handovers).where(eq(handovers.id, testIds.handoverId));
    ...
    if (testIds.conversationId) {
      ...
      logger.info(
        `Deleted test messages for conversation (ID: ${testIds.conversationId})`,
      );
    }
    ...
    // Delete lead
    if (testIds.leadId) {
      await db.delete(leads).where(eq(leads.id, testIds.leadId));
    ...
    if (testIds.customerId) {
      await db.delete(customers).where(eq(customers.id, testIds.customerId));
