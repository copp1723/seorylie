...
} from "../server/services/email-listener";
...
class EmailTestSuite {
    ...
    try {
      ...
      if (status.isInitialized && status.isConnected) {
        this.addResult(
          "Service Status",
          true,
          "Email service is properly initialized and connected",
        );
      } else if (status.isInitialized && !status.isConnected) {
        this.addResult(
          "Service Status",
          true,
          "Email service initialized but using development mode",
          status,
        );
      } else {
        this.addResult(
          "Service Status",
          false,
          status.error || "Service not properly initialized",
          status,
        );
      }
    } catch (error) {
  ...
  private async testEmailTemplates(): Promise<void> {
    ...
    const templateTests = [
      ...
      },
      {
        ...
        test: () =>
          sendPasswordResetEmail(this.testEmail, "test-reset-token-123"),
      },
      {
      ...
      },
      {
        name: "Report Email",
        test: () =>
          sendReportEmail(this.testEmail, "report-123", "Weekly Performance"),
      },
      {
        name: "Handover Email",
        test: () =>
          sendHandoverEmail(this.testEmail, {
            customerName: "John Doe",
            dealershipName: "Test Dealership",
            conversationSummary:
              "Customer inquired about a 2024 Honda Civic. Interested in financing options.",
            nextSteps: [
              "Schedule test drive",
      ...
      },
      {
        ...
        test: () =>
          sendTemplatedEmail("welcome", this.testEmail, {
            userName: "Test User",
    ...
    for (const templateTest of templateTests) {
      try {
        ...
        if (result.success) {
          this.addResult(
            `Template - ${templateTest.name}`,
            true,
            `Template rendered and sent successfully`,
          );
        } else {
          this.addResult(
            `Template - ${templateTest.name}`,
            false,
            result.error || "Template failed",
          );
        }
      } catch (error) {
        this.addResult(
  ...
  private async testNotificationSystem(): Promise<void> {
    ...
    const notificationTests = [
      ...
      },
      {
        ...
        message:
          "This is an error notification that requires immediate attention.",
      },
    ];
  ...
  private async testBounceHandling(): Promise<void> {
    ...
    for (const bounceTest of bounceTests) {
      try {
        ...
        if (!result.success) {
          this.addResult(
            `Bounce Handling - ${bounceTest.name}`,
            true,
            `Correctly handled invalid email: ${result.error}`,
          );
        } else {
          this.addResult(
            `Bounce Handling - ${bounceTest.name}`,
        ...
        this.addResult(
          `Bounce Handling - ${bounceTest.name}`,
          true,
          `Correctly caught exception: ${error.message}`,
        );
      }
    }
  }
    ...
    try {
      ...
      if (emailResult.success) {
        this.addResult(
          "Inventory Import",
          true,
          "Email inventory import processed successfully",
          emailResult.results,
        );
      } else {
        this.addResult(
          "Inventory Import",
          false,
          emailResult.error || "Import failed",
        );
      }
      ...
      this.addResult(
        "Inventory Import",
        false,
        `Exception during inventory import test: ${error.message}`,
      );
    }
  }
  ...
  private generateSummary(): void {
    ...
    if (failed === 0) {
      ...
      console.log(
        "  🔍 Check email service logs for detailed error information.",
      );
    }
