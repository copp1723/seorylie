...
class MockEmailTestSuite {
  ...
  async runAllTests(): Promise<void> {
    console.log(
      "🧪 Starting Mock Email Service Tests (No External Dependencies)...\n",
    );
    ...
    const templateTests = [
      {
        ...
        test: () =>
          this.validateEmailTemplate("welcome", {
            userName: "Test User",
    ...
    const interfaceTests = [
      {
        ...
        test: () =>
          this.mockEmailFunction(() =>
            sendWelcomeEmail(this.testEmail, "Test User"),
          ),
      },
      {
        ...
        test: () =>
          this.mockEmailFunction(() =>
            sendPasswordResetEmail(this.testEmail, "test-token"),
          ),
      },
      {
        ...
        test: () =>
          this.mockEmailFunction(() =>
            sendNotificationEmail(this.testEmail, "Test", "Message"),
          ),
      },
      {
        ...
        test: () =>
          this.mockEmailFunction(() =>
            sendReportEmail(this.testEmail, "report-123", "Performance"),
          ),
      },
      {
  ...
  private async testBounceHandlingLogic(): Promise<void> {
    ...
    for (const test of bounceTests) {
      ...
      this.addResult(
        ...
        success,
        success
          ? "Bounce handling logic correct"
          : `Expected retry=${test.shouldRetry}, got retry=${shouldRetry}`,
      );
    }
  }
  ...
  private async validateEmailStructure(
    subject: string,
    content: string,
    keywords: string[],
  ) {
    ...
    return {
      ...
      message: hasKeywords
        ? "Email structure valid"
        : "Missing required keywords or content",
    };
  }
  ...
  private validateInventoryEmail(emailData: any) {
    const hasRequiredSubject = emailData.subject
      .toLowerCase()
      .includes("inventory");
    const hasTsvAttachment = emailData.attachments.some(
      (att) => att.filename.endsWith(".tsv") || att.filename.endsWith(".txt"),
    );

    return {
      isValid: hasRequiredSubject && hasTsvAttachment,
      message:
        hasRequiredSubject && hasTsvAttachment
