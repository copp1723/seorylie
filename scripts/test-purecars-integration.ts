...
async function testGetConversation(conversationId: number) {
  ...
  const response = await apiRequest(
    `/conversations/${conversationId}?dealerId=${config.dealerId}`,
  );
...
async function runInteractiveTest() {
  try {
    ...
    while (running) {
      ...
      console.log(
        "4. View conversation history (requires active conversation)",
      );
      ...
      switch (choice) {
        ...
        case "6":
          ...
          console.log(
            `Dealer ID changed to ${config.dealerId}. Any active conversation has been cleared.`,
          );
          break;
...
async function runAutomatedTest() {
  try {
    ...
    const initialMessage =
      "Hi, I'm interested in your SUV models. Do you have anything with good gas mileage?";
    ...
    const followUpMessage =
      "I'd prefer something that's not too big, maybe a compact SUV. What's your price range?";
    ...
    const handoverMessage =
      "Actually, I'd like to speak with a sales representative about financing options and test drive scheduling.";
    ...
    if (handoverResponse && handoverResponse.status === "escalated") {
      ...
      const manualHandoverResponse = await testHandover(
        conversationId,
        "Customer requested to speak with sales rep",
      );
...
// Main function
async function main() {
  const testType = await prompt(
    "Run interactive test or automated test? (i/a): ",
  );
