...
function analyzeSqlContent(sql: string): {
  ...
  const analysis = {
    tables: { created: [] as string[], altered: [] as string[], dropped: [] as string[] },
  ...
  const createTableMatches = normalizedSql.match(/CREATE TABLE\s+(?:IF NOT EXISTS\s+)?(\w+)/g);
  if (createTableMatches) {
    createTableMatches.forEach(match => {
      const tableName = match.replace(/CREATE TABLE\s+(?:IF NOT EXISTS\s+)?/, '');
  ...
  const dropTableMatches = normalizedSql.match(/DROP TABLE\s+(?:IF EXISTS\s+)?(\w+)/g);
  if (dropTableMatches) {
  ...
  const createIndexMatches = normalizedSql.match(/CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:IF NOT EXISTS\s+)?(\w+)/g);
  if (createIndexMatches) {
    createIndexMatches.forEach(match => {
      const indexName = match.replace(/CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:IF NOT EXISTS\s+)?/, '');
      analysis.indexes.created.push(indexName.toLowerCase());
    });
  }

  const dropIndexMatches = normalizedSql.match(/DROP INDEX\s+(?:IF EXISTS\s+)?(\w+)/g);
  if (dropIndexMatches) {
  ...
  const dropConstraintMatches = normalizedSql.match(/DROP CONSTRAINT\s+(?:IF EXISTS\s+)?(\w+)/g);
  if (dropConstraintMatches) {
    dropConstraintMatches.forEach(match => {
      const constraintName = match.replace(/DROP CONSTRAINT\s+(?:IF EXISTS\s+)?/, '');
...
function analyzeMigrations(migrations: MigrationFile[]): MigrationAnalysis {
  ...
  migrations.forEach(migration => {
    versionCounts[migration.version] = (versionCounts[migration.version] || 0) + 1;
    ...
    if (migration.isRollback) {
      ...
      analysis.migrationOrder.push(`${migration.version}_${migration.description}`);
    }
  ...
  Object.entries(versionCounts).forEach(([version, count]) => {
    if (count > 2) { // More than forward + rollback
  ...
  forwardMigrations.forEach(version => {
    const hasRollback = migrations.some(m => m.version === version && m.isRollback);
    if (!hasRollback) {
...
function generateMigrationReport(migrations: MigrationFile[], analysis: MigrationAnalysis): string {
...
${analysis.migrationOrder.map((migration, index) => 
  `${index + 1}. ${migration}`
).join('\n')}
...
${analysis.totalMigrations > 0 && analysis.missingRollbacks.length === 0 && analysis.duplicateVersions.length === 0 
...
async function main() {
  try {
    ...
    const hasIssues = analysis.missingRollbacks.length > 0 || analysis.duplicateVersions.length > 0;
