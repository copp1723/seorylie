...
async function createTestPersona(dealershipId: number) {
  ...
  const dealershipName =
    (await prompt(
      `Dealership name (default: ${defaultArguments.dealershipName}): `,
    )) || defaultArguments.dealershipName;
  const make =
    (await prompt(
      `Primary vehicle make (default: ${defaultArguments.make}): `,
    )) || defaultArguments.make;
  const inventoryHighlights =
    (await prompt(`Inventory highlights (default: summarized): `)) ||
    defaultArguments.inventoryHighlights;
  const currentPromotions =
    (await prompt(`Current promotions (default: summarized): `)) ||
    defaultArguments.currentPromotions;
...
async function listPersonas(dealershipId: number) {
  ...
  try {
    const dealershipPersonas = await db
      .select()
      .from(personas)
      .where(eq(personas.dealershipId, dealershipId));
    ...
    dealershipPersonas.forEach((persona, index) => {
      console.log(
        `${index + 1}. ${persona.name}${persona.isDefault ? " (Default)" : ""}`,
      );
...
async function viewPersona(personaId: number) {
  ...
  try {
    ...
    console.log(
      `Name: ${persona.name}${persona.isDefault ? " (Default)" : ""}`,
    );
  ...
  try {
    ...
    try {
      ...
      const feedback = await prompt(
        "Did the response meet your expectations? (y/n): ",
      );
...
async function updatePersona(personaId: number) {
  ...
  try {
    ...
    // Get updated values
    const name =
      (await prompt(`New name (current: ${persona.name}): `)) || persona.name;
    ...
    if (updateTemplateStr.toLowerCase() === "y") {
      ...
      promptTemplate =
        (await prompt("Enter new template (press Enter to keep current):\n")) ||
    ...
    if (updateArgsStr.toLowerCase() === "y") {
      ...
      for (const [key, value] of Object.entries(args)) {
        ...
        const newValue =
          (await prompt(
            `New value for ${key} (press Enter to keep current): `,
          )) || value;
        args[key] = newValue;
      }
    ...
    const isDefaultStr = await prompt(
      `Set as default persona? (current: ${persona.isDefault ? "Yes" : "No"}): `,
    );
    const isDefault =
      isDefaultStr.toLowerCase() === "y" ? true : persona.isDefault;
    ...
    while (running) {
      ...
      switch (choice) {
        ...
        case "3": {
          ...
          if (personas.length > 0) {
            const personaId = parseInt(
              await prompt("Enter persona ID to view: "),
            );
        ...
        case "4": {
          ...
          if (personas.length > 0) {
            const personaId = parseInt(
              await prompt("Enter persona ID to update: "),
            );
        ...
        case "5": {
          ...
          if (personas.length > 0) {
            const personaId = parseInt(
              await prompt("Enter persona ID to test: "),
            );
