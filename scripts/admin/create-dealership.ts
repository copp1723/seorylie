...
program
  ...
  .option(
    ...
    "Email domain (optional, defaults to domain.com)",
  )
  .option(
    "--tier <tier>",
    "Service tier (trial, standard, premium, enterprise)",
    "standard",
  )
...
async function main() {
  try {
    ...
    if (
      ...
      console.log(
        chalk.yellow(
          "   Please provide all required parameters or use --interactive mode.",
        ),
      );
      console.log();
      program.help();
    ...
    const setupResult = await setupDealership(
      dealershipDetails,
      emailConfig,
      adminUser,
      options.generateApiKey,
    );
    ...
    console.log(
      `  1. Share the setup documentation with the customer (${options.outputDir}/setup-guide.md)`,
    );
    console.log(
      `  2. Verify email processing is working: npm run test:email -- --dealership=${setupResult.dealershipId}`,
    );
    console.log(
      `  3. Send the admin login URL to the dealership admin: ${setupResult.adminLoginUrl}`,
    );
  } catch (error) {
    ...
    const answers = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          input.length >= 3 ? true : "Name must be at least 3 characters",
      },
      {
        type: "input",
        name: "legalName",
      ...
      },
      {
        ...
        default:
          options.domain ||
          (options.name
            ? options.name.toLowerCase().replace(/[^a-z0-9]/g, "-")
            : ""),
        validate: (input) =>
      ...
      },
      {
        ...
        default:
          options.emailDomain ||
          (options.domain ? `${options.domain}.com` : ""),
      },
      {
        type: "list",
        name: "tier",
      ...
      },
      {
        ...
        validate: (input) =>
          /^#[0-9A-Fa-f]{6}$/.test(input)
            ? true
            : "Please enter a valid hex color code (e.g., #336699)",
      },
      {
        type: "input",
        name: "phone",
      ...
      },
      {
        ...
        default:
          options.website ||
          (options.domain ? `https://www.${options.domain}.com` : ""),
      },
      {
        type: "input",
        name: "timezone",
    ...
    const answers = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          validateEmail(input) ? true : "Please enter a valid email address",
      },
      {
        type: "input",
        name: "displayName",
      ...
      },
      {
        ...
        validate: (input) =>
          input.length > 0 ? true : "IMAP hostname is required",
      },
      {
        type: "input",
        name: "imapPort",
      ...
      },
      {
        ...
        validate: (input) =>
          input.length > 0 ? true : "IMAP username is required",
      },
      {
        type: "password",
        name: "imapPass",
        ...
        mask: "*",
        validate: (input) =>
          input.length > 0 ? true : "IMAP password is required",
      },
      {
        type: "confirm",
        name: "imapUseSsl",
...
function displaySetupSummary(setupResult: SetupResult): void {
  ...
  console.log(
    chalk.blue(`🏢  DEALERSHIP SETUP SUMMARY: ${setupResult.dealership.name}`),
  );
  console.log(chalk.blue("================================================="));
  console.log();
  ...
  console.log(
    `  IMAP: ${setupResult.emailConfig.imapHost}:${setupResult.emailConfig.imapPort}`,
  );
  console.log(
    `  SMTP: ${setupResult.emailConfig.smtpHost || "Not configured"}`,
  );
  ...
  console.log(chalk.yellow("\nDocumentation:"));
  console.log(
    `  Setup Guide: ./dealership-docs/${setupResult.dealership.domain}/setup-guide.md`,
  );
  console.log(
    `  Technical Details: ./dealership-docs/${setupResult.dealership.domain}/technical-details.md`,
  );
  console.log(
    `  Sample ADF: ./dealership-docs/${setupResult.dealership.domain}/sample-adf.xml`,
  );
}
