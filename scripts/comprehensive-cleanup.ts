...
const findFiles = (pattern: string | RegExp, baseDir: string = ROOT_DIR): string[] => {
  try {
    if (typeof pattern === 'string') {
      ...
      return allFiles.filter(file => 
        !file.includes('/') && pattern.test(file)
      );
    }
  } catch (error) {
    return [];
...
const organizeTestScripts = async (): Promise<void> => {
  ...
  for (const file of testScripts) {
    ...
    if (await askForConfirmation(`Move test script: ${file} -> ${targetPath}?`)) {
...
const organizeJsonReports = async (): Promise<void> => {
  ...
  for (const file of jsonReports) {
    ...
    if (await askForConfirmation(`Move JSON report: ${file} -> ${targetPath}?`)) {
      moveFile(file, targetPath);
    }
  }
};
...
const organizeExampleFiles = async (): Promise<void> => {
  ...
  logger.info(`Found ${exampleFiles.length} example/demo files in root directory`);
  ...
  for (const file of exampleFiles) {
    ...
    if (await askForConfirmation(`Move example file: ${file} -> ${targetPath}?`)) {
      moveFile(file, targetPath);
    }
  }
};
...
const organizeUtilityScripts = async (): Promise<void> => {
  ...
  logger.info(`Found ${utilityScripts.length} utility scripts in root directory`);
  ...
  for (const file of utilityScripts) {
    ...
    if (await askForConfirmation(`Move utility script: ${file} -> ${targetPath}?`)) {
      moveFile(file, targetPath);
    }
  }
};
...
const organizeRemainingFiles = async (): Promise<void> => {
  ...
  const output = execSync(command).toString().trim();
  const allRootFiles = output ? output.split('\n').map(f => path.basename(f)) : [];
...
const analyzeRootDirectory = (): void => {
  ...
  const essentialSet = new Set(ROOT_ESSENTIAL_FILES);
  const missingEssential = ROOT_ESSENTIAL_FILES.filter(file => !allRootFiles.includes(file));
...
// Main function
const main = async () => {
  logger.info(`Starting comprehensive cleanup (dry-run: ${DRY_RUN}, no-confirm: ${NO_CONFIRM})`);
