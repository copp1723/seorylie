...
async function runTest(
  ...
  try {
    ...
    console.log(
      `${colors.green}✓ PASSED: ${name} (${duration}ms)${colors.reset}`,
    );
    results.passed++;
    results.tests.push({
      name,
    ...
    const duration = Date.now() - startTime;
    console.error(
      `${colors.red}✗ FAILED: ${name} (${duration}ms)${colors.reset}`,
    );
    console.error(
...
async function validateCIFramework(): Promise<void> {
  console.log(
    `${colors.bright}${colors.blue}=== ADF-013 CI Testing Framework Validation ====${colors.reset}\n`,
  );
  ...
  await runTest("Dependency injection", async () => {
    ...
    if (!(mockService instanceof MockImapServer)) {
      throw new Error(
        "Service factory failed to return mock implementation when forced",
      );
    }
    ...
    if (realService instanceof MockImapServer) {
      throw new Error(
        "Service factory returned mock implementation when not forced",
      );
    }
  ...
  await runTest("Jest configuration", async () => {
    ...
    if (
      !jestConfig.projects ||
      !Array.isArray(jestConfig.projects) ||
      jestConfig.projects.length < 3
    ) {
      throw new Error(
        "Jest config missing projects configuration for unit/integration/e2e",
      );
    }
  ...
  await runTest("CI workflow configuration", async () => {
    const ciConfigPath = path.resolve(
      process.cwd(),
      ".github/workflows/ci.yml",
    );
    ...
    if (
      !ciConfigContent.includes("NODE_ENV: test") ||
  ...
  await runTest("Test coverage validation", async () => {
    ...
    console.log(
      `Coverage validation successful: ${coverage.lines}% lines, ${coverage.functions}% functions`,
    );
  });
    ...
    try {
      ...
      const { success, output } = await runCommandWithTimeout(
        ...
        [
          "jest",
          '--testMatch="**/test/unit/**/*.test.ts"',
          '--testNamePattern="should work without real credentials"',
        ],
        30000,
      );
  ...
  await runTest("Cleanup procedures", async () => {
    ...
    try {
      ...
      const artifactsExist =
        fs.existsSync(
          path.resolve(process.cwd(), `test-artifact-1-${tempId}.tmp`),
        ) ||
        fs.existsSync(
          path.resolve(process.cwd(), `test-artifact-2-${tempId}.tmp`),
        );
  ...
  await runTest("Acceptance criteria validation", async () => {
    ...
    const ciPipelinePasses =
      results.tests.find((t) => t.name === "CI workflow configuration")
        ?.result === "pass" &&
      results.tests.find(
        (t) => t.name === "Environment setup without real credentials",
      )?.result === "pass";

    const unitTestsCover90Percent =
      results.tests.find((t) => t.name === "Test coverage validation")
        ?.result === "pass";

    const e2eTestUnder2Minutes =
      results.tests.find((t) => t.name === "E2E test execution time")
        ?.result === "pass";

    if (!ciPipelinePasses) {
      throw new Error(
        "Acceptance criterion failed: CI pipeline passes without real external credentials",
      );
    }

    if (!unitTestsCover90Percent) {
      throw new Error(
        "Acceptance criterion failed: Unit tests cover 90% of ADF services",
      );
    }

    if (!e2eTestUnder2Minutes) {
      throw new Error(
        "Acceptance criterion failed: E2E test processes fixture lead end-to-end within 2 minutes",
      );
    }
  ...
  results.tests.forEach((test) => {
    ...
    console.log(
      `${resultColor}${resultSymbol} ${test.name} (${test.duration}ms)${colors.reset}`,
    );
  });
  ...
  if (results.failed > 0) {
    ...
    console.log(
      `${colors.green}✓ CI pipeline passes without real external credentials${colors.reset}`,
    );
    console.log(
      `${colors.green}✓ Unit tests cover 90% of ADF services${colors.reset}`,
    );
    console.log(
      `${colors.green}✓ E2E test processes fixture lead end-to-end within 2 minutes${colors.reset}`,
    );
  }
}
