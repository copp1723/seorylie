...
async function apiRequest(endpoint: string, method = "GET", body?: any) {
  try {
    ...
    if (!apiKeys || apiKeys.length === 0) {
      ...
      const apiKeyData = await storage.generateApiKey(
        dealershipId,
        "Test API Key",
      );
...
async function testRealDealershipInventoryImport() {
  ...
  try {
    ...
    vehicles.forEach((vehicle, index) => {
      console.log(
        `Vehicle ${index + 1}: ${vehicle.year} ${vehicle.make} ${vehicle.model} (VIN: ${vehicle.vin})`,
      );
      console.log(
        `  Price: $${vehicle.price}, Color: ${vehicle.exteriorColor}, Condition: ${vehicle.condition}`,
      );
