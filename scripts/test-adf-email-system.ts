...
const defaultTestConfig: TestConfig = {
  ...
  response: 'Thank you for your interest in the 2024 Honda Civic! This vehicle features excellent fuel economy, advanced safety features, and a spacious interior. We have several models available and would love to schedule a test drive for you. Please let us know when would be convenient for you to visit our showroom.'
};
...
async function testEmailService() {
  ...
  console.log(`📬 SMTP Host: ${process.env.EMAIL_HOST || process.env.SMTP_HOST || '0.0.0.0'}`);
  console.log(`🔌 SMTP Port: ${process.env.EMAIL_PORT || process.env.SMTP_PORT || '587'}`);

  try {
    await emailService.initialize();
    const status = await emailService.getServiceStatus();
    console.log(`✅ Email service status:`, status);
    console.log(`✅ Email service connection: ${status.isConnected ? 'Success' : 'Failed'}\n`);
...
async function testSimpleOrchestrator(config: TestConfig) {
  ...
  try {
    ...
    const selectedAgent = testMessage.toLowerCase().includes('interested') ? 'inventory-agent' : 'general-agent';
...
async function testMetricsIntegration() {
  ...
  try {
    ...
    prometheusMetrics.recordEmailDelivery('test-dealership', 'test-provider', 'sent');
    prometheusMetrics.recordAIResponseLatency(1500, 'test-dealership', 'test-model', 'test-response');
...
async function runLoadTest(config: TestConfig, emailCount: number = 5) {
  ...
  console.log(`   📈 Rate: ${(emailCount / (duration / 1000)).toFixed(2)} emails/second`);
  console.log(`   🎯 Success Rate: ${((successful / emailCount) * 100).toFixed(1)}%\n`);
