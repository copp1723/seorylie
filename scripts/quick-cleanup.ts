...
const cleanupRedundantConfigs = async (): Promise<void> => {
  ...
  for (const configFile of REDUNDANT_CONFIG_FILES) {
    ...
    if (fileExists(filePath)) {
      ...
      if (await askForConfirmation(`Remove redundant config file: ${configFile}?`)) {
...
const organizeDocs = async (): Promise<void> => {
  ...
  const rootMdFiles = findFiles('*.md', ROOT_DIR)
  ...
  for (const file of filesToMove) {
    ...
    } else if (file.match(/.*_REPORT\.md$/) || file.match(/.*-report\.md$/) || file.match(/.*_SUMMARY\.md$/)) {
    ...
    if (await askForConfirmation(`Move documentation file: ${file} -> ${targetPath}?`)) {
...
const findUnusedJsFiles = async (): Promise<void> => {
  ...
  logger.info(`Found ${potentialDuplicates.length} JS files with TS equivalents`);
  ...
  for (const file of potentialDuplicates) {
    ...
    if (await askForConfirmation(`Remove JS file with TS equivalent: ${file}?`)) {
      removeFile(file);
    }
  }
};
...
// Main function
const main = async () => {
  logger.info(`Starting quick cleanup (dry-run: ${DRY_RUN}, no-confirm: ${NO_CONFIRM})`);
