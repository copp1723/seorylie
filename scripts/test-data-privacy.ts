...
import { encrypt, decrypt, maskPII, clearKeyCache } from '../server/utils/crypto';
import { redactSensitiveInfo, redactSensitivePatterns } from '../server/middleware/log-redaction';
...
const config = {
  database: {
    connectionString: process.env.TEST_DATABASE_URL || process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/cleanrylie',
...
const results = {
  ...
  failed: 0,
  skipped: 0,
  tests: {} as Record<string, { status: 'passed' | 'failed' | 'skipped'; duration: number; error?: string }>
};
...
async function runSetupTests() {
  ...
  await runTest('Database Migration Status', async () => {
    ...
    try {
      ...
      if (!byteaColumns.includes('customer_email') || 
...
async function runEncryptionTests() {
  ...
  await runTest('Basic Encryption/Decryption', async () => {
    const testValue = 'This is a test value with PII: john.doe@example.com and 555-123-4567';
    ...
    if (decrypted !== testValue) {
      throw new Error(`Decryption failed. Expected: "${testValue}", Got: "${decrypted}"`);
    }
  });
  
  await runTest('Encryption Performance', async () => {
    const testValues = Array(100).fill(0).map((_, i) => 
    ...
    console.log(`   Encrypted and decrypted ${testValues.length} values in ${duration.toFixed(2)}ms (${(duration / testValues.length).toFixed(2)}ms per value)`);
    
    // Performance should be reasonable
    if (duration > 5000) {
      throw new Error(`Encryption performance is too slow: ${duration.toFixed(2)}ms for ${testValues.length} values`);
    }
  });
  ...
  await runTest('PII Masking', async () => {
    ...
    if (maskedEmail === testEmail || !maskedEmail.includes('@') || !maskedEmail.includes('***')) {
      throw new Error(`Email masking failed: ${maskedEmail}`);
    }
    
    // Phone should be masked but retain last 4 digits
    if (maskedPhone === testPhone || !maskedPhone.includes('***') || !maskedPhone.includes('4567')) {
      throw new Error(`Phone masking failed: ${maskedPhone}`);
    }
    
    // Name should be masked but retain initials
    if (maskedName === testName || !maskedName.includes('J***') || !maskedName.includes('S***')) {
  ...
  await runTest('Key Cache Management', async () => {
    ...
    await Promise.all(Array(10).fill(0).map((_, i) => encrypt(`Cache test ${i}`)));
...
async function runLogRedactionTests() {
  ...
  await runTest('Redact Sensitive Patterns', async () => {
    ...
    console.log('   Redacted sample:', redacted.split('\n').slice(1, 3).join('\n'));
  });
  ...
  await runTest('Redact Object Properties', async () => {
    ...
    if (redacted.user.email !== '[REDACTED]' || 
    ...
    if (redacted.user.preferences.newsletter !== true || 
    ...
    if (redacted.comments.includes('john.doe@example.com') || 
        redacted.comments.includes('555-123-4567')) {
      throw new Error('Sensitive patterns in text were not redacted');
    }
    
    console.log('   Redacted object:', JSON.stringify(redacted, null, 2).substring(0, 100) + '...');
  });
  ...
  await runTest('Logger Integration', async () => {
    ...
    try {
      ...
      if (logContent.includes('john.doe@example.com') || 
...
async function runDatabaseEncryptionTests() {
  ...
  await runTest('PII Column Encryption', async () => {
    ...
    try {
      ...
      if (!(row.customer_name instanceof Buffer) || 
      ...
      if (nameStr.includes(testData.lead.customerName) || 
  ...
  await runTest('Database Decryption Functions', async () => {
    ...
    try {
      ...
      // Verify decryption worked
      if (row.name !== testData.lead.customerName || 
  ...
  await runTest('Decrypted View', async () => {
    ...
    try {
      ...
      if (row.customer_name !== testData.lead.customerName || 
...
async function runGdprEndpointTests() {
  ...
  await runTest('Right to be Forgotten Endpoint', async () => {
    ...
    try {
      ...
      `, [tempLeadId])).rows[0].id;
      
      // Call the GDPR forget endpoint
      const response = await axios.post(`${config.api.baseUrl}/api/gdpr/forget`, {
        email: tempEmail
      });
      
      if (response.status !== 200) {
        throw new Error(`GDPR forget endpoint returned status ${response.status}`);
      }
  ...
  await runTest('Data Export Endpoint', async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 401) {
        console.log('   Data export endpoint correctly requires authentication');
      } else {
        throw error;
      }
    }
  ...
  await runTest('Consent Update Endpoint', async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 401) {
        console.log('   Consent update endpoint correctly requires authentication');
      } else {
        throw error;
      }
    }
...
async function runDataExportTests() {
  ...
  await runTest('Export Decrypted Data', async () => {
    ...
    try {
      ...
      if (row.vehicle_make !== testData.lead.vehicleMake || 
  ...
  await runTest('CSV Export Format', async () => {
    ...
    try {
      ...
      if (!csv.includes(testData.lead.customerName) || 
...
async function runPurgeScriptTests() {
  ...
  await runTest('Purge Script Execution', async () => {
    ...
    try {
      ...
      const encryptedName1 = await client.query(`SELECT encrypt_pii($1) AS encrypted`, ['Expired User']);
      const encryptedEmail1 = await client.query(`SELECT encrypt_pii($1) AS encrypted`, ['expired@example.com']);
      const encryptedPhone1 = await client.query(`SELECT encrypt_pii($1) AS encrypted`, ['555-111-2222']);
      ...
      const encryptedName2 = await client.query(`SELECT encrypt_pii($1) AS encrypted`, ['Very Old User']);
      const encryptedEmail2 = await client.query(`SELECT encrypt_pii($1) AS encrypted`, ['very-old@example.com']);
      const encryptedPhone2 = await client.query(`SELECT encrypt_pii($1) AS encrypted`, ['555-333-4444']);
      ...
      const { stdout, stderr } = await execPromise(`DRY_RUN=true tsx ${scriptPath}`);
      ...
      if (!stdout.includes('would anonymize') || !stdout.includes('would delete')) {
...
function printSummary() {
  ...
  const categories: Record<string, Array<{ name: string; status: string; duration: number }>> = {};
