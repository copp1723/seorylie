...
async function testEndpoints() {
  ...
  console.log(chalk.yellow(`⚠️ Skipping ${ENDPOINTS.apiValidate} - requires authentication`));
  skippedTests++;
...
async function testEndpoint(endpoint: string, description: string) {
  ...
  try {
    ...
    if (response.ok) {
      console.log(chalk.green(`✅ ${description}: ${response.status} ${response.statusText} (${duration}ms)`));
      passedTests++;
    } else {
      console.log(chalk.red(`❌ ${description}: ${response.status} ${response.statusText}`));
      failedTests++;
    }
  } catch (error) {
    console.log(chalk.red(`❌ ${description}: Failed to connect - ${error instanceof Error ? error.message : String(error)}`));
    failedTests++;
  }
}
...
async function verifyMetrics() {
  ...
  try {
    ...
    if (metricsFound) {
      console.log(chalk.green(`✅ Performance tracking metrics verified successfully`));
      passedTests++;
    } else {
      console.log(chalk.red(`❌ Required performance tracking metrics not found`));
      failedTests++;
    }
  } catch (error) {
    console.log(chalk.red(`❌ Failed to fetch metrics: ${error instanceof Error ? error.message : String(error)}`));
    failedTests++;
  }
...
function parseAndVerifyMetrics(metricsText: string): boolean {
    ...
    if (metricMatch) {
      ...
      if (REQUIRED_METRICS.includes(metricName)) {
        ...
        if (labelsMatch) {
          ...
          metrics[metricName].labels = [...new Set([...metrics[metricName].labels, ...labels])];
          
          // Check for required labels
          const missingLabels = REQUIRED_LABELS.filter(label => !labels.includes(label));
          if (missingLabels.length === 0) {
            console.log(chalk.green(`  ✅ All required labels present for ${metricName}`));
          } else {
            console.log(chalk.yellow(`  ⚠️ Missing labels for ${metricName}: ${missingLabels.join(', ')}`));
          }
        }
      }
    }
  ...
  const missingMetrics = REQUIRED_METRICS.filter(metric => !metrics[metric].found);
  
  if (missingMetrics.length > 0) {
    console.log(chalk.red(`❌ Missing required metrics: ${missingMetrics.join(', ')}`));
    return false;
  }
  ...
  for (const metric of REQUIRED_METRICS) {
    ...
    const missingLabels = REQUIRED_LABELS.filter(label => !metrics[metric].labels.includes(label));
    if (missingLabels.length > 0) {
      console.log(chalk.yellow(`⚠️ Metric ${metric} is missing labels: ${missingLabels.join(', ')}`));
      allLabelsPresent = false;
    }
  }
  
  // Check for specific route labels from our test endpoints
  const routeLabelsFound = lines.some(line => 
    (line.includes('route="/health"') || line.includes('route="/api/v1/health"')) && 
    (line.includes('http_requests_total') || line.includes('http_request_duration_ms'))
  );
  
  if (routeLabelsFound) {
    console.log(chalk.green('✅ Found metrics with test endpoint routes'));
  } else {
    console.log(chalk.yellow('⚠️ Could not find metrics for test endpoint routes'));
    allLabelsPresent = false;
  }
...
function reportResults() {
  ...
  if (failedTests > 0) {
    ...
    console.log(chalk.green('Performance tracking is correctly implemented and metrics are being collected.'));
    process.exit(0);
  }
}
