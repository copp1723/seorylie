...
async function testLeadHandoverFormat() {
  try {
    ...
    const apiKeyResponse = await fetch(
      `${BASE_URL}/api/dealerships/${dealership.id}/apikeys`,
      {
        method: "POST",
    ...
    const personasResponse = await fetch(
      `${BASE_URL}/api/dealerships/${dealership.id}/personas`,
    );
    const personas = await personasResponse.json();

    if (!personas || personas.length === 0) {
      console.error(
        "No personas found. Make sure the database is seeded with at least one persona.",
      );
      return;
    }
    ...
    const updateResponse = await fetch(
      `${BASE_URL}/api/personas/${persona.id}`,
      {
        method: "PATCH",
        headers: {
    ...
    const initialMessage =
      "Hi there, I'm Kyle. I just moved to the area and my car broke down yesterday. I need a reliable vehicle for work ASAP.";
    ...
    const financesMessage =
      "I had some medical bills last year that hurt my credit score, but I've been at my job for 3 years with stable income. Would financing be a problem?";
    ...
    const preferencesMessage =
      "I need something fuel-efficient but spacious enough for my tools. I'm a contractor, so I'm hauling equipment daily.";
    ...
    const purchaseMessage =
      "I need to make this happen today. My buddy can drop me off at the dealership this afternoon. What's the next step to secure financing and drive home in a new truck?";
    ...
    if (purchaseData.status === "escalated") {
      console.log(
        "✅ Successfully detected high buying intent and escalated to human support.",
      );
      console.log(
        `✅ Escalation reason: ${purchaseData.escalationReason || "Not specified"}`,
      );

      console.log("\n📝 A handover dossier was generated with the new format:");
      console.log(
        "1. Lead Identification - Customer details and purchase timeline",
      );
      console.log("2. Conversation Summary - Key points and lead intent");
      console.log(
        "3. Relationship Building Information - Personal insights and communication style",
      );
      console.log(
        "4. Sales Strategies - Engagement tips and closing strategies",
      );

      console.log("\n📧 An email would have been sent to: sales@example.com");
      console.log(
        "The email contains all necessary information for the sales representative to pick up the conversation and close the deal.",
      );
    }
