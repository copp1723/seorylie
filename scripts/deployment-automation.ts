...
class DeploymentAutomation {
  ...
  private async runTests(): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.yellow('⚠️ Integration tests failed, continuing with deployment'));
    }
  ...
  private async deployToRender(): Promise<void> {
    ...
    console.log(chalk.blue('📤 Pushing to main branch (triggers auto-deploy)...'));
    
    // Commit and push changes
    try {
      this.runCommand('git add .', 'Stage changes');
      this.runCommand('git commit -m "Automated deployment commit"', 'Commit changes');
  ...
  private async postDeploymentValidation(): Promise<void> {
    ...
    while (retries < maxRetries) {
      try {
        ...
        console.log(chalk.yellow(`⏳ Retry ${retries}/${maxRetries} in 10 seconds...`));
   ...
   */
  private runCommand(command: string, description: string, timeout: number = 60000): void {
