...
async function testEmailReports() {
  try {
    ...
    const scheduleResponse = await fetch(
      `${BASE_URL}/api/dealerships/${dealership.id}/reports/schedule`,
      {
        method: "POST",
        headers: {
    ...
    console.log(
      `Found ${schedulesData.length} scheduled reports:`,
      schedulesData,
    );
    ...
    const generateResponse = await fetch(
      `${BASE_URL}/api/dealerships/${dealership.id}/reports/generate`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (scheduleData && scheduleData.id) {
      ...
      const deleteResponse = await fetch(
        `${BASE_URL}/api/dealerships/${dealership.id}/reports/schedule/${scheduleData.id}`,
        {
          method: "DELETE",
          headers: {
            "X-API-Key": apiKey,
