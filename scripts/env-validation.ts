...
class EnvironmentValidator {
    ...
    this.results.push({
      ...
      value:
        value && status !== "missing"
          ? this.maskSensitiveValue(variable, value)
          : undefined,
    });
  }
      ...
      if (!value) {
        if (rule.required) {
          this.addResult(
            rule.name,
            "missing",
            `Required environment variable is missing: ${rule.description}`,
          );
        } else {
          this.addResult(
            rule.name,
            "warning",
            `Optional environment variable is missing: ${rule.description}`,
          );
        }
        continue;
      }
  ...
  checkEnvironmentSpecificRequirements(): void {
    ...
    console.log(
      `🎯 Checking ${nodeEnv} environment specific requirements...\n`,
    );
  ...
  private validateProductionRequirements(): void {
    const productionRequired = ["DATABASE_URL", "JWT_SECRET", "SESSION_SECRET"];
    ...
    if (process.env.JWT_SECRET === "your-super-secret-jwt-key-here") {
      this.addResult(
        "JWT_SECRET",
        "invalid",
        "Using default development JWT secret in production",
      );
    }
  ...
  private validateDevelopmentRequirements(): void {
    ...
    if (!process.env.OPENAI_API_KEY) {
      this.addResult(
        "OPENAI_API_KEY",
        "warning",
        "AI features will not work without OpenAI API key",
      );
    }
  }
  ...
  run(): void {
    ...
    const hasErrors = this.results.some(
      (r) => r.status === "invalid" || r.status === "missing",
    );
