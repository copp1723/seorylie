...
const ALL_TABLES = {
  ...
  magicLinkInvitations: {
    table: mainSchema.magicLinkInvitations,
    schema: "main" as const,
  },
  sessions: { table: mainSchema.sessions, schema: "main" as const },
  reportSchedules: {
    table: mainSchema.reportSchedules,
    schema: "main" as const,
  },
  promptExperiments: {
    table: mainSchema.promptExperiments,
    schema: "main" as const,
  },
  promptVariants: { table: mainSchema.promptVariants, schema: "main" as const },
  experimentVariants: {
    table: mainSchema.experimentVariants,
    schema: "main" as const,
  },
  ...
  leadSources: {
    table: leadSchema.leadSourcesTable,
    schema: "lead-management" as const,
  },
  customers: {
    table: leadSchema.customers,
    schema: "lead-management" as const,
  },
  vehicleInterests: {
    table: leadSchema.vehicleInterests,
    schema: "lead-management" as const,
  },
  leads: { table: leadSchema.leads, schema: "lead-management" as const },
  conversations: {
    table: leadSchema.conversations,
    schema: "lead-management" as const,
  },
  messages: { table: leadSchema.messages, schema: "lead-management" as const },
  handovers: {
    table: leadSchema.handovers,
    schema: "lead-management" as const,
  },
  leadActivities: {
    table: leadSchema.leadActivities,
    schema: "lead-management" as const,
  },

  // Extensions schema (Advanced features)
  escalationTriggers: {
    table: extSchema.escalationTriggers,
    schema: "extensions" as const,
  },
  ...
  userInvitations: {
    table: extSchema.userInvitations,
    schema: "extensions" as const,
  },
  auditLogs: { table: extSchema.auditLogs, schema: "extensions" as const },
  customerProfiles: {
    table: extSchema.customerProfiles,
    schema: "extensions" as const,
  },
  customerInteractions: {
    table: extSchema.customerInteractions,
    schema: "extensions" as const,
  },
  customerInsights: {
    table: extSchema.customerInsights,
    schema: "extensions" as const,
  },
  responseSuggestions: {
    table: extSchema.responseSuggestions,
    schema: "extensions" as const,
  },
};
...
function analyzeTableStructure(
  tableName: string,
  schemaType: "main" | "lead-management" | "extensions",
): TableDefinition {
...
function buildRelationshipMap(tables: TableDefinition[]): {
  parentChildMap: Record<string, string[]>;
  dependencyChain: string[];
} {
...
function generateRecommendations(report: SchemaAnalysisReport): string[] {
  ...
  const missingCoreEntities = CORE_ENTITIES.filter(
    (entity) => !report.tables.some((table) => table.name === entity),
  );

  if (missingCoreEntities.length > 0) {
    recommendations.push(
      `❌ Missing critical core entities: ${missingCoreEntities.join(", ")}`,
    );
    recommendations.push(
      "🔧 Run database migrations to create missing core tables",
    );
  }

  // Check for proper foreign key relationships
  const tablesWithoutForeignKeys = report.tables.filter(
    (table) =>
      table.foreignKeys.length === 0 &&
      !["dealerships", "sessions", "reportSchedules"].includes(table.name),
  );

  if (tablesWithoutForeignKeys.length > 0) {
    recommendations.push(
      `⚠️  Tables without foreign key relationships: ${tablesWithoutForeignKeys.map((t) => t.name).join(", ")}`,
    );
  }

  // Check for multi-tenant setup
  const tablesWithoutDealershipId = report.tables.filter(
    (table) =>
  ...
  if (tablesWithoutDealershipId.length > 0) {
    recommendations.push(
      `🏢 Tables that may need dealership_id for multi-tenancy: ${tablesWithoutDealershipId.map((t) => t.name).join(", ")}`,
    );
  }

  // Check for audit trails
  const tablesWithoutTimestamps = report.tables.filter(
    (table) => !table.columns.some((col) => col.name.includes("created_at")),
  );

  if (tablesWithoutTimestamps.length > 0) {
    recommendations.push(
      `📅 Tables missing audit timestamps: ${tablesWithoutTimestamps.map((t) => t.name).join(", ")}`,
    );
  }
  ...
  const dataIntegrityRules = {
    ...
    requiredFields: Object.fromEntries(
      tables.map((table) => [
        table.name,
        table.columns
          .filter((col) => !col.nullable && !col.isPrimaryKey)
          .map((col) => col.name),
      ]),
    ),
...
${Object.entries(report.relationships.parentChildMap)
  .map(([parent, children]) => `- **${parent}** → [${children.join(", ")}]`)
  .join("\n")}

### Dependency Chain (Creation Order)
${report.relationships.dependencyChain
  .map((table, index) => `${index + 1}. ${table}`)
  .join("\n")}
...
${
  report.dataIntegrityRules.cascadingDeletes.length > 0
    ? report.dataIntegrityRules.cascadingDeletes
        .map((rule) => `- ${rule}`)
        .join("\n")
