...
async function setupMultiTenantTestData() {
  logger.info(
    "🚀 Starting enhanced multi-tenant test data setup for Ticket #14...",
  );
  ...
  try {
    ...
    await db
      .delete(dealerships)
      .where(eq(dealerships.subdomain, "test-dealership-a"));
    await db
      .delete(dealerships)
      .where(eq(dealerships.subdomain, "test-dealership-b"));
    await db
      .delete(dealerships)
      .where(eq(dealerships.subdomain, "test-dealership-c"));
    ...
    logger.info(
      `✅ Created dealerships: A (ID: ${dealershipA[0].id}), B (ID: ${dealershipB[0].id}), C (ID: ${dealershipC[0].id})`,
    );

    // Create users for each dealership with different roles
    logger.info(
      "👥 Creating users with different roles for each dealership...",
    );
    ...
    logger.info(
      "🚗 Creating distinct vehicle inventory for each dealership...",
    );
    ...
    const testData = {
      dealerships: [
        {
          id: dealershipA[0].id,
          name: dealershipA[0].name,
          subdomain: dealershipA[0].subdomain,
        },
        {
          id: dealershipB[0].id,
          name: dealershipB[0].name,
          subdomain: dealershipB[0].subdomain,
        },
        {
          id: dealershipC[0].id,
          name: dealershipC[0].name,
          subdomain: dealershipC[0].subdomain,
        },
      ],
      userCredentials: {
