...
const FILES_TO_RENAME = {
  "0002_sms_delivery_tracking.sql": "0010_sms_delivery_tracking.sql",
  "0002_sms_delivery_tracking_rollback.sql":
    "0010_sms_delivery_tracking_rollback.sql",
};
...
function updateReferences(): void {
  ...
  log(
    `Note: You may need to manually update references to the old migration files in:`,
  );
...
async function fixDuplicateMigrations(): Promise<void> {
  ...
  try {
    ...
    if (!validateNewFilesExist()) {
      ...
      log(
        "Please create the new migration files before running this script.",
        true,
      );
      process.exit(1);
    }
    ...
    Object.entries(FILES_TO_RENAME).forEach(([oldFile, newFile]) => {
      ...
      if (fs.existsSync(oldFilePath) && fs.existsSync(newFilePath)) {
        ...
        if (sizeDiff > 0.1) {
          log(
            `WARNING: File size difference > 10% between ${oldFile} and ${newFile}`,
            true,
          );
    ...
    try {
      if (fs.existsSync(path.join(process.cwd(), ".git"))) {
        ...
        execSync(
          'git commit -m "fix: rename duplicate migration 0002_sms_delivery_tracking to 0010_sms_delivery_tracking"',
        );
