...
program
  ...
  .option('--archive-dir <path>', 'Custom archive directory', '.branch-archives')
  .option('--verify-integration', 'Verify branches are fully integrated before deletion')
...
async function main() {
  ...
  try {
    ...
    if (config.reportOnly) {
      log(chalk.yellow('Report-only mode active. No branches will be modified.'));
      saveReport();
      return;
    }
...
function printHeader() {
  ...
  console.log(chalk.bold.blue('🧹 PLATFORM INTEGRATION BRANCH CLEANUP (INT-015) 🧹'));
  console.log(chalk.bold.blue('=================================================\n'));
  
  console.log(chalk.cyan('Current configuration:'));
  console.log(chalk.cyan(`• Dry run mode: ${config.dryRun ? chalk.green('ON') : chalk.red('OFF')}`));
  console.log(chalk.cyan(`• Interactive mode: ${config.interactive ? chalk.green('ON') : chalk.red('OFF')}`));
  console.log(chalk.cyan(`• Archive directory: ${config.archiveDir}`));
  console.log(chalk.cyan(`• Verify integration: ${config.verifyIntegration ? chalk.green('ON') : chalk.red('OFF')}`));
  console.log(chalk.cyan(`• Skip remote operations: ${config.skipRemote ? chalk.green('ON') : chalk.red('OFF')}`));
  console.log(chalk.cyan(`• Force mode: ${config.force ? chalk.yellow('ON - USE CAUTION') : chalk.green('OFF')}`));
  console.log(chalk.cyan(`• Report only: ${config.reportOnly ? chalk.green('ON') : chalk.red('OFF')}\n`));
}
...
function verifyCurrentBranch() {
  try {
    ...
    if (currentBranch !== 'integration/production-readiness-phase1') {
      if (config.force) {
        report.warnings.push(`Not on integration branch. Currently on: ${currentBranch}`);
        log(chalk.yellow(`⚠️  WARNING: You are not on the integration branch. Currently on: ${currentBranch}`));
      } else {
        throw new Error(`Must be on integration/production-readiness-phase1 branch. Currently on: ${currentBranch}`);
      }
    } else {
      log(chalk.green(`✓ Verified: Currently on integration branch: ${currentBranch}`));
    }
  } catch (error) {
  ...
  try {
    if (!fs.existsSync(config.archiveDir)) {
      if (!config.dryRun && !config.reportOnly) {
        ...
        log(chalk.yellow(`Would create archive directory: ${config.archiveDir}`));
      }
    } else {
...
function getAllBranches(): Branch[] {
  try {
    ...
    const allBranchNames = Array.from(new Set([...localBranches, ...remoteBranches]));
    ...
    return allBranchNames.map(name => {
      try {
        const lastCommitInfo = execSync(`git log -1 --format="%H|%an|%ae|%ad|%s" ${name}`).toString().trim().split('|');
...
function extractTicketReference(branchName: string, commitMessage: string): string | undefined {
  // Try to extract from branch name first
  const branchTicketMatch = branchName.match(/^([CHIUT]\d+)-/) || branchName.match(/^(INT-\d+)/);
  if (branchTicketMatch) {
    return branchTicketMatch[1];
  }
  
  // Try to extract from commit message
  const commitTicketMatch = commitMessage.match(/\[(INT-\d+|[CHIUT]\d+)\]/) || 
...
async function verifyBranchIntegration(branches: Branch[]) {
  ...
  for (const branch of branches) {
    if (branch.category === BranchCategory.PROTECTED || 
        branch.category === BranchCategory.INTEGRATION) {
    ...
    try {
      ...
      if (!uniqueCommits) {
        ...
        log(chalk.green(`✓ Branch ${branch.name} is fully merged into ${integrationBranch}`));
      } else {
        const commitCount = uniqueCommits.split('\n').length;
        branch.mergeStatus = MergeStatus.PARTIALLY_MERGED;
        log(chalk.yellow(`⚠️  Branch ${branch.name} has ${commitCount} commits not in ${integrationBranch}`));
        
        // Add to warnings
        report.warnings.push(`Branch ${branch.name} has ${commitCount} commits not in ${integrationBranch}`);
      }
    } catch (error) {
      branch.mergeStatus = MergeStatus.UNKNOWN;
      log(chalk.red(`✗ Failed to verify merge status for ${branch.name}`));
      report.errors.push(`Failed to verify merge status for ${branch.name}: ${error}`);
    }
  }
...
function printBranchReport(branches: Branch[]) {
  ...
  if (config.verifyIntegration) {
    ...
    console.log(`• Fully Merged: ${mergeStatusCounts[MergeStatus.FULLY_MERGED]}`);
    console.log(`• Partially Merged: ${mergeStatusCounts[MergeStatus.PARTIALLY_MERGED]}`);
  ...
  const integratedBranches = branches.filter(b => INTEGRATED_BRANCHES.includes(b.name));
  if (integratedBranches.length === 0) {
    console.log('• None found');
  } else {
    integratedBranches.forEach(branch => {
      const ticketRef = branch.ticketReference ? `[${branch.ticketReference}]` : '';
      console.log(`• ${branch.name} ${ticketRef}`);
    });
  }
  
  console.log(chalk.bold('\nPotentially Unintegrated Branches:'));
  const unintegratedBranches = branches.filter(b => 
...
async function selectBranchesForCleanup(branches: Branch[]): Promise<Branch[]> {
  ...
  const eligibleBranches = branches.filter(
    b => b.category !== BranchCategory.PROTECTED && 
         !PROTECTED_BRANCHES.includes(b.name)
  );
  
  // Default selection: all integrated branches
  let selectedBranches = eligibleBranches.filter(b => 
    INTEGRATED_BRANCHES.includes(b.name) || 
    (b.mergeStatus === MergeStatus.FULLY_MERGED && b.category === BranchCategory.FEATURE)
  );
    ...
    const { selectedBranchNames } = await inquirer.prompt([
      {
        ...
        choices: eligibleBranches.map(branch => {
          const ticketRef = branch.ticketReference ? `[${branch.ticketReference}]` : '';
          const integrated = INTEGRATED_BRANCHES.includes(branch.name) ? '[INTEGRATED]' : '';
          const mergeStatus = branch.mergeStatus ? `[${branch.mergeStatus}]` : '';
          
          return {
            name: `${branch.name} ${ticketRef} ${integrated} ${mergeStatus}`,
            value: branch.name,
            checked: INTEGRATED_BRANCHES.includes(branch.name) || branch.mergeStatus === MergeStatus.FULLY_MERGED
          };
        })
      }
    ]);
    
    selectedBranches = eligibleBranches.filter(b => selectedBranchNames.includes(b.name));
  }
  
  log(chalk.green(`✓ Selected ${selectedBranches.length} branches for cleanup`));
...
async function confirmCleanup(branches: Branch[]): Promise<boolean> {
  ...
  console.log(chalk.bold('The following branches will be archived and deleted:'));
  branches.forEach(branch => {
    const ticketRef = branch.ticketReference ? `[${branch.ticketReference}]` : '';
    console.log(`• ${branch.name} ${ticketRef}`);
  });
  ...
  const { confirm } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'confirm',
      message: chalk.bold.red('Are you sure you want to proceed with branch cleanup?'),
      default: false
    }
  ]);
...
async function archiveBranches(branches: Branch[]) {
  ...
  if (config.dryRun) {
    log(chalk.yellow(`Would archive ${branches.length} branches to ${archiveFilePath}`));
  ...
  try {
    ...
    const metadataPath = path.join(config.archiveDir, `metadata-${archiveTimestamp}.json`);
    const metadata = {
    ...
    for (const branch of branches) {
      ...
      execSync(`git format-patch --stdout origin/main..${branch.name} > ${patchPath}`);
    ...
    log(chalk.green(`✓ Archived ${branches.length} branches to ${archiveFilePath}`));
    report.branchesArchived = branches;
  } catch (error) {
...
function updateCiCdReferences(deletedBranches: Branch[]) {
  if (deletedBranches.length === 0 || config.dryRun || config.reportOnly) return;
      ...
      for (const file of workflowFiles) {
        ...
        for (const branch of deletedBranches) {
          ...
          if (branchRegex.test(content)) {
            updatedContent = updatedContent.replace(branchRegex, 'integration/production-readiness-phase1');
            hasChanges = true;
          }
        }
    ...
    if (fs.existsSync('README.md')) {
      ...
      for (const branch of deletedBranches) {
        ...
        if (branchRegex.test(readmeContent)) {
          updatedReadme = updatedReadme.replace(branchRegex, 'integration/production-readiness-phase1');
          hasChanges = true;
        }
      }
...
function saveReport() {
  ...
  try {
    if (!config.dryRun) {
      ...
      const mdReportPath = path.join(config.archiveDir, `branch-cleanup-report-${reportTimestamp}.md`);
