...
function loadBaseline(): BundleSizeBaseline {
  const baselinePath = path.join(ROOT_DIR, '.stabilization', 'bundle.baseline.json');
...
function updateBaseline(analysis: BundleAnalysis): void {
  ...
  try {
    ...
    console.error(`❌ Failed to update baseline file: ${error instanceof Error ? error.message : error}`);
    process.exit(1);
  }
}
...
async function checkBundleSize(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.log('⚠️ Baseline file not found or invalid. Creating a new baseline...');
      updateBaseline(analysis);
      return;
    }
    ...
    const tolerancePercentage = baseline.validation_rules.tolerance_percentage || 10;
    const maxWithTolerance = Math.floor(maxAllowedSize * (1 + tolerancePercentage / 100));
    ...
    console.log(`Baseline:      ${formatBytes(baseline.bundle_size.current_size_bytes)}`);
    console.log(`Max Allowed:   ${formatBytes(maxAllowedSize)}`);
    console.log(`With Tolerance (${tolerancePercentage}%): ${formatBytes(maxWithTolerance)}`);
    ...
    if (analysis.totalBundleSize > maxWithTolerance) {
      ...
      const excessPercentage = ((analysis.totalBundleSize / maxAllowedSize) - 1) * 100;
      
      console.log('❌ Bundle Size Check Failed');
      console.log(`Bundle size (${formatBytes(analysis.totalBundleSize)}) exceeds maximum allowed (${formatBytes(maxWithTolerance)})`);
      console.log(`Excess: ${formatBytes(excessBytes)} (+${excessPercentage.toFixed(1)}%)`);
      console.log('');
      ...
      console.log('  - Update baseline if increase is intentional: npm run bundle:update');
    ...
    const utilizationPercentage = (analysis.totalBundleSize / maxAllowedSize) * 100;
