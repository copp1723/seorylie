 ...
 */
async function setupRlsContext(
  dealershipId: number,
  role: string,
): Promise<void> {
  await db.execute(`SELECT setup_rls_context($1, $2)`, [dealershipId, role]);
  console.log(
    chalk.blue(`Set RLS context: dealership=${dealershipId}, role=${role}`),
  );
}
...
async function createTestData(): Promise<void> {
  ...
  console.log(
    chalk.green(`Created ${testData.dealerships.length} dealerships`),
  );
  console.log(chalk.green(`Created ${testData.vehicles.length} vehicles`));
  console.log(
    chalk.green(`Created ${testData.conversations.length} conversations`),
  );
}
...
async function cleanupTestData(): Promise<void> {
  ...
  for (const conversation of testData.conversations) {
    await db.execute(`DELETE FROM conversations WHERE id = $1`, [
      conversation.id,
    ]);
  }
  ...
  try {
    ...
    if (passed === expectedToPass) {
      ...
      if (expectedToPass) {
        ...
        console.log(
          chalk.red(`  Expected test to fail but it passed (security issue!)`),
        );
      }
    }
  } catch (error) {
...
async function testCrossTenantIsolation(): Promise<boolean> {
  ...
  const vehicle = testData.vehicles.find(
    (v) => v.dealershipId === dealership2.id,
  );
  if (!vehicle) {
  ...
  const conversation = testData.conversations.find(
    (c) => c.dealershipId === dealership2.id,
  );
  if (!conversation) {
    throw new Error(
      "Test data issue: No conversation found for second dealership",
    );
  }
...
async function runRlsTests(): Promise<void> {
  console.log(
    chalk.yellow("=== ROW LEVEL SECURITY (RLS) VALIDATION TESTS ==="),
  );
  console.log(
    chalk.yellow("This script tests database-level security isolation"),
  );
  ...
  try {
    ...
    await runTest(
      "RLS functions exist and are properly configured",
      testRlsFunctionsExist,
    );
    await runTest(
      "Super admin can access all dealerships data",
      testSuperAdminAccess,
    );
    await runTest(
      "Regular user can only access their dealership data",
      testRegularUserAccess,
    );
    await runTest(
      "Cross-tenant isolation prevents access to other dealerships",
      testCrossTenantIsolation,
    );
    await runTest(
      "Direct ID-based access to other dealerships is prevented",
      testDirectIdAccess,
    );
    await runTest(
      "No RLS context prevents access to all data",
      testNoContextAccess,
    );
    await runTest(
      "Dealership admin can access all data within their dealership",
      testDealershipAdminAccess,
    );
    ...
    if (testResults.failed === 0) {
      ...
      console.log(
        chalk.red(
          "Review the test results and fix any issues before deploying to production",
        ),
      );
    }
  } catch (error) {
