...
async function main() {
  ...
  if (!customerId || !filePath) {
    console.error('Usage: pnpm ts-node scripts/ingest-leads.ts <customerId> <file>');
    process.exit(1);
  }
  ...
  for (const adf of leads) {
    ...
    if (result.success && result.mappedLead) {
      await axios.post(`${process.env.API_BASE_URL}/api/v1/leads/ingest/${customerId}`, result.mappedLead);
      count++;
