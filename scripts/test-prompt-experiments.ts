...
async function createPromptVariant(dealershipId: number) {
  ...
  const description = await prompt(
    "Enter a brief description of this variant: ",
  );

  const isControlStr = await prompt(
    "Is this the control variant for experiments? (y/n): ",
  );
...
async function listPromptVariants(dealershipId?: number) {
  ...
  try {
    ...
    variants.forEach((variant, index) => {
      console.log(
        `${index + 1}. ${variant.name}${variant.isControl ? " (Control)" : ""}${variant.isActive ? "" : " (Inactive)"}`,
      );
...
async function viewPromptVariant(variantId: number) {
  ...
  try {
    ...
    console.log(
      `Name: ${variant.name}${variant.isControl ? " (Control)" : ""}${variant.isActive ? "" : " (Inactive)"}`,
    );
    ...
    if (metrics.length > 0) {
      ...
      const escalationRate =
        (metrics.filter((m) => m.wasEscalated).length / metrics.length) * 100;
      ...
      if (metrics.some((m) => m.customerRating !== null)) {
        ...
        const avgRating =
          ...
          ratedMetrics.length;
        console.log(
          `Average customer rating: ${avgRating.toFixed(1)}/5 (from ${ratedMetrics.length} ratings)`,
        );
      }
    }
...
async function createExperiment(dealershipId: number) {
  ...
  if (variants.length < 2) {
    console.log(
      "You need at least 2 active prompt variants to create an experiment.",
    );

    const createNewVariant = await prompt(
      "Would you like to create a new variant now? (y/n): ",
    );
  ...
  const endDateStr = await prompt(
    `Enter end date (YYYY-MM-DD, default: ${defaultEndDate.toISOString().split("T")[0]}): `,
  );
  ...
  if (endDateStr) {
    try {
      ...
      console.log(
        "Invalid date format, using default end date (2 weeks from now).",
      );
    }
  }

  // Select variants to include
  const selectedVariantIndices = (
    await prompt(
      'Enter the variant numbers to include (comma-separated, e.g., "1,3,4"): ',
    )
  )
    .split(",")
  ...
  try {
    ...
    let remainingTraffic =
      100 - defaultTrafficPerVariant * selectedVariants.length;

    for (const variant of selectedVariants) {
      const trafficAllocation =
        defaultTrafficPerVariant + (remainingTraffic > 0 ? 1 : 0);
      ...
      console.log(
        `- Added variant "${variant.name}" with ${trafficAllocation}% traffic allocation`,
      );
    }
...
async function listExperiments(dealershipId?: number) {
  ...
  try {
    let query = db
      ...
      .orderBy(desc(promptExperiments.startDate));
    ...
    for (const experiment of experiments) {
      ...
      console.log(
        `Status: ${status}${experiment.isActive ? "" : " (Inactive)"}`,
      );
      if (experiment.description)
        console.log(`Description: ${experiment.description}`);
      console.log(
        `Period: ${experiment.startDate.toLocaleDateString()} to ${experiment.endDate ? experiment.endDate.toLocaleDateString() : "Ongoing"}`,
      );
      ...
      if (expVariants.length > 0) {
        ...
        for (const { expVariant, variant } of expVariants) {
          console.log(
            `- ${variant.name}${variant.isControl ? " (Control)" : ""}: ${expVariant.trafficAllocation}% traffic`,
          );
        }
      }
...
async function viewExperiment(experimentId: number) {
  ...
  try {
    ...
    console.log(
      `End Date: ${experiment.endDate ? experiment.endDate.toLocaleString() : "Ongoing"}`,
    );
      ...
      for (const { expVariant, variant } of expVariants) {
        ...
        if (metrics.length > 0) {
          ...
          const avgTokensUsed =
            ...
            metrics.length;
          const escalationRate =
            (metrics.filter((m) => m.wasEscalated).length / metrics.length) *
            100;
          ...
          if (metrics.some((m) => m.customerRating !== null)) {
            const ratedMetrics = metrics.filter(
              (m) => m.customerRating !== null,
            );
            const avgRating =
              ratedMetrics.reduce(
                (sum, m) => sum + (m.customerRating || 0),
                0,
              ) / ratedMetrics.length;
            console.log(
              `- Avg customer rating: ${avgRating.toFixed(1)}/5 (from ${ratedMetrics.length} ratings)`,
            );
          }
        } else {
...
async function testPromptVariant(variantId: number) {
  ...
  try {
    ...
    SAMPLE_CONVERSATION_TYPES.forEach((type, idx) => {
      console.log(
        `${idx + 1}. ${type.replace("_", " ").charAt(0).toUpperCase() + type.replace("_", " ").slice(1)}`,
      );
    });

    const typeChoice = await prompt("\nEnter choice (1-6): ");
    const conversationType =
      SAMPLE_CONVERSATION_TYPES[parseInt(typeChoice) - 1] ||
      SAMPLE_CONVERSATION_TYPES[0];

    // Display message options for the selected type
    console.log(
      `\nSelect a sample message for "${conversationType.replace("_", " ")}":`,
    );
    sampleMessages[conversationType as keyof typeof sampleMessages].forEach(
      (msg, idx) => {
        console.log(`${idx + 1}. ${msg}`);
      },
    );
    console.log(
      `${sampleMessages[conversationType as keyof typeof sampleMessages].length + 1}. Enter your own message`,
    );
    ...
    if (
      parseInt(messageChoice) >
      sampleMessages[conversationType as keyof typeof sampleMessages].length
    ) {
      ...
      if (
        ...
        testMessage =
          sampleMessages[conversationType as keyof typeof sampleMessages][idx];
      } else {
        testMessage =
          sampleMessages[conversationType as keyof typeof sampleMessages][0];
      }
    }
    ...
    try {
      ...
      console.log(
        `- Assistant response length: ${response.message.length} chars`,
      );
    ...
    while (running) {
      ...
      switch (choice) {
        ...
        case "3": {
          ...
          if (variants.length > 0) {
            const variantId = parseInt(
              await prompt("Enter variant ID to view: "),
            );
        ...
        case "4": {
          ...
          if (variants.length > 0) {
            const variantId = parseInt(
              await prompt("Enter variant ID to test: "),
            );
        ...
        case "7": {
          ...
          if (experiments.length > 0) {
            const experimentId = parseInt(
              await prompt("Enter experiment ID to view: "),
            );
