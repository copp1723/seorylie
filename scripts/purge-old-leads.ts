...
const config = {
  ...
  retention: {
    ...
    extendedConsentDays: parseInt(
      process.env.GDPR_EXTENDED_CONSENT_DAYS || "1095",
      10,
    ), // ~3 years
    veryOldRecordsDays: parseInt(
      process.env.GDPR_VERY_OLD_RECORDS_DAYS || "1825",
      10,
    ), // ~5 years
  ...
  execution: {
    ...
    auditLogPath:
      ...
      path.join(process.cwd(), "logs", "data-retention"),
  },
};
...
async function ensureAuditLogDirectory(): Promise<void> {
  try {
    ...
    logger.debug("Audit log directory ensured", {
  ...
  try {
    ...
    try {
      ...
      if (!encryptionKeyResult.rows[0].encryption_key) {
        throw new Error(
          "Database encryption key (app.encryption_key) is not set",
        );
      }
...
async function anonymizeExpiredRecords(): Promise<void> {
  ...
  try {
    if (config.execution.dryRun) {
      ...
      logger.info("Dry run: would anonymize records", {
        count: stats.recordsAnonymized,
      });
    } else {
      ...
      logger.info("Anonymized expired records", {
        count: stats.recordsAnonymized,
      });
...
async function deleteVeryOldRecords(): Promise<void> {
  ...
  try {
    if (config.execution.dryRun) {
      ...
      logger.info("Dry run: would delete very old records", {
        count: stats.recordsDeleted,
      });
    } else {
      ...
      while (processed < stats.recordsScanned) {
        ...
        for (const record of batchResult.rows) {
          if (record.consent_given) {
            ...
            extendedRetention.setDate(
              extendedRetention.getDate() +
                config.retention.extendedConsentDays,
            );
...
async function generateAuditReport(): Promise<string> {
  ...
  try {
    ...
    const reportPath = path.join(
      config.execution.auditLogPath,
      `data-retention-${timestamp}.json`,
    );
    ...
    const report = {
      ...
      stats: {
        ...
        durationMs:
          (stats.endTime || new Date()).getTime() - stats.startTime.getTime(),
      },
      config: {
        dryRun: config.execution.dryRun,
        retentionDays: config.retention.defaultDays,
...
async function sendCompletionNotification(reportPath: string): Promise<void> {
  ...
  try {
    logger.info("Sending completion notification", {
      to: maskPII(config.execution.notifyEmail, "email"),
    });
