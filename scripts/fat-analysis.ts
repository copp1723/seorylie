...
const findFiles = (patterns: string[], baseDir: string = ROOT_DIR): string[] => {
  try {
    const patternString = patterns.map(p => `-name "${p}"`).join(' -o ');
    const ignoreString = IGNORE_PATTERNS.map(p => `-not -path "*/${p}/*"`).join(' ');
...
const countFileUsage = (filePath: string, baseDir: string = ROOT_DIR): number => {
  try {
    ...
    const requireCount = parseInt(execSync(requireCommand).toString().trim(), 10);
...
const analyzePackageJsonScripts = (): void => {
  ...
  const packageJson = readJsonFile<{ scripts: Record<string, string> }>(PACKAGE_JSON_PATH);
  ...
  if (scriptCount > 30) {
    logger.warn(`Excessive number of scripts (${scriptCount}). Consider consolidating.`);
  }
  ...
  Object.entries(scriptGroups).forEach(([prefix, groupScripts]) => {
    ...
    if (groupScripts.length > 5) {
      logger.warn(`  - Excessive "${prefix}" scripts (${groupScripts.length}). Consider consolidation.`);
  ...
  if (testScripts.length > 10) {
    ...
    const unusedTestScripts = testScripts.filter(([_, command]) => command.includes('|| exit 0'));
    if (unusedTestScripts.length > 0) {
      logger.warn(`Found ${unusedTestScripts.length} potentially unused test scripts (using "|| exit 0" pattern):`);
  ...
  const duplicateCommands = Object.entries(commandCounts).filter(([_, count]) => count > 1);
  ...
  const redundantScriptGroups = Object.entries(similarScripts).filter(([_, names]) => names.length > 1);
...
const analyzeUnusedDependencies = async (): Promise<void> => {
  ...
  const uiFrameworks = ['@radix-ui', '@mui', '@chakra-ui', '@mantine', 'antd', 'bootstrap'];
  ...
  try {
    ...
    try {
      ...
      const depcheckOutput = execSync('npx depcheck --json', { stdio: 'pipe' }).toString();
      ...
      logger.info('Depcheck not available, using simple grep-based analysis...');
      
      const allFiles = findFiles(['*.ts', '*.tsx', '*.js', '*.jsx']);
      const fileContents = allFiles.map(file => fs.readFileSync(file, 'utf8')).join('\n');
      ...
      const unusedDependencies = Object.keys(dependencies).filter(dep => {
        ...
        const regex = new RegExp(`(import|require|from).*['"].*${normalizedDep}`, 'i');
...
const analyzeLargeFiles = (): void => {
  ...
  const allFiles = findFiles(['*.ts', '*.tsx', '*.js', '*.jsx', '*.json', '*.md']);
  ...
  if (largeFiles.length > 0) {
    logger.warn(`Found ${largeFiles.length} large files (>${MAX_FILE_SIZE_KB}KB):`);
...
const analyzeRedundantConfigs = (): void => {
  ...
  configPatterns.forEach(({ name, pattern }) => {
    ...
    if (configs.length > 1) {
      ...
      logger.verbose(`Found single ${name} configuration: ${path.relative(ROOT_DIR, configs[0])}`);
    }
  });
  ...
  if (envFiles.length > 3) { // .env, .env.example, .env.local is reasonable
...
const analyzeDeadCode = (): void => {
  ...
  const sourceFiles = findFiles(['*.ts', '*.tsx']).filter(file => 
    !file.includes('test/') && 
    !file.includes('.test.') && 
    !file.includes('.spec.') &&
  ...
  if (lowUsageFiles.length > 0) {
    logger.warn(`Found ${lowUsageFiles.length} potentially unused or low-usage files:`);
  ...
  try {
    ...
    const filesWithComments = execSync(commentedCodeCommand).toString().trim().split('\n').filter(Boolean);
    
    if (filesWithComments.length > 0) {
      logger.info(`Found ${filesWithComments.length} files with commented code blocks.`);
      logger.info('Consider removing commented code or converting to proper documentation.');
      ...
      if (VERBOSE) {
        ...
        if (filesWithComments.length > 10) {
          logger.verbose(`  ... and ${filesWithComments.length - 10} more files`);
        }
      }
    }
  } catch (error) {
...
const analyzeDuplicateCode = (): void => {
  ...
  const utilFunctions = [
    ...
    'getConfig', 'loadConfig', 'readConfig',
    'logger', 'createLogger', 'setupLogger',
    'fetchData', 'getData', 'getResource',
    'handleError', 'processError', 'formatError',
  ];
  ...
  utilFunctions.forEach(funcName => {
    try {
      ...
      if (count > 1) {
        logger.warn(`Potential duplicate utility function: "${funcName}" (${count} occurrences)`);
        ...
        if (VERBOSE) {
          ...
          const locations = execSync(locationCommand).toString().trim().split('\n');
  ...
  const componentFiles = findFiles(['*.tsx']).filter(file => 
  ...
  componentNames.forEach(name => {
    ...
    componentNameCounts[normalizedName] = (componentNameCounts[normalizedName] || 0) + 1;
  });
...
const analyzeTestingFrameworks = (): void => {
  ...
  if (frameworks.length > 1) {
    logger.warn(`Multiple testing frameworks detected: ${frameworks.join(', ')}`);
...
const generateRecommendations = (): void => {
  ...
  logger.info('Based on the analysis, here are recommended actions to trim the fat:');
  ...
  logger.info('   - Remove dependencies with "|| exit 0" patterns (likely unused)');
  ...
  logger.info('2. Create a cleanup branch for implementing these recommendations');
  logger.info('3. Run tests after each cleanup step to ensure functionality');
  logger.info('4. Consider creating automated cleanup scripts for ongoing maintenance');
};

// Main function
const main = async () => {
  logger.info(`Starting fat analysis${VERBOSE ? ' (verbose mode)' : ''}${FIX ? ' (fix mode)' : ''}`);
