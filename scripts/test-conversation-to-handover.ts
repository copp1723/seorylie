...
In these cases, politely offer to connect them with a sales representative.`;
    ...
    return {
      response:
        "I apologize, but I'm having trouble responding right now. Let me connect you with a sales representative who can assist you better.",
      shouldEscalate: true,
...
async function runSimulatedConversation(
  ...
  await db.insert(messages).values({
    ...
    isFromCustomer: false,
    channel: "text",
    metadata: aiResponse.shouldEscalate
      ? { handoverReason: aiResponse.reason }
      : undefined,
  });
  ...
  if (continueConversation.toLowerCase() === "y") {
    ...
    while (conversationActive) {
      ...
      await db.insert(messages).values({
        ...
        isFromCustomer: false,
        channel: "text",
        metadata: nextAiResponse.shouldEscalate
          ? { handoverReason: nextAiResponse.reason }
          : undefined,
      });
  ...
  return {
    ...
    handoverTriggered: allMessages.some(
      (msg) =>
        !msg.isFromCustomer && msg.metadata && msg.metadata.handoverReason,
    ),
  };
}
...
async function runBatchTesting(dealershipId: number, personaId?: number) {
  ...
  const scenarioKeys = Object.keys(sampleConversations) as Array<
    keyof typeof sampleConversations
  >;
  const results: Array<{
    scenario: string;
  ...
  for (const scenarioKey of scenarioKeys) {
    for (
      ...
      complexityLevel < sampleConversations[scenarioKey].customerReplies.length;
      complexityLevel++
    ) {
      console.log(
...
async function testIntentDetection(
  ...
  for (
    let complexityLevel = 0;
    complexityLevel < scenario.customerReplies.length;
    complexityLevel++
  ) {
...
`;
  ...
  const reportPath = path.join(
    TEST_LOGS_DIR,
    `intent-detection-${scenarioKey}-${Date.now()}.txt`,
  );
  fs.writeFileSync(reportPath, reportContent);
  ...
  try {
    ...
    if (personaList.length > 0) {
      ...
      personaList.forEach((persona, index) => {
        console.log(
          `${index + 1}. ${persona.name}${persona.isControl ? " (Control)" : ""}`,
        );
      });

      const personaIndex = parseInt(
    ...
    while (running) {
      ...
      switch (choice) {
        case "1": {
          ...
          if (scenarioIndex >= 0 && scenarioIndex < scenarioKeys.length) {
            ...
            const complexityLevels =
              sampleConversations[scenarioKey].customerReplies.length;
            console.log(
              `\nThis scenario has ${complexityLevels} complexity levels:`,
            );
            for (let i = 0; i < complexityLevels; i++) {
              console.log(
                `${i + 1}. ${i === 0 ? "Standard" : i === 1 ? "Challenging" : "Difficult"}`,
              );
            }

            const complexityIndex =
              parseInt(await prompt("\nSelect complexity level (number): ")) -
              1;
        ...
        case "2":
          await runBatchTesting(selectedDealership.id, selectedPersonaId);
          break;
