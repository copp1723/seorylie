...
class HealthChecker {
  ...
  async checkEnvironment(): Promise<void> {
    ...
    const optionalEnvVars = ["REDIS_URL", "OPENAI_API_KEY", "NODE_ENV"];
    ...
    for (const envVar of optionalEnvVars) {
      if (!process.env[envVar]) {
        this.addResult(
          "Environment",
          "warning",
          `Missing optional environment variable: ${envVar}`,
        );
        missingOptional++;
      }
    }
    ...
    console.log(
      `✅ Environment check completed (${missingRequired} missing required, ${missingOptional} missing optional)`,
    );
  }
    ...
    try {
      ...
      if (foundProblematic.length > 0) {
        this.addResult(
          "Package",
          "unhealthy",
          "Found problematic dependencies",
          {
  ...
  async checkFileSystem(): Promise<void> {
    ...
    console.log(
      `✅ File system check completed (${missingFiles} missing files, ${missingDirs} missing dirs)`,
    );
  }
