...
const trimPackageJsonScripts = async (): Promise<void> => {
  ...
  if (DRY_RUN) {
    uniqueScriptsToRemove.forEach((script) => {
      logger.dryRun(
        `Would remove script: ${script}: ${originalScripts[script]}`,
      );
    });
    return;
  }
  ...
  if (uniqueScriptsToRemove.length > 0) {
    ...
    if (await askForConfirmation(confirmMessage)) {
      ...
      if (writeJsonFile(packageJsonPath, packageJson)) {
        logger.success(
          `Removed ${uniqueScriptsToRemove.length} scripts from package.json`,
        );
      }
    }
  }
...
const trimDependencies = async (): Promise<void> => {
  ...
  const unusedDependencies = [
    ...
    ...(packageJson.dependencies?.jest && packageJson.dependencies?.vitest
      ? ["jest"]
      : []),
    ...(packageJson.devDependencies?.jest && packageJson.devDependencies?.vitest
      ? ["jest"]
      : []),
  ...
  if (depsToRemove.length > 0) {
    ...
    if (await askForConfirmation(confirmMessage)) {
      ...
      if (writeJsonFile(packageJsonPath, packageJson)) {
        logger.success(
          `Removed ${depsToRemove.length} dependencies from package.json`,
        );
...
const trimConfigFiles = async (): Promise<void> => {
  ...
  if (
    configFiles.jest &&
    configFiles.jest.length > 0 &&
    configFiles.vitest &&
    configFiles.vitest.length > 0
  ) {
...
const removeDeadCode = async (): Promise<void> => {
  ...
  if (AGGRESSIVE && deadFiles.length > 0) {
    ...
    logger.info(
      "Skipping dead code removal. Use --aggressive to remove unused files.",
    );
    deadFiles.forEach((file) => {
...
const main = async () => {
  logger.info(
    `Starting fat trimming (dry-run: ${DRY_RUN}, no-confirm: ${NO_CONFIRM}, aggressive: ${AGGRESSIVE})`,
  );
