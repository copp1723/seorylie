...
async function testPoolConfiguration() {
  ...
  if (stats.max !== EXPECTED_MAX_CONNECTIONS) {
    throw new Error(`Max connections is ${stats.max}, expected ${EXPECTED_MAX_CONNECTIONS}`);
  }
  ...
  if (poolConfig.idleTimeoutMillis !== EXPECTED_IDLE_TIMEOUT_MS) {
    throw new Error(`Idle timeout is ${poolConfig.idleTimeoutMillis}ms, expected ${EXPECTED_IDLE_TIMEOUT_MS}ms`);
  }
  
  console.log(chalk.gray(`  ✓ Max connections: ${stats.max}`));
  console.log(chalk.gray(`  ✓ Idle timeout: ${poolConfig.idleTimeoutMillis}ms`));
}
...
async function testGetPoolStats() {
  ...
  if (duringStats.active <= initialActive && initialActive === 0) {
    console.log(chalk.yellow('  ⚠ Warning: Could not verify active connections change during query'));
    console.log(chalk.yellow('    This may be normal in a test environment with low load'));
  }
  ...
  console.log(chalk.gray(`  ✓ During query active connections: ${duringStats.active}`));
  console.log(chalk.gray(`  ✓ After query active connections: ${afterStats.active}`));
}
...
async function testPrometricsMetrics() {
  ...
  const poolMetrics = metrics.filter(m => 
    m.name.startsWith('db_pool_') || 
    m.name === 'pg_pool_connections'
  );
  ...
  const missingMetrics = requiredMetrics.filter(name => !foundMetrics.includes(name));
  
  if (missingMetrics.length > 0) {
    throw new Error(`Missing required metrics: ${missingMetrics.join(', ')}`);
  }
  
  console.log(chalk.gray(`  ✓ Found ${poolMetrics.length} database pool metrics`));
  console.log(chalk.gray(`  ✓ All required metrics are registered`));
  
  // Verify metrics values
  for (const metric of poolMetrics) {
    console.log(chalk.gray(`  ✓ ${metric.name}: ${metric.values[0]?.value ?? 'N/A'}`));
  }
}
...
async function testDatabaseConnectivity() {
  console.log(chalk.gray(`  ▶ Running ${TEST_QUERIES_COUNT} test queries (${CONCURRENT_QUERIES} concurrent)...`));
  ...
  async function runQuery(id: number) {
    try {
      ...
      const result = await db.query('SELECT $1::text as message', [`Test query ${id}`]);
  ...
  console.log(chalk.gray(`  ✓ All ${results.length} queries completed successfully`));
  console.log(chalk.gray(`  ✓ Total duration: ${totalDuration.toFixed(2)}ms`));
  console.log(chalk.gray(`  ✓ Average query duration: ${avgDuration.toFixed(2)}ms`));
  
  // Check pool stats after the test
  const statsAfter = await getPoolStats();
  console.log(chalk.gray(`  ✓ Pool stats after test: active=${statsAfter.active}, idle=${statsAfter.idle}, waiting=${statsAfter.waiting}`));
}
...
async function testPoolBehavior() {
  ...
  console.log(chalk.gray(`  ▶ Testing pool behavior with ${OVERLOAD_QUERIES} concurrent queries...`));
  ...
  async function runSlowQuery(id: number) {
    try {
      ...
      const result = await db.query('SELECT pg_sleep(0.5), $1::text as id', [`query-${id}`]);
      const duration = performance.now() - start;
      return { id, duration };
    } catch (error) {
      return { id, error };
  ...
  console.log(chalk.gray(`  ✓ Pool stats before load test: active=${statsBefore.active}, idle=${statsBefore.idle}, waiting=${statsBefore.waiting}`));
  
  // Run all queries concurrently
  const startTime = performance.now();
  const queries = Array.from({ length: OVERLOAD_QUERIES }, (_, i) => runSlowQuery(i));
  ...
  if (errors.length > 0) {
    console.log(chalk.yellow(`  ⚠ Warning: ${errors.length} queries failed during load test`));
  }
  
  // Verify that we had waiting connections during the test
  if (statsDuring.waiting === 0 && OVERLOAD_QUERIES > EXPECTED_MAX_CONNECTIONS) {
    console.log(chalk.yellow(`  ⚠ Warning: Expected waiting connections during load test, but got 0`));
    console.log(chalk.yellow(`    This might indicate the pool isn't enforcing max connections properly`));
  } else {
    console.log(chalk.gray(`  ✓ Pool had ${statsDuring.waiting} waiting connections during load test`));
  }
  
  // Check stats after test
  const statsAfter = await getPoolStats();
  
  console.log(chalk.gray(`  ✓ Pool stats during load test: active=${statsDuring.active}, waiting=${statsDuring.waiting}`));
  console.log(chalk.gray(`  ✓ Pool stats after load test: active=${statsAfter.active}, idle=${statsAfter.idle}, waiting=${statsAfter.waiting}`));
  console.log(chalk.gray(`  ✓ Load test completed in ${totalDuration.toFixed(2)}ms`));
  
  // Verify pool recovered properly
  if (statsAfter.waiting > 0) {
    throw new Error(`Pool still has ${statsAfter.waiting} waiting connections after test completion`);
  }
}
...
function printSummary() {
  ...
  console.log(`${chalk.green(`Passed: ${results.passed}`)} | ${chalk.red(`Failed: ${results.failed}`)}`);
