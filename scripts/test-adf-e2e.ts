...
import { adfLeads, emailDeliveryTracking } from "../shared/adf-schema";
import {
  dealerships,
...
function setupMockServices() {
  ...
  const originalAiServiceGetCompletion =
    aiService.getCompletion.bind(aiService);
  aiService.getCompletion = async (prompt: string, options: any = {}) => {
    logger.info("Mock AI service called", {
      prompt: prompt.substring(0, 100) + "...",
    });

    // Generate a mock response based on the prompt
    let responseText =
      "Thank you for your interest in the Honda Accord! I would be happy to provide more information.";
...
async function createTestDealership() {
  ...
  await db.insert(dealershipHandoverSettings).values({
...
async function waitForProcessingCompletion() {
  ...
  while (Date.now() - startWaitTime < maxWaitTime) {
    ...
    const pendingEvents = requiredEvents.filter(
      (event) => !eventTracker[event],
    );
  ...
  throw new Error(
    `Processing timed out after ${maxWaitTime}ms. Some events were not triggered.`,
  );
}
...
async function verifyDatabaseState() {
  ...
  console.assert(
    lead.externalId === `E2E-TEST-${TEST_CONFIG.testId}`,
    "Lead external ID mismatch",
  );
  console.assert(
    lead.customerName.includes(TEST_CONFIG.customerName.split(" ")[0]),
  ...
  logger.info(
    `Email delivery tracking found: ${emailDeliveries[0].deliveryStatus}`,
  );
  console.assert(
    emailDeliveries[0].recipientEmail === TEST_CONFIG.customerEmail,
  ...
  if (eventTracker["intent.ready_for_handover"]) {
    ...
    if (handover.dossier) {
      ...
      console.assert(
        typeof handover.dossier === "object",
        "Dossier is not an object",
      );
    }
  }
...
function verifyMetricsCollection() {
  ...
  if (eventTracker["intent.ready_for_handover"]) {
    ...
    if (missingHandoverMetrics.length > 0) {
      logger.warn(
        `Some handover metrics are missing: ${missingHandoverMetrics.join(", ")}`,
      );
    } else {
...
async function testErrorScenarioAndRecovery() {
  ...
  if (errorLeads.length > 0) {
    ...
    if (retryDeliveries.length > 0) {
      logger.info(
        `Retry delivery status: ${retryDeliveries[0].deliveryStatus}`,
      );
    }
  }
...
async function cleanupTestEnvironment() {
  ...
  if (process.env.CI !== "true" && testDealershipId) {
    ...
    // Delete dealership
    await db.delete(dealerships).where(eq(dealerships.id, testDealershipId));
  ...
  if (testDuration > TEST_TIMEOUT_MS / 1000) {
    logger.warn(
      `Test duration (${testDuration.toFixed(2)}s) exceeds target of ${TEST_TIMEOUT_MS / 1000}s`,
    );
  } else {
    logger.info(
      `Test duration (${testDuration.toFixed(2)}s) is within target of ${TEST_TIMEOUT_MS / 1000}s`,
    );
  }
}
