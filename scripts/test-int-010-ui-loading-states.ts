...
const CONFIG = {
  ...
  screenshotDir: join(process.cwd(), 'test-reports', 'ui-loading-states', 'screenshots'),
...
async function testBlankScreens(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    for (let time = 0; time <= maxTime; time += screenshotInterval) {
      ...
      // Take a screenshot
      const screenshotPath = join(CONFIG.screenshotDir, `${pageName.toLowerCase()}-${time}ms.png`);
      ...
      const hasContent = await page.evaluate(() => {
        ...
        const hasLoadingIndicators = document.querySelectorAll('[role="progressbar"], .skeleton-loader, .loading-overlay').length > 0;
      ...
      if (hasContent && firstContentTime === 0) {
        ...
        await page.evaluate(() => {
          ...
          window.performance.measure('blank-screen-time', 'navigation-start', 'first-content');
        });
      }
      ...
      if (firstContentTime > 0 && time - firstContentTime > CONFIG.maxBlankScreenTime) {
        break;
      }
    }
...
async function testSkeletonLoaders(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const skeletonStats = await page.evaluate((selectors) => {
      ...
      const conversation = document.querySelectorAll('.skeleton-conversation').length;
    ...
    if (hasSkeletonLoaders) {
      ...
      const hasLoadedContent = await page.evaluate(() => {
        ...
        return document.querySelectorAll('.conversation-table, .analytics-dashboard, .status-card').length > 0;
      });
    ...
    if (hasSkeletonLoaders) {
      const hasAnimations = await page.evaluate(() => {
        ...
        for (const skeleton of Array.from(skeletons)) {
          ...
          if (style.animation || style.animationName || skeleton.classList.contains('skeleton-wave') || 
              skeleton.classList.contains('skeleton-pulse') || skeleton.classList.contains('skeleton-shimmer')) {
            hasAnimation = true;
            break;
          }
        }
...
async function testReactSuspense(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const lazyLoadingStats = await page.evaluate(() => {
      ...
      const lazyLoadedComponents = document.querySelectorAll('[data-testid*="lazy-"], [id*="lazy-"]').length;
      
      // Check for any error boundaries that might have caught lazy loading errors
      const errorBoundaries = document.querySelectorAll('.error-boundary, [data-testid*="error-boundary"]').length;
      
      return {
        lazyLoadedComponents,
        errorBoundaries,
        viewportLazyLoads: document.querySelectorAll('[data-viewport-lazy-loaded="true"]').length
      };
    });
...
async function testLayoutShift(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const initialPositions = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('h1, h2, button, .card, .table'))
        .map(el => {
          const rect = el.getBoundingClientRect();
          return {
    ...
    for (const initial of initialPositions) {
      ...
      if (final) {
        ...
        const sizeChange = Math.abs((final.width * final.height) - (initial.width * initial.height));
        
        const shift = verticalShift + horizontalShift + sizeChange;
        if (shift > 5) { // Threshold for noticeable shift
          totalShift += shift;
          shiftDetails.push({
            element: initial.id,
            verticalShift,
    ...
    writeFileSync(join(CONFIG.screenshotDir, `${pageName.toLowerCase()}-initial.png`), initialScreenshot);
    writeFileSync(join(CONFIG.screenshotDir, `${pageName.toLowerCase()}-final.png`), finalScreenshot);
    ...
    if (totalShift < 50 && (!cls || cls < CONFIG.performanceThresholds.maxLayoutShift)) {
      addTestResult(
        testName,
        'pass',
...
async function testAccessibility(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const accessibilityResults = await page.evaluate((selectors) => {
      ...
      const progressIndicators = document.querySelectorAll(selectors.progressIndicator);
      results.progressbars = progressIndicators.length;
      
      for (const el of Array.from(progressIndicators)) {
        if (!el.getAttribute('aria-valuemin') && !el.getAttribute('aria-valuemax')) {
    ...
    if (accessibilityResults.ariaBusy > 0 || accessibilityResults.progressbars > 0) {
...
async function testPerformanceImpact(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    if (fcp > CONFIG.performanceThresholds.maxFirstContentfulPaint) {
      performanceIssues.push(`First Contentful Paint (${fcp.toFixed(2)}ms) exceeds threshold (${CONFIG.performanceThresholds.maxFirstContentfulPaint}ms)`);
    }
    
    if (tti > CONFIG.performanceThresholds.maxTimeToInteractive) {
      performanceIssues.push(`Time to Interactive (${tti.toFixed(2)}ms) exceeds threshold (${CONFIG.performanceThresholds.maxTimeToInteractive}ms)`);
    }
    
    if (cls > CONFIG.performanceThresholds.maxLayoutShift) {
      performanceIssues.push(`Cumulative Layout Shift (${cls.toFixed(3)}) exceeds threshold (${CONFIG.performanceThresholds.maxLayoutShift})`);
    }
...
async function testErrorHandling(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const errorHandlingResults = await page.evaluate(() => {
      ...
      const errorMessages = Array.from(document.querySelectorAll('.error, .error-message, [role="alert"]'))
        .map(el => el.textContent?.trim())
        .filter(Boolean);
      
      // Look for retry buttons
      const retryButtons = document.querySelectorAll('button:has-text("Retry"), button:has-text("Try Again")').length;
    ...
    if (errorHandlingResults.hasErrorMessage || errorHandlingResults.hasRetryButton) {
      addTestResult(
        testName,
        'pass',
      ...
      if (errorHandlingResults.hasRetryButton) {
        ...
        const errorGone = await page.evaluate(() => {
          return document.querySelectorAll('.error, .error-message, [role="alert"]').length === 0;
        });
...
async function testLoadingContext(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    await page.goto(`${CONFIG.baseUrl}${CONFIG.pages.find(p => p.name === pageName)?.path || '/'}`, { waitUntil: 'networkidle' });
    ...
    if (loadingEvents.length > 0) {
      ...
      const unstoppedKeys = [...loadingKeys].filter(key => !stoppedKeys.has(key));
      ...
      if (unstoppedKeys.length === 0) {
        addTestResult(
          ...
          { startEvents: startEvents.length, stopEvents: stopEvents.length, loadingKeys: loadingKeys.size }
        );
      } else {
        addTestResult(
          testName,
          ...
          { startEvents: startEvents.length, stopEvents: stopEvents.length, unstoppedKeys }
        );
      }
    } else {
      addTestResult(
...
async function testViewportLazyLoading(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const initialLazyComponents = await page.evaluate(() => {
      ...
      return {
        viewportLazyLoad: document.querySelectorAll('.viewport-lazy-load, [data-viewport-lazy]').length,
        lazyLoadFallback: document.querySelectorAll('[data-testid="lazy-fallback"]').length
      };
    });
    ...
    const afterScrollLazyComponents = await page.evaluate(() => {
      return {
        viewportLazyLoad: document.querySelectorAll('.viewport-lazy-load, [data-viewport-lazy]').length,
        lazyLoadFallback: document.querySelectorAll('[data-testid="lazy-fallback"]').length,
        lazyLoaded: document.querySelectorAll('[data-viewport-loaded="true"]').length
      };
    });
    
    if (initialLazyComponents.viewportLazyLoad > 0 || 
...
async function testLoadingStatePersistence(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      if (loadingContext && loadingContext.startLoading) {
        ...
        loadingContext.startLoading('persistent:test-loading', 'Testing persistent loading state');
      }
    });
    ...
    if (otherPage) {
      await page.goto(`${CONFIG.baseUrl}${otherPage.path}`, { waitUntil: 'networkidle' });
...
async function testResponsiveBehavior(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    // Save screenshots
    writeFileSync(join(CONFIG.screenshotDir, `${pageName.toLowerCase()}-desktop.png`), desktopScreenshot);
    writeFileSync(join(CONFIG.screenshotDir, `${pageName.toLowerCase()}-mobile.png`), mobileScreenshot);
    ...
    if (responsiveResults.totalSkeletons > 0) {
      const responsivePercentage = (responsiveResults.responsiveSkeletons / responsiveResults.totalSkeletons) * 100;
    ...
    } else if (responsiveResults.hasOverlay && responsiveResults.overlayIsResponsive) {
      addTestResult(
        testName,
        'pass',
...
async function testAnalyticsTracking(page: Page, pageName: string): Promise<void> {
  ...
  try {
    ...
    const loadingEvents = analyticsEvents.filter(event => 
    ...
    if (loadingEvents.length > 0) {
      addTestResult(
        ...
        { totalEvents: analyticsEvents.length, loadingEvents: loadingEvents.length }
      );
    } else {
      addTestResult(
        testName,
    ...
    for (const pageConfig of CONFIG.pages) {
      ...
      try {
        ...
        await context.tracing.stop({
          path: join(CONFIG.traceDir, `${pageConfig.name.toLowerCase()}-trace.zip`),
        });
      } catch (error) {
