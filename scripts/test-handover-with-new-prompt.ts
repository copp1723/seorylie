...
import {
  createHandoverDossier,
  sendHandoverDossierEmail,
...
async function createTestData() {
  ...
  await db.insert(messages).values([
    {
      ...
      content:
        "Hi, I'm interested in buying a new SUV for my family. We need something spacious with good safety features.",
      isFromCustomer: true,
    ...
    },
    {
      conversationId: conversation.id,
      content:
        "Hello! Thank you for reaching out to Test Dealership. I'd be happy to help you find the perfect SUV for your family. We have several models with excellent safety ratings and spacious interiors. Could you tell me more about what features are most important to you?",
    ...
    },
    {
      conversationId: conversation.id,
      content:
        "Safety is our top priority since we have two young kids. We also need good cargo space for road trips. Our budget is around $40,000, but we might go a bit higher for the right vehicle. Can you recommend something?",
      isFromCustomer: true,
      channel: "chat", // Add the required channel field
    },
    {
    ...
    },
    {
      conversationId: conversation.id,
      content:
        "Those sound promising! What about fuel efficiency? We do a lot of driving, so that's important too. And do they have third-row seating?",
      isFromCustomer: true,
      channel: "chat", // Add the required channel field
    },
  ]);
  ...
  console.log(
    `Test data created successfully! Conversation ID: ${conversation.id}`,
  );
  return conversation;
}
...
async function testHandover() {
  try {
    ...
    const dossier = await createHandoverDossier({
      ...
      escalationReason:
        "Customer has specific questions that require expert knowledge",
    });
    ...
    if (sendEmail.toLowerCase() === "y") {
      ...
      if (success) {
        ...
        console.log(
          "Failed to send handover email. Check logs for more details.",
        );
      }
    }
