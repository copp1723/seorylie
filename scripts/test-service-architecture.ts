...
async function testServiceArchitecture() {
  ...
  try {
    ...
    console.log(`   Registered services: ${serviceRegistry.getServiceNames().join(', ')}`);
    ...
    try {
      ...
      console.log('⚠️ Some services failed to initialize (expected in test environment)');
    ...
    try {
      ...
      // Show service health
      console.log('\n   Service health:');
      Object.entries(registryHealth.services).forEach(([serviceName, health]) => {
    ...
    try {
      ...
      console.log(`   Auth Service - Requests: ${authMetrics.requestCount}, Errors: ${authMetrics.errorCount}`);
      console.log(`   Auth Service - Avg Response Time: ${authMetrics.averageResponseTime.toFixed(2)}ms`);
      
      const healthMetrics = healthCheckService.getMetrics();
      console.log(`   Health Service - Requests: ${healthMetrics.requestCount}, Errors: ${healthMetrics.errorCount}`);
      console.log(`   Health Service - Avg Response Time: ${healthMetrics.averageResponseTime.toFixed(2)}ms`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`   Authenticated connections: ${wsStats.authenticatedConnections}`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`   Agent Squad enabled: ${featuresConfig.agentSquadEnabled}`);
    } catch (error) {
