...
class GrafanaValidator {
   ...
   */
  private validateADFDashboard(dashboard: GrafanaDashboard, result: ValidationResult): void {
    // Check for ADF tags
    if (dashboard.tags?.includes('adf')) {
      ...
      dashboard.panels.forEach(panel => {
        panel.targets?.forEach(target => {
          if (target.expr && adfMetrics.some(metric => target.expr!.includes(metric))) {
            hasADFMetrics = true;
          }
        });
      });
      ...
      dashboard.panels.forEach(panel => {
        panel.targets?.forEach(target => {
          if (target.datasource?.type !== 'prometheus') {
            result.warnings.push(`Panel "${panel.title}" should use Prometheus datasource for ADF metrics`);
          }
        });
      });
    }
   ...
   */
  private validateBestPractices(dashboard: GrafanaDashboard, result: ValidationResult): void {
    ...
    if (dashboard.refresh && !['5s', '10s', '30s', '1m', '5m', '15m', '30m', '1h'].includes(dashboard.refresh)) {
      result.warnings.push(`Unusual refresh interval: ${dashboard.refresh}`);
    }

    // Check time range
    if (dashboard.time) {
      const validTimeRanges = ['now-1h', 'now-6h', 'now-24h', 'now-7d', 'now-30d'];
      if (!validTimeRanges.includes(dashboard.time.from)) {
        result.warnings.push(`Consider using standard time range instead of: ${dashboard.time.from}`);
      }
    }
  }
