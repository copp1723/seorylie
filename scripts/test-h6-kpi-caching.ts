...
const logger = {
  ...
  debug: (message: string, context?: any) => console.debug(`[DEBUG] ${new Date().toISOString()} ${message}`, context || ''),
  group: (label: string) => console.group(label),
  groupEnd: () => console.groupEnd(),
  logMetric: (metricName: string, value: any, unit?: string) => console.log(`[METRIC] ${metricName}: ${value}${unit ? ' ' + unit : ''}`),
};
...
if (REDIS_URL) {
  try {
    ...
    redisClient.on('error', (err) => logger.error('Redis Client Error (background)', err));
  } catch (e) {
...
interface TestMetrics {
  ...
  failed: number;
  skipped: number;
  suiteMetrics: Record<string, { name: string; total: number; passed: number; failed: number; skipped: number; durationMs: number }>;
}
const testMetrics: TestMetrics = { totalTests: 0, passed: 0, failed: 0, skipped: 0, suiteMetrics: {} };
...
async function startSuite(suiteName: string) {
  ...
  logger.group(`\n🧪 SUITE: ${suiteName}`);
  testMetrics.suiteMetrics[suiteName] = { name: suiteName, total: 0, passed: 0, failed: 0, skipped: 0, durationMs: 0 };
}
...
function endSuite() {
  ...
  if (suite) {
    ...
    logger.info(`Suite ${currentSuiteName} Summary: Passed: ${suite.passed}, Failed: ${suite.failed}, Skipped: ${suite.skipped} (Total: ${suite.total})`);
  }
  logger.groupEnd();
  currentSuiteName = '';
}

async function runTest<T>(testName: string, testFn: () => Promise<T>, skip = false): Promise<T | undefined> {
  const suite = testMetrics.suiteMetrics[currentSuiteName];
  suite.total++;
...
async function getKpi(endpoint: string, params?: Record<string, any>): Promise<{ data: any; duration: number }> {
  const startTime = Date.now();
  try {
    ...
    const duration = Date.now() - startTime;
    logger.error(`Failed to get KPI: ${endpoint}`, { params, error: e.message, duration });
    throw e;
  }
}
...
async function getCacheStatsAPI(): Promise<any> {
  try {
    ...
    logger.warn('Failed to get /kpi/cache/stats, trying /performance/cache/stats', kpiError);
    try {
...
async function testPerformanceAndTTL() {
  ...
  await runTest('Initial fetch (cache miss)', async () => {
    ...
    assert(typeof data.totalConversations === 'number', 'Expected totalConversations');
  });

  await runTest('Second fetch (cache hit, sub-50ms)', async () => {
    const { duration, data } = await getKpi(kpiEndpoint, params);
    logger.logMetric('Cached fetch duration', duration, 'ms');
    assert(duration < SUB_50MS_THRESHOLD, `Cached response should be < ${SUB_50MS_THRESHOLD}ms, got ${duration}ms`);
    assert(data !== null, 'Data should not be null on cached fetch');
  });

  await runTest(`Wait for TTL expiry (approx ${KPI_CACHE_TTL_SECONDS + 5}s)`, async () => {
    logger.info(`Waiting for ${KPI_CACHE_TTL_SECONDS + 5} seconds...`);
    await sleep((KPI_CACHE_TTL_SECONDS + 5) * 1000);
  });

  await runTest('Fetch after TTL (cache miss or background refresh)', async () => {
    const { duration, data } = await getKpi(kpiEndpoint, params);
  ...
  await runTest('Fetch again (should be cached, sub-50ms)', async () => {
    ...
    assert(duration < SUB_50MS_THRESHOLD, `Second cached response should be < ${SUB_50MS_THRESHOLD}ms, got ${duration}ms`);
...
async function testETLEventInvalidation() {
  ...
  const etlEventPayload = { source: 'final_watchdog', event: 'kpi_data_updated' }; // This should invalidate all KPIs
  ...
  await runTest('Cache a KPI', async () => {
    ...
    assert(duration < SUB_50MS_THRESHOLD, `KPI should be cached, got ${duration}ms`);
  });

  await runTest('Simulate final_watchdog ETL event (kpi_data_updated)', async () => {
  ...
  await runTest('Fetch KPI after ETL event (should be a cache miss)', async () => {
  ...
  await runTest('Simulate specific ETL event (leads_updated)', async () => {
    ...
    const specificEtlPayload = { source: 'final_watchdog', event: 'leads_updated' };
...
async function testBackgroundRefresh() {
  ...
  await runTest('Cache a KPI with potential background refresh', async () => {
    await clearAllCacheAPI();
    const { data: initialData, duration: initialDuration } = await getKpi(kpiEndpoint, params);
    logger.logMetric('Initial fetch for background refresh test', initialDuration, 'ms');
  ...
  await runTest(`Wait for a period less than TTL but enough for background refresh trigger (e.g., ${KPI_CACHE_TTL_SECONDS * 0.6}s)`, async () => {
    const waitTime = KPI_CACHE_TTL_SECONDS * 0.6; // e.g., 18s for 30s TTL
    logger.info(`Waiting for ${waitTime} seconds for background refresh to potentially trigger...`);
    await sleep(waitTime * 1000);
  });

  let dataAfterPotentialRefresh: any;
  await runTest('Fetch again, expecting fast response (from cache, possibly old or bg refreshed)', async () => {
    const { data, duration } = await getKpi(kpiEndpoint, params);
    logger.logMetric('Fetch after partial TTL (background refresh check)', duration, 'ms');
    assert(duration < SUB_50MS_THRESHOLD * 2, `Should still be relatively fast, got ${duration}ms`); // Allow a bit more leeway
    assert(data !== null, 'Data should exist');
    dataAfterPotentialRefresh = data;
  });

  await runTest(`Wait beyond TTL (e.g., ${KPI_CACHE_TTL_SECONDS + 5}s total) to ensure original item would have expired`, async () => {
  ...
  await runTest('Fetch after full TTL cycle, check if data was refreshed', async () => {
    const { data: finalData, duration } = await getKpi(kpiEndpoint, params);
    logger.logMetric('Fetch after full TTL (background refresh validation)', duration, 'ms');
    ...
    if (finalData.lastUpdated && dataAfterPotentialRefresh.lastUpdated) {
      assert(new Date(finalData.lastUpdated) >= new Date(dataAfterPotentialRefresh.lastUpdated), 'Data should have been refreshed by background process or new fetch.');
      ...
      logger.warn('Cannot validate data freshness due to missing lastUpdated fields.');
    }
  });
...
async function testCacheWarming() {
  ...
  await runTest('Clear cache and register KPI for warming', async () => {
    await clearAllCacheAPI();
    try {
      await apiClient.post(`/kpi/cache/warm/${kpiName}`, null, { params: { dealershipId: DEFAULT_DEALERSHIP_ID, period: 'test_warm' } });
      logger.info(`KPI ${kpiName} registered for warming.`);
    } catch (e) {
      logger.warn(`Failed to register KPI for warming via API, test might be limited.`, e);
    }
  });

  await runTest('Wait for cache warming interval (e.g., 65s for 60s interval)', async () => {
    const warmingInterval = 60; // Assume 60s from CacheService config
    logger.info(`Waiting for ${warmingInterval + 5} seconds for cache warmer to run...`);
  ...
  await runTest('Fetch KPI, expecting it to be cached by warmer', async () => {
    ...
    assert(duration < SUB_50MS_THRESHOLD * 2, `Should be reasonably fast if warmed, got ${duration}ms`);
    
    if (redisClient) {
        const existsInRedis = await redisClient.exists(cacheKeyForWarming);
        assert(existsInRedis > 0, `Key ${cacheKeyForWarming} should exist in Redis if warmed.`);
...
async function testTagBasedInvalidation() {
  ...
  const paramsDept1 = { dealershipId: DEFAULT_DEALERSHIP_ID, period: 'tag_test' };
  const paramsDept2 = { dealershipId: DEFAULT_DEALERSHIP_ID + 1, period: 'tag_test' };
  ...
  await runTest('Verify KPIs for dealership 1 are invalidated', async () => {
    ...
    logger.logMetric(`Fetch KPI 1 (Dept1) after tag invalidation`, res.duration, 'ms');
    ...
    logger.logMetric(`Fetch KPI 2 (Dept1) after tag invalidation`, res.duration, 'ms');
  ...
  await runTest('Verify KPI for dealership 2 is NOT invalidated', async () => {
    ...
    logger.logMetric(`Fetch KPI 1 (Dept2) after tag invalidation (should be hit)`, res.duration, 'ms');
    assert(res.duration < SUB_50MS_THRESHOLD, 'KPI 1 (Dept2) should still be cached');
  });
...
async function testAccuracyAndConsistency() {
  ...
  const kpiEndpoint = '/kpi/conversations/summary';
  const params = { dealershipId: DEFAULT_DEALERSHIP_ID, period: 'accuracy_test' };
  const compareEndpoint = `/kpi/conversations/summary/compare`; // From kpi-routes.ts

  await runTest('Fetch cached vs live data using compare endpoint', async () => {
    await clearAllCacheAPI();
    const { data } = await apiClient.get(compareEndpoint, { params });
    assert(data.success === true || data.cached, 'Compare endpoint should return data'); // Adapt to actual response
    const { cached, live } = data.data || data;

    assert(cached !== null && live !== null, 'Both cached and live data should exist');
    assert(typeof cached.totalConversations === 'number', 'Cached data structure error');
    assert(typeof live.totalConversations === 'number', 'Live data structure error');
    ...
    if (cached.lastUpdated && live.lastUpdated) {
        assert(new Date(live.lastUpdated) >= new Date(cached.lastUpdated), 'Live data should be at least as fresh as cached data.');
    }
...
async function testMonitoringIntegration() {
  ...
  const params = { dealershipId: DEFAULT_DEALERSHIP_ID, period: 'monitoring_test' };
  ...
  await runTest('Get initial cache stats', async () => {
    ...
    logger.debug('Initial Cache Stats:', initialStats.kpiQueries || initialStats);
  });
  ...
  await runTest('Get updated cache stats and verify increments', async () => {
    ...
    logger.debug('Updated Cache Stats:', updatedStats.kpiQueries || updatedStats);
    ...
    assert(updatedKpiStats.total >= initialKpiStats.total + 1, `KPI total queries should increment. Got ${updatedKpiStats.total}, expected >= ${initialKpiStats.total + 1}`);
    assert(updatedKpiStats.cached >= initialKpiStats.cached + 2, `KPI cached queries should increment by 2. Got ${updatedKpiStats.cached}, expected >= ${initialKpiStats.cached + 2}`);
    
    logger.logMetric('Cache Hits (from stats)', updatedStats.hits, '');
    logger.logMetric('Cache Misses (from stats)', updatedStats.misses, '');
    logger.logMetric('KPI Queries Total (from stats)', updatedKpiStats.total, '');
    logger.logMetric('KPI Queries Cached (from stats)', updatedKpiStats.cached, '');
    logger.logMetric('KPI Avg Response Time (from stats)', updatedKpiStats.avgResponseTime, 'ms');
    logger.logMetric('KPI Sub-50ms % (from stats)', updatedKpiStats.sub50msPercentage, '%');
...
async function testErrorHandling() {
  ...
  await runTest('KPI factory function error', async () => {
    try {
      ...
      assert(axiosError.response?.status === 500 || axiosError.response?.status === 503, `Expected 500/503 status, got ${axiosError.response?.status}`);
      logger.info('Correctly handled KPI factory error.', { status: axiosError.response?.status, data: axiosError.response?.data });
    }
  }, true); // Skipping for now as endpoint doesn't exist

  // Test case: Redis down (if Redis is configured)
  await runTest('Redis unavailable (manual test if applicable)', async () => {
    ...
    logger.info('MANUAL STEP: Temporarily stop Redis server if you want to test this.');
  ...
  }, true); // Skipping as it's a manual/disruptive test
...
async function testConcurrentRequests() {
  ...
  const params = { dealershipId: DEFAULT_DEALERSHIP_ID, period: 'concurrent_test' };

  await runTest('Multiple concurrent requests for an uncached KPI', async () => {
    await clearAllCacheAPI();
    const numRequests = 10;
    const promises = [];
    logger.info(`Sending ${numRequests} concurrent requests to ${kpiEndpoint}`);
    ...
    results.forEach((res, i) => {
      assert(res.data !== null, `Concurrent request ${i + 1} data should not be null`);
      if (i === 0) {
        firstData = res.data;
      } else {
        // Compare structure or key fields. Exact match if factory is deterministic and no data changes.
        assert(JSON.stringify(res.data) === JSON.stringify(firstData), `Data from concurrent request ${i + 1} should match first request`);
      }
      logger.logMetric(`Concurrent request ${i+1} duration`, res.duration, 'ms');
    });
...
async function main() {
  ...
  if (REDIS_URL && redisClient) {
    try {
        ...
        logger.error('Initial Redis connection failed. Some tests might behave differently.', e);
        if (redisClient) await redisClient.disconnect();
        redisClient = null; // Ensure it's null if connection failed
    }
  } else {
    logger.warn('Redis URL not configured or client init failed. Tests will rely on in-memory cache behavior.');
  }
  ...
  try {
    ...
    logger.error('Critical error during test execution, stopping script.', { message: e.message, stack: e.stack });
    testMetrics.failed++; // Count this as a general failure
  } finally {
    if (redisClient) {
      await redisClient.quit().catch(e => logger.error('Error quitting Redis client', e));
    }
    ...
    Object.values(testMetrics.suiteMetrics).forEach(suite => {
        logger.info(`SUITE: ${suite.name} | Total: ${suite.total}, Passed: ${suite.passed}, Failed: ${suite.failed}, Skipped: ${suite.skipped} | Duration: ${suite.durationMs}ms`);
    });
    logger.info('----------------------------------------------------');
    logger.info(`OVERALL TOTAL: ${testMetrics.totalTests}, PASSED: ${testMetrics.passed}, FAILED: ${testMetrics.failed}, SKIPPED: ${testMetrics.skipped}`);
    logger.info('====================================================');

    if (testMetrics.failed > 0) {
      logger.error('🔴 Some tests failed!');
      process.exit(1);
    } else if (testMetrics.passed === 0 && testMetrics.totalTests > 0 && testMetrics.skipped === testMetrics.totalTests) {
