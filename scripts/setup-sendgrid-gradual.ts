...
class SendGridMigrationSetup {
  ...
  async run() {
    ...
    console.log('You can migrate gradually without affecting your current email processing.\n');
  ...
  private async gatherConfiguration() {
    ...
    // Webhook Secret
    const secret = await this.promptUser('Enter webhook verification secret (or press Enter to generate): ');
    ...
    this.config.webhookUrl = await this.promptUser(`Webhook URL (${defaultUrl}): `) || defaultUrl;
...
`;
    ...
    if (envContent.includes('SENDGRID_API_KEY')) {
      ...
      const update = await this.promptUser('Update existing configuration? (y/n): ');
  ...
  private updateExistingEnvConfig(envContent: string, newConfig: string): string {
...
`;
    ...
    const planPath = path.join(process.cwd(), 'docs', 'sendgrid-migration-progress.md');
  ...
  private async setupWebhookEndpoint() {
    ...
    const webhookRoutePath = path.join(process.cwd(), 'server', 'routes', 'sendgrid-webhook-routes.ts');
    
    if (fs.existsSync(webhookRoutePath)) {
      console.log('✅ Webhook route already exists');
    } else {
      console.log('⚠️  Webhook route not found - you may need to add it to your server');
    }
    ...
    if (fs.existsSync(serverIndexPath)) {
      ...
      if (!serverContent.includes('sendgrid-webhook-routes')) {
        ...
        console.log('');
        console.log('import sendgridRoutes from \'./routes/sendgrid-webhook-routes\';');
