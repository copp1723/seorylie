...
async function seedDatabase() {
  ...
  try {
    ...
    await db.insert(personas).values({
      ...
      promptTemplate:
        "Be friendly, approachable and helpful. Focus on {priorityFeatures}. Use a {tone} tone.",
      arguments: {
    ...
    await db.insert(personas).values({
      ...
      promptTemplate:
        "Provide detailed technical information. Focus on {priorityFeatures}. Use a {tone} tone.",
      arguments: {
    ...
    await db.insert(personas).values({
      ...
      name: "Concierge",
      promptTemplate:
        "Offer a premium concierge experience. Focus on {priorityFeatures}. Use a {tone} tone.",
      arguments: {
    ...
    const createdKeys = await db
      ...
      .from(apiKeys)
      .innerJoin(dealerships, (join) =>
        join.on(apiKeys.dealershipId, dealerships.id),
      );
