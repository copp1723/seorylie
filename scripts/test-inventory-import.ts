...
async function testInventoryImport() {
  ...
  const dealershipIndex =
    parseInt(
      await prompt("\nSelect dealership to import inventory for (number): "),
    ) - 1;
  ...
  const useExistingFile =
    (
      await prompt("\nUse existing inventory TSV file? (y/n): ")
    ).toLowerCase() === "y";
  ...
  const confirmImport =
    (
      await prompt(
        `\nImport inventory from ${tsvFilePath} for ${selectedDealership.name}? (y/n): `,
      )
    ).toLowerCase() === "y";
  ...
  const currentVehicles = await db
    .select()
    .from(vehicles)
    .where(eq(vehicles.dealershipId, selectedDealership.id));
  console.log(
    `Current vehicle count for dealership: ${currentVehicles.length}`,
  );
  ...
  fs.createReadStream(tsvFilePath)
    ...
    .on("end", async () => {
        ...
        for (const vehicle of batch) {
          try {
            ...
            if (existingVehicle.length > 0) {
              ...
              console.log(
                `Updated vehicle: ${vehicle.year} ${vehicle.make} ${vehicle.model} (${vehicle.vin})`,
              );
            } else {
              // Insert the vehicle
              ...
              console.log(
                `Imported vehicle: ${vehicle.year} ${vehicle.make} ${vehicle.model} (${vehicle.vin})`,
              );
            }
      ...
      const updatedVehicles = await db
        ...
        .where(eq(vehicles.dealershipId, selectedDealership.id));
      console.log(
        `New vehicle count for dealership: ${updatedVehicles.length}`,
      );
