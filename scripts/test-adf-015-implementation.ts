...
const utils = {
  ...
  test: async (name: string, category: string, fn: () => Promise<void>) => {
    results.total++;
  ...
  cleanupTestData: async () => {
    ...
    await db.execute(
      `DELETE FROM conversations WHERE dealership_id IN ($1, $2)`,
      [CONFIG.testDealershipId, CONFIG.testDealershipId2],
    );

    await db.execute(
      `DELETE FROM adf_sms_responses WHERE dealership_id IN ($1, $2)`,
      [CONFIG.testDealershipId, CONFIG.testDealershipId2],
    );

    await db.execute(`DELETE FROM adf_leads WHERE dealership_id IN ($1, $2)`, [
      CONFIG.testDealershipId,
      CONFIG.testDealershipId2,
    ]);
...
async function runTests() {
  try {
    ...
    await utils.test("Apply migration", "Database", async () => {
      ...
      try {
        ...
        if (columnResult.length === 0) {
          throw new Error(
            "Migration failed: adf_lead_id column not found in conversations table",
          );
        }
        ...
        if (tableResult.length === 0) {
          throw new Error(
            "Migration failed: conversation_events table not found",
          );
        }
    ...
    await utils.test("Generate mock data", "Setup", async () => {
      ...
      if (
        !testData ||
        !testData.conversationIds ||
        testData.conversationIds.length === 0
      ) {
    ...
    const authToken = await utils.createAuthToken(
      CONFIG.testUserId,
      CONFIG.testDealershipId,
    );
    ...
    await utils.test("GET /api/adf/conversations", "API", async () => {
      const response = await axios.get(
        `${CONFIG.apiBaseUrl}/adf/conversations`,
        {
      ...
      if (incorrectDealership) {
        throw new Error(
          "Response contains conversations from incorrect dealership",
        );
      }
    ...
    await utils.test(
      "GET /api/adf/conversations with filtering",
      "API",
      async () => {
        // Test status filter
        const statusResponse = await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations?status=active`,
          {
            headers: { Authorization: authToken },
          },
        );
        ...
        if (
          !statusData.conversations.every((c: any) => c.status === "active")
        ) {
        ...
        const channelResponse = await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations?channel=sms`,
          {
            headers: { Authorization: authToken },
          },
        );
    ...
    await utils.test("GET /api/adf/conversations/:id", "API", async () => {
      ...
      const response = await axios.get(
        `${CONFIG.apiBaseUrl}/adf/conversations/${conversationId}`,
        {
          headers: { Authorization: authToken },
        },
      );
    ...
    await utils.test(
      "GET /api/adf/conversations/:id/messages",
      "API",
      async () => {
        if (!testData || !testData.conversationIds) {
          throw new Error("Test data not available");
        ...
        const conversationId = testData.conversationIds[0];
        const response = await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations/${conversationId}/messages`,
          {
            headers: { Authorization: authToken },
          },
        );
        ...
        if (incorrectConversation) {
          throw new Error(
            "Response contains messages from incorrect conversation",
          );
        }
    ...
    await utils.test(
      "GET /api/adf/conversations/:id/lead-context",
      "API",
      async () => {
        if (!testData || !testData.conversationIds) {
          throw new Error("Test data not available");
        ...
        const conversationId = testData.conversationIds[0];
        const response = await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations/${conversationId}/lead-context`,
          {
            headers: { Authorization: authToken },
          },
        );
    ...
    await utils.test(
      "POST /api/adf/conversations/:id/status",
      "API",
      async () => {
        if (!testData || !testData.conversationIds) {
          throw new Error("Test data not available");
    ...
    await utils.test("GET /api/adf/conversations/stats", "API", async () => {
      const response = await axios.get(
        `${CONFIG.apiBaseUrl}/adf/conversations/stats`,
        {
          headers: { Authorization: authToken },
        },
      );
    ...
    await utils.test(
      "WebSocket connection and channel subscription",
      "WebSocket",
      async () => {
        return new Promise<void>((resolve, reject) => {
          const ws = new WebSocket(
            `${CONFIG.wsBaseUrl}/dealership/${CONFIG.testDealershipId}/conversations`,
          );
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        ws.on("open", async () => {
          ...
          try {
            ...
            ws.close();
            reject(
              new Error(`Failed to trigger conversation update: ${error}`),
            );
          }
        });
    ...
    await utils.test(
      ...
      async () => {
        ...
        const page1Response = await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations?limit=2&page=1`,
          {
            headers: { Authorization: authToken },
          },
        );
        ...
        const page2Response = await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations?limit=2&page=2`,
          {
            headers: { Authorization: authToken },
          },
        );
        ...
        if (page1Data.conversations[0].id === page2Data.conversations[0].id) {
          throw new Error(
            "Pagination not working correctly - same conversations on different pages",
          );
        }
      },
    );
    ...
    await utils.test(
      ...
      async () => {
        ...
        if (
          page1Data.messages.length > 0 &&
          page2Data.messages.length > 0 &&
          page1Data.messages[0].id === page2Data.messages[0].id
        ) {
          throw new Error(
            "Cursor pagination not working correctly - same messages on different pages",
          );
        }
      },
    );
    ...
    await utils.test("Search functionality", "Filtering", async () => {
      ...
      if (!data.conversations || data.conversations.length === 0) {
        throw new Error(
          "Search functionality not working correctly - no results found",
        );
      }
    });
    ...
    await utils.test(
      "Dealership isolation for conversations",
      "Security",
      async () => {
        // Create auth token for second dealership
        const dealership2Token = await utils.createAuthToken(
          CONFIG.testUserId,
          CONFIG.testDealershipId2,
        );
    ...
    await utils.test(
      "Dealership isolation for conversation details",
      "Security",
      async () => {
        if (!testData || !testData.conversationIds) {
          throw new Error("Test data not available");
        ...
        try {
          await axios.get(
            `${CONFIG.apiBaseUrl}/adf/conversations/${dealership1ConversationId}`,
            {
              headers: { Authorization: dealership2Token },
            },
          );
          ...
          throw new Error(
            "Dealership isolation not working correctly - able to access conversation from different dealership",
          );
        } catch (error) {
          if (axios.isAxiosError(error) && error.response) {
            ...
            if (
              ...
              throw new Error(
                `Expected 403 or 404 status code, got ${error.response.status}`,
              );
            }
          } else {
    ...
    await utils.test("Invalid conversation ID", "Error Handling", async () => {
      try {
        ...
        // If we get here, the request succeeded when it should have failed
        throw new Error(
          "Error handling not working correctly - invalid conversation ID should return error",
        );
      } catch (error) {
        if (axios.isAxiosError(error) && error.response) {
          if (error.response.status !== 404) {
            throw new Error(
              `Expected 404 status code, got ${error.response.status}`,
            );
          }
    ...
    await utils.test("Invalid query parameters", "Error Handling", async () => {
      try {
        await axios.get(
          `${CONFIG.apiBaseUrl}/adf/conversations?limit=invalid`,
          {
            headers: { Authorization: authToken },
          },
        );
        ...
        // If we get here, the request succeeded when it should have failed
        throw new Error(
          "Error handling not working correctly - invalid parameters should return error",
        );
      } catch (error) {
        if (axios.isAxiosError(error) && error.response) {
          if (error.response.status !== 400) {
            throw new Error(
              `Expected 400 status code, got ${error.response.status}`,
            );
          }
    ...
    await utils.test("Unauthorized access", "Error Handling", async () => {
      try {
        ...
        // If we get here, the request succeeded when it should have failed
        throw new Error(
          "Error handling not working correctly - unauthorized access should return error",
        );
      } catch (error) {
        if (axios.isAxiosError(error) && error.response) {
          if (error.response.status !== 401 && error.response.status !== 403) {
            throw new Error(
              `Expected 401 or 403 status code, got ${error.response.status}`,
            );
          }
        } else {
          throw error; // Re-throw unexpected errors
    ...
    await utils.test("TypeScript compilation", "TypeScript", async () => {
      const result = await execAsync(
        "npx tsc --noEmit server/routes/adf-conversation-routes.ts",
      );
    ...
    if (process.env.CI) {
      ...
      await utils.test(
        ...
        async () => {
          ...
          const ChatMessage =
            require("../client/src/components/chat-message").ChatMessage;
          ...
          if (!container.innerHTML.includes("Test message")) {
            throw new Error(
              "ChatMessage component did not render message text",
            );
          }
        },
      );
    ...
    await utils.test("Rollback migration", "Cleanup", async () => {
      ...
      try {
        ...
        if (columnResult.length > 0) {
          throw new Error(
            "Rollback failed: adf_lead_id column still exists in conversations table",
          );
        }
        ...
        if (tableResult.length > 0) {
          throw new Error(
            "Rollback failed: conversation_events table still exists",
          );
        }
      } finally {
        client.release();
