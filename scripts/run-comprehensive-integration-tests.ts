...
async function main() {
  ...
  try {
    ...
    console.log(chalk.blue.bold('🔍 INT-013 - Comprehensive Integration Testing'));
    ...
    console.log(chalk.blue(`⏱️  Total Duration: ${duration.toFixed(2)} seconds`));
...
async function runPhase(phase: string, fn: () => Promise<void>) {
  ...
  try {
    await fn();
    result.success = true;
    console.log(chalk.green(`✅ Phase ${phase.toUpperCase()} completed successfully`));
  } catch (error) {
    result.success = false;
    const errorMessage = error instanceof Error ? error.message : String(error);
    result.errors.push(errorMessage);
    console.error(chalk.red(`❌ Phase ${phase.toUpperCase()} failed: ${errorMessage}`));
...
function setupDirectories() {
  [TEST_RESULTS_DIR, COVERAGE_DIR, ARTIFACTS_DIR, LOG_DIR, REPORT_DIR].forEach(dir => {
...
async function validateEnvironment() {
  ...
  if (freeMemory < 1024 * 1024 * 1024) { // 1GB
    console.warn(chalk.yellow('⚠️ Warning: Less than 1GB of free memory available'));
  }
...
async function runUnitTests() {
  ...
  const { stdout, stderr } = await execAsync('npm run test:unit -- --coverage', { timeout: LONG_TIMEOUT });
  
  // Check for test failures
  if (stderr && stderr.includes('FAIL ')) {
    const failedTests = stderr.split('\n')
  ...
  if (coverageSummary.lines < 80) {
    console.warn(chalk.yellow(`⚠️ Warning: Line coverage (${coverageSummary.lines}%) is below 80% threshold`));
  }
  ...
  console.log(chalk.green(`✅ Unit tests passed with ${coverageSummary.lines}% line coverage`));
...
async function runIntegrationTests() {
  ...
  const { stdout, stderr } = await execAsync('npm run test:integration', { timeout: LONG_TIMEOUT });
...
async function runComprehensivePlatformTest() {
  ...
  try {
    ...
    const { stdout, stderr } = await execAsync('npx jest test/integration/comprehensive-platform-test.ts --runInBand', 
      { timeout: LONG_TIMEOUT * 2 }); // Double timeout for this intensive test
...
async function runE2ETests() {
  ...
  const { stdout, stderr } = await execAsync('npx playwright test', { timeout: LONG_TIMEOUT });
  ...
  if (fs.existsSync('playwright-report')) {
    fs.cpSync('playwright-report', path.join(ARTIFACTS_DIR, 'playwright-report'), { recursive: true });
  }
...
async function runLoadTests() {
  ...
  for (const script of loadTestScripts) {
    ...
    try {
      const { stdout, stderr } = await execAsync(`k6 run ${script} --summary-export=${path.join(TEST_RESULTS_DIR, `k6-${path.basename(script, '.js')}.json`)}`, 
      ...
      if (results.http_req_duration.p95 > PERFORMANCE_SLA.responseTime) {
        console.warn(chalk.yellow(`⚠️ Warning: p95 response time (${results.http_req_duration.p95}ms) exceeds SLA (${PERFORMANCE_SLA.responseTime}ms)`));
      }
      
      if (results.http_req_failed.rate > PERFORMANCE_SLA.errorRate) {
        console.warn(chalk.yellow(`⚠️ Warning: Error rate (${results.http_req_failed.rate * 100}%) exceeds SLA (${PERFORMANCE_SLA.errorRate * 100}%)`));
      }
      
    } catch (error) {
      throw new Error(`Load test failed for ${script}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
async function runSecurityScan() {
  ...
  try {
    ...
    const totalVulnerabilities = Object.values(vulnerabilities).reduce((sum: number, severity: any) => sum + severity.length, 0);
    
    if (totalVulnerabilities > 0) {
      console.warn(chalk.yellow(`⚠️ Warning: Found ${totalVulnerabilities} vulnerabilities in npm packages`));
      
      // Check for critical vulnerabilities
      const criticalVulnerabilities = vulnerabilities.critical?.length || 0;
      if (criticalVulnerabilities > 0) {
        throw new Error(`Security scan failed: Found ${criticalVulnerabilities} critical vulnerabilities`);
      }
    }
    ...
    if (await commandExists('zap-cli')) {
      ...
      if (zapStdout.includes('High') || zapStdout.includes('Critical')) {
        console.warn(chalk.yellow('⚠️ Warning: ZAP scan found high or critical security issues'));
      }
    } else {
      console.log(chalk.yellow('⚠️ OWASP ZAP not available, skipping dynamic security scan'));
    }
    ...
    if (await commandExists('snyk')) {
      ...
      try {
        const { stdout: snykStdout, stderr: snykStderr } = await execAsync('snyk test --json', { timeout: LONG_TIMEOUT });
        ...
        if (error instanceof Error && 'stdout' in error) {
          ...
          console.warn(chalk.yellow('⚠️ Warning: Snyk found security vulnerabilities'));
        } else {
          console.warn(chalk.yellow(`⚠️ Snyk scan error: ${error instanceof Error ? error.message : String(error)}`));
        }
      }
    } else {
      console.log(chalk.yellow('⚠️ Snyk not available, skipping code security scan'));
    }
...
async function checkDockerHealth() {
  ...
  if (!await commandExists('docker')) {
    console.warn(chalk.yellow('⚠️ Docker not available, skipping Docker health checks'));
    return;
  }
  ...
  try {
    ...
    await execAsync('docker build -t cleanrylie-test .', { timeout: LONG_TIMEOUT });
    
    // Run container with health check
    console.log(chalk.blue('Running container with health check...'));
    await execAsync('docker run --name cleanrylie-test -d --health-cmd "curl -f http://localhost:3000/health || exit 1" --health-interval=5s cleanrylie-test');
    
    // Wait for container to start
    await new Promise(resolve => setTimeout(resolve, 10000));
    
    // Check container health
    const { stdout } = await execAsync('docker inspect --format="{{.State.Health.Status}}" cleanrylie-test');
    ...
    const { stdout: healthLogs } = await execAsync('docker inspect --format="{{json .State.Health}}" cleanrylie-test');
    ...
    throw new Error(`Docker health check failed: ${error instanceof Error ? error.message : String(error)}`);
  } finally {
    // Clean up Docker container
    try {
      await execAsync('docker rm -f cleanrylie-test');
    } catch (error) {
      console.warn(chalk.yellow(`⚠️ Failed to clean up Docker container: ${error instanceof Error ? error.message : String(error)}`));
    }
  }
}
...
async function detectMemoryLeaks() {
  ...
  try {
    ...
    const wsMemoryGrowth = await measureMemoryGrowthDuringOperation(async () => {
    ...
    const cacheMemoryGrowth = await measureMemoryGrowthDuringOperation(async () => {
    ...
    const apiMemoryGrowth = await measureMemoryGrowthDuringOperation(async () => {
    ...
    throw new Error(`Memory leak detection failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function testAPIBackwardsCompatibility() {
  ...
  try {
    ...
    const v1Results = await Promise.all(v1Endpoints.map(async endpoint => {
      try {
    ...
    const oldFormatResults = await Promise.all(oldFormatTests.map(async test => {
      try {
    ...
    const incompatibleEndpoints = v1Results.filter(result => !result.compatible);
    const incompatibleFormats = oldFormatResults.filter(result => !result.compatible);
    
    if (incompatibleEndpoints.length > 0 || incompatibleFormats.length > 0) {
      const issues = [
        ...incompatibleEndpoints.map(e => `V1 endpoint ${e.endpoint} is not compatible`),
        ...incompatibleFormats.map(f => `${f.name} is not compatible`),
      ];
      
      throw new Error(`API backwards compatibility issues detected: ${issues.join(', ')}`);
    }
    ...
    throw new Error(`API backwards compatibility testing failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function validatePerformanceSLAs() {
  ...
  try {
    ...
    const fetch = require('node-fetch');
    const performanceResults: Record<string, { 
      responseTime: number[],
      success: boolean,
      errors: string[],
    }> = {};
      ...
      for (let i = 0; i < 10; i++) {
        try {
          ...
          if (response.status >= 400) {
            ...
            performanceResults[endpoint].errors.push(`Request ${i} failed with status ${response.status}`);
          }
        } catch (error) {
          performanceResults[endpoint].success = false;
          performanceResults[endpoint].errors.push(`Request ${i} failed: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
    ...
    for (const [endpoint, results] of Object.entries(performanceResults)) {
      const responseTimesSorted = [...results.responseTime].sort((a, b) => a - b);
      const avgResponseTime = responseTimesSorted.reduce((sum, time) => sum + time, 0) / responseTimesSorted.length;
      ...
      performanceMetrics[endpoint] = {
        ...
        p95ResponseTime,
        successRate,
        meetsSLA: avgResponseTime <= PERFORMANCE_SLA.responseTime && successRate >= (1 - PERFORMANCE_SLA.errorRate),
      };
    }
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ results: performanceResults, metrics: performanceMetrics }, null, 2)
    );
    ...
    const failedEndpoints = Object.entries(performanceMetrics)
      ...
      .map(([endpoint, metrics]) => `${endpoint} (avg: ${metrics.avgResponseTime.toFixed(2)}ms, p95: ${metrics.p95ResponseTime.toFixed(2)}ms, success: ${metrics.successRate * 100}%)`);
    
    if (failedEndpoints.length > 0) {
      throw new Error(`Performance SLAs not met for endpoints: ${failedEndpoints.join(', ')}`);
    }
    
    console.log(chalk.green('✅ Performance SLAs validated'));
    
    // Save performance details to test results
    testResults.performance.details = { results: performanceResults, metrics: performanceMetrics };
  } catch (error) {
    throw new Error(`Performance SLA validation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
  ...
  try {
    ...
    const reportData = {
      summary: {
        ...
        totalDuration: Object.values(testResults).reduce((sum, result) => sum + (result.endTime - result.startTime), 0) / 1000,
    ...
    fs.writeFileSync(
      path.join(REPORT_DIR, 'test-report.md'),
      markdownReport
    );
    
    // Copy reports to artifacts directory
    fs.cpSync(REPORT_DIR, path.join(ARTIFACTS_DIR, 'reports'), { recursive: true });
    
    console.log(chalk.green('✅ Test report generated'));
    console.log(chalk.blue(`📊 Report available at: ${path.join(REPORT_DIR, 'test-report.html')}`));
  } catch (error) {
...
async function parseCoverageReport(): Promise<{ lines: number, statements: number, functions: number, branches: number }> {
  try {
    ...
    console.warn(chalk.yellow(`⚠️ Failed to parse coverage report: ${error instanceof Error ? error.message : String(error)}`));
    return { lines: 0, statements: 0, functions: 0, branches: 0 };
  }
}
...
function parseK6Results(output: string): any {
  try {
    ...
    const jsonMatch = output.match(/(?<=data_received.*\n)(.+\n)+?(?=checks.*\n)/);
    ...
    const metricRegexes = [
      { name: 'http_req_duration', regex: /http_req_duration\s*:\s*avg=(\d+\.?\d*)ms\s*min=(\d+\.?\d*)ms\s*med=(\d+\.?\d*)ms\s*max=(\d+\.?\d*)ms\s*p\(90\)=(\d+\.?\d*)ms\s*p\(95\)=(\d+\.?\d*)ms/ },
    ...
    console.warn(chalk.yellow(`⚠️ Failed to parse k6 results: ${error instanceof Error ? error.message : String(error)}`));
    return {};
  }
}
...
async function measureMemoryGrowthDuringOperation(operation: () => Promise<void>): Promise<number> {
  ...
  console.log(`Memory usage: ${formatBytes(memoryBefore)} -> ${formatBytes(memoryAfter)} (${formatBytes(memoryGrowth)} growth)`);
...
async function notifyCISystem(success: boolean): Promise<void> {
  ...
  if (process.env.CI_WEBHOOK_URL) {
    try {
      ...
      console.warn(chalk.yellow(`⚠️ Failed to notify CI webhook: ${error instanceof Error ? error.message : String(error)}`));
    }
  }
}
...
<body>
  ...
  <h2>Test Details</h2>
  ${Object.entries(data.details).map(([phase, result]) => `
...
function generateJUnitXml(results: any): string {
  ...
  const failedTests = Object.values(results).filter((result: any) => !result.success).length;
