...
const generateTestKey = async (dealershipId: number) => {
  try {
    const response = await fetch(
      `${process.env.BASE_URL || "http://localhost:3000"}/api/dealerships/${dealershipId}/apikeys`,
      {
        method: "POST",
        headers: {
...
const testInboundMessage = async (apiKey: string, dealershipId: number) => {
  ...
  try {
    const response = await fetch(
      `${process.env.BASE_URL || "http://localhost:3000"}/api/inbound`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          ...
          customerMessage:
            "Hi, I saw your ad for the Toyota RAV4. Is it still available?",
          channel: "sms",
...
const testReplyEndpoint = async (apiKey: string, conversationId: number) => {
  ...
  try {
    const response = await fetch(
      `${process.env.BASE_URL || "http://localhost:3000"}/api/reply`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          conversationId,
          message:
            "Does it have all-wheel drive? And what color options are available?",
        }),
      },
    );
...
const testHandoverEndpoint = async (apiKey: string, conversationId: number) => {
  ...
  try {
    const response = await fetch(
      `${process.env.BASE_URL || "http://localhost:3000"}/api/handover`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
