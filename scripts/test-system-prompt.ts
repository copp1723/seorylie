...
async function processChatMessage(
  ...
  let systemPrompt = DEFAULT_SYSTEM_PROMPT.replace("[ARG-Agent Name]", "Rylie")
...
async function runInteractiveTest() {
  ...
  let customerName = await prompt(
    'Enter customer name (default: "Customer"): ',
  );
...
async function main() {
  ...
  const mode = await prompt(
    "Choose test mode (1 for interactive, 2 for automated): ",
  );
