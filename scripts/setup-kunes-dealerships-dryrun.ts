...
async function dryRunKunesSetup() {
  ...
  console.log('This shows what would be created without touching the database.\n');
  ...
  kunesLocations.forEach((location, index) => {
    ...
    console.log(`   🏙️  Location: ${addressInfo.city}, ${addressInfo.state} ${addressInfo.zip}`);
    ...
    console.log(`         personality: "Friendly and knowledgeable RV sales associate at ${location.name}..."`);
    console.log(`         systemPrompt: "You are ${location.personaName}, a sales associate at ${location.name}..."`);
  ...
  const duplicatePersonas = Array.from(personaCounts.entries()).filter(([persona, count]) => count > 1);
  ...
  const invalidEmails = kunesLocations.filter(location => 
    !location.adfEmail.includes('@localwerksmail.com')
  );
  ...
  kunesLocations.forEach(location => {
    console.log(`"${location.name}","${location.address}","${location.phone}","${location.website}","${location.adfEmail}","${location.personaName}"`);
  });
}
