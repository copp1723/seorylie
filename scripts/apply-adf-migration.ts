...
async function applyAdfMigration(): Promise<void> {
  try {
    const migrationPath = path.join(process.cwd(), 'migrations', '0010_adf_lead_ingestion_system.sql');
...
async function applyAdfSmsMigration(): Promise<void> {
  try {
    const migrationPath = path.join(process.cwd(), 'migrations', '0011_adf_sms_responses_table.sql');
    let migrationContent = fs.readFileSync(migrationPath, 'utf-8');
