// Production server with mock database for alpha testing
require('dotenv').config();
const http = require('http');
const { Pool } = require('./db-mock'); // Use mock database
const fs = require('fs');
...
const db = new Pool({
  connectionString: process.env.DATABASE_URL || 'mock://localhost/rylie_seo_dev'
});

// Test database connection on startup
db.query('SELECT NOW() as current_time').then(result => {
  console.log('✅ Connected to mock database at:', result.rows[0].current_time);
}).catch(err => {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/health') {
      ...
      res.writeHead(200);
      res.end(JSON.stringify({ 
        message: 'Rylie SEO API with Mock Database',
        port: PORT,
        endpoints: {
          health: '/health',
      ...
      res.end(JSON.stringify({ 
        ...
        properties: [{
          ...
          property_id: '493777160',
          property_name: 'Alpha Test Ford GA4',
          measurement_id: 'G-XXXXXXXXXX',
          website_url: 'https://alphatestford.com',
          is_active: true
        }]
      }));
      ...
      res.end(JSON.stringify({ 
        ...
        task_id: Date.now(),
        external_id: data.id || `webhook-${Date.now()}`
      }));
        ...
        weekly_summary: [
          {
            ...
            total_tasks: 3,
            total_completed: 2,
            avg_hours_to_complete: 48,
            completion_rate: 66.7
          }
        ],
        current_week_breakdown: [
          { task_type: 'blog_post', count: 2, completed: 2 },
          { task_type: 'landing_page', count: 1, completed: 0 }
        ],
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`✅ Using mock database for alpha testing`);
  console.log(`✅ Demo login: demo@alphatestford.com / demo123`);
});