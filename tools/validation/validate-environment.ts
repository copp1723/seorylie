...
class EnvironmentValidator {
    ...
    const requiredVars = [
      ...
      },
      {
        ...
        validator: (value: string) => {
          ...
          if (value.length < 50) return 'Too short for a valid SendGrid API key';
          if (value.includes('your-') || value.includes('change-me')) {
            return 'Must not contain placeholder values';
          }
          return null;
      ...
      requiredVars.push(
        {
          name: 'REDIS_URL',
          validator: (value: string) => {
            if (!this.isValidUrl(value) && !value.startsWith('redis://') && !value.startsWith('rediss://')) {
  ...
  validateOptionalVariables(): void {
    ...
    const optionalVars = [
      { name: 'PORT', validator: (v: string) => isNaN(parseInt(v)) ? 'Must be a number' : null },
      { name: 'LOG_LEVEL', validator: (v: string) => !['debug', 'info', 'warn', 'error'].includes(v) ? 'Must be debug, info, warn, or error' : null },
      { name: 'REDIS_HOST', validator: (v: string) => v.includes(' ') ? 'Invalid hostname format' : null },
      { name: 'REDIS_PORT', validator: (v: string) => isNaN(parseInt(v)) || parseInt(v) < 1 || parseInt(v) > 65535 ? 'Must be a valid port number' : null },
      { name: 'TWILIO_ACCOUNT_SID', validator: (v: string) => !v.startsWith('AC') ? 'Must start with "AC"' : null },
      { name: 'TWILIO_AUTH_TOKEN', validator: (v: string) => v.length < 20 ? 'Too short for valid Twilio auth token' : null },
      { name: 'FRONTEND_URL', validator: (v: string) => !this.isValidUrl(v) ? 'Must be a valid URL' : null },
      { name: 'SMTP_PORT', validator: (v: string) => isNaN(parseInt(v)) ? 'Must be a number' : null },
      { name: 'EMAIL_MAX_RETRIES', validator: (v: string) => isNaN(parseInt(v)) ? 'Must be a number' : null }
    ];
    ...
    for (const varConfig of optionalVars) {
      const value = process.env[varConfig.name];
      if (value) {
        if (value.includes('your-') || value.includes('change-me') || value.includes('placeholder')) {
    ...
    if (invalid.length === 0) {
      this.addResult(
        ...
        configured.length > 0 ? configured : ['No optional variables configured']
      );
    } else {
      this.addResult(
        'Optional Variables',
  ...
  validateEnvironmentSettings(): void {
    ...
    if (nodeEnv === 'production') {
      ...
      if (process.env.DISABLE_SSL === 'true' || process.env.SSL_DISABLED === 'true') {
        securityIssues.push('SSL is disabled');
      }

      const sessionSecret = process.env.SESSION_SECRET || '';
      if (sessionSecret.includes('default') || sessionSecret.includes('change-me') || sessionSecret.length < 64) {
        securityIssues.push('Session secret is weak or contains default values');
      }
  ...
  async validateDatabaseConnection(): Promise<void> {
    if (this.options.skipConnectionTests) {
      this.addResult('Database Connection', 'warning', 'Skipped (connection tests disabled)');
      return;
    }
    ...
    if (!databaseUrl) {
      this.addResult('Database Connection', 'fail', 'DATABASE_URL not configured', undefined, true);
      return;
    }
    ...
    try {
      ...
      if (isReachable) {
        ...
        this.addResult(
          ...
          ['Check if database server is running', 'Verify network connectivity', 'Check firewall settings'],
          true
        );
      }
    } catch (error) {
  ...
  async validateRedisConnection(): Promise<void> {
    if (this.options.skipConnectionTests) {
      this.addResult('Redis Connection', 'warning', 'Skipped (connection tests disabled)');
      return;
    }
    ...
    if (!redisUrl && this.options.environment === 'production') {
      this.addResult('Redis Connection', 'fail', 'REDIS_URL not configured for production', undefined, true);
      return;
    }

    if (!redisUrl) {
      this.addResult('Redis Connection', 'warning', 'Redis not configured (optional for development)');
      return;
    }
    ...
    try {
      ...
      if (isReachable) {
        this.addResult('Redis Connection', 'pass', `Redis at ${host}:${port} is reachable`);
      } else {
        const status = this.options.environment === 'production' ? 'fail' : 'warning';
        this.addResult(
          'Redis Connection',
          status,
   ...
   * Test TCP connection with timeout
   */
  private testConnection(host: string, port: number, timeout: number = 5000): Promise<boolean> {
  ...
  validateFileSystem(): void {
    ...
    const requiredPaths = [
      ...
      { path: './dist', description: 'Build output directory', createIfMissing: true },
      { path: './uploads', description: 'File uploads directory', createIfMissing: true }
    ];
    ...
    for (const { path, description, createIfMissing } of requiredPaths) {
      if (existsSync(path)) {
        ...
        try {
          ...
          issues.push(`Cannot create ${description}: ${error instanceof Error ? error.message : String(error)}`);
        }
      } else {
    ...
    if (issues.length === 0) {
      this.addResult('File System', 'pass', 'All required paths are accessible', verified);
    } else {
      this.addResult('File System', 'warning', 'Some file system issues detected', issues);
    }
  }
  ...
  printResults(): { hasFailures: boolean; hasWarnings: boolean } {
    ...
    if (hasFailures) {
      ...
      if (criticalFailures.length > 0) {
        console.log(chalk.red.bold(`   ${criticalFailures.length} critical issues must be resolved before deployment`));
      ...
      console.log(chalk.yellow('   Environment is functional but some optimizations are recommended'));
    } else {
      console.log(chalk.green.bold('✅ VALIDATION PASSED'));
      console.log(chalk.green('   Environment is properly configured for deployment'));
    }
   ...
   * Run all validations
   */
  async runAll(): Promise<{ exitCode: number; hasFailures: boolean; hasWarnings: boolean }> {
    console.log(chalk.bold.blue(`🚀 Starting Environment Validation (${this.options.environment} mode)\\n`));
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const options: ValidationOptions = {
    environment: args.includes('--production') ? 'production' : 
