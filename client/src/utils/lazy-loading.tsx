...
export function enhancedLazy<T extends ComponentType<any>>(
  ...
  if (!disableCache && componentCache.has(id)) {
    return componentCache.get(id)!.component as LazyExoticComponent<T> & {
    ...
    while (attempts <= retries) {
      try {
        ...
        performanceMetrics.averageLoadTime =
          performanceMetrics.totalLoadTime / performanceMetrics.successfulLoads;
  ...
  const LazyComponent = reactLazy(loadComponent) as LazyExoticComponent<T> & {
    preload: () => Promise<void>;
  };
...
export const preloadComponents = async (
  ...
  return Promise.all(
    importFns.map((importFn) => preloadComponent(importFn, options)),
  );
};
...
export function withLazyLoading<P extends object>(
  ...
  const WrappedComponent: React.FC<P> = (props) => {
    const { startLoading, stopLoading, setProgress, setError, isLoadingKey } =
      useLoading();
    ...
    const handleRetry = useCallback(() => {
      ...
      startLoading(
        loadingKey,
        `Retrying component load (${state.retryCount + 1})...`,
      );
...
export const analyzeBundleSize = (componentId: string, size: number): void => {
