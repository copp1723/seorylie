...
const AgentStudio: React.FC = () => {
  ...
  const [agentConfig, setAgentConfig] = useState<AgentConfig>({
    ...
    prompt:
      "You are a helpful AI assistant designed to provide information and assistance.",
    tools: [],
  });
  ...
  const [selectedSandboxId, setSelectedSandboxId] = useState<number | null>(
    null,
  );
  const [currentSession, setCurrentSession] = useState<SandboxSession | null>(
    null,
  );
  ...
  const fetchTools = async () => {
    try {
      ...
      const watchdogTool = data.tools.find(
        (tool: Tool) => tool.name === "watchdog_analysis",
      );
      if (watchdogTool) {
        setAgentConfig((prev) => ({
          ...prev,
  ...
  const initializeWebSocket = (session: SandboxSession) => {
    ...
    ws.onclose = () => {
      ...
      addConsoleMessage(
        "system",
        "Disconnected from sandbox WebSocket channel",
      );
    };
  ...
  const handleWebSocketMessage = (message: any) => {
    ...
    switch (message.type) {
      ...
      case "agent_response":
        addConsoleMessage(
          "agent",
          message.content || "Agent response received",
        );
        break;
  ...
  const handleToolStream = (message: any) => {
    ...
    switch (streamEvent.type) {
      ...
      case "data":
        addConsoleMessage(
          "tool",
          `Received data from tool: ${message.toolName}`,
        );
        if (streamEvent.data) {
      ...
      case "error":
        addConsoleMessage(
          "error",
          streamEvent.error?.message || "Tool execution error",
        );
        break;
  ...
  const handleToolResponse = (toolName: string, data: any) => {
    ...
    if (toolName === "watchdog_analysis") {
      if (data.insights && Array.isArray(data.insights)) {
        ...
        addConsoleMessage(
          "system",
          `Received ${data.insights.length} insights from watchdog_analysis`,
        );
      } else if (data.success === false) {
        addConsoleMessage(
          "error",
          data.error?.message || "Analytics query failed",
        );
      }
    }
  };
        ...
        if (agentConfig.tools.includes("watchdog_analysis")) {
          setTimeout(() => {
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
              wsRef.current.send(
                JSON.stringify({
                  ...
                  parameters: {
                    ...
                    question:
                      "What are the key sales trends from last quarter?",
                  },
              ...
              addConsoleMessage(
                "system",
                "Sent analytics query to watchdog_analysis tool",
              );
            }
          }, 2000);
        }
                  ...
                  <>
                    <div className="grid gap-1.5 leading-none">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setSelectedSandboxId(Number(e.target.value))
                        }
                      ...
                      <div className="mt-4 space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="text-sm font-medium">
                            {sandboxes.find((s) => s.id === selectedSandboxId)
                              ?.current_hourly_usage || 0}{" "}
                            /
                            {sandboxes.find((s) => s.id === selectedSandboxId)
                              ?.token_limit_per_hour || 0}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">
                ...
                <div className="bg-black rounded-md p-4 h-[400px] overflow-y-auto font-mono text-sm text-white">
                  {consoleMessages.length === 0 ? (
                    <div className="text-gray-400 text-center mt-8">
                      ...
                      <p className="mt-2">
                        Click "Run Agent" to execute the agent and see results
                        here.
                      </p>
                    </div>
                  ) : (
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 rounded-md p-8 h-[400px] flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        Run an agent with the watchdog_analysis tool enabled to
                        see insights here.
                      </p>
                    </div>
                  </div>
                                ...
                                <div className="flex items-center mt-1">
                                  <div className="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700">
                                    <div
                                      ...
                                      style={{
                                        width: `${Math.max(0, Math.min(100, insight.score * 100))}%`,
                                      }}
                                    ></div>
                                  </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Button variant="outline" className="h-auto py-4 justify-start">
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Configure advanced agent parameters
                  </div>
                </div>
              </Button>
              ...
              <Button variant="outline" className="h-auto py-4 justify-start">
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Link to analytics data sources
                  </div>
                </div>
              </Button>
