...
export default function AuditLogs() {
  ...
  const { data: auditLogs, isLoading: isLoadingLogs } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filterDealership)
        queryParams.append("dealershipId", filterDealership.toString());
  ...
  const { data: accessLogs, isLoading: isLoadingAccessLogs } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest<{
        logs: ApiKeyAccessLog[];
        total: number;
      }>(`/api/api-access-logs?${queryParams.toString()}`);
      return response;
    },
  ...
  const renderAuditLogs = () => {
    if (isLoadingLogs) {
      return (
        <div className="flex justify-center my-10">Loading audit logs...</div>
      );
    }
              ...
              <TableRow key={log.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-1.5">
                    <span className="material-icons text-sm">
                      {eventCategoryIcons[
                        log.details.meta
                          ?.eventCategory as keyof typeof eventCategoryIcons
                      ] || "info"}
                    </span>
                ...
                <TableCell>
                  {log.details.meta?.eventCategory ? (
                    <Badge variant="outline">
                      {log.details.meta.eventCategory.replace(/_/g, " ")}
                    </Badge>
                  ) : (
                    "-"
                  )}
                ...
                <TableCell>
                  {log.details.meta?.riskLevel ? (
                    <Badge
                      className={getRiskLevelClass(log.details.meta.riskLevel)}
                      variant="outline"
                    >
                      {log.details.meta.riskLevel}
                    </Badge>
  ...
  const renderApiAccessLogs = () => {
    ...
    if (!accessLogs?.logs || accessLogs.logs.length === 0) {
      return (
        <div className="text-center my-10">
          No API access logs found matching the criteria.
        </div>
      );
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Create a comprehensive security compliance report for a specific
              time period
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={filterDealership?.toString() || ""}
                  onValueChange={(value) =>
                    setFilterDealership(value ? parseInt(value) : null)
                  }
                >
                  <SelectTrigger>
            ...
            <CardContent className="pt-6">
              <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <>
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setFilterType(value === "all_types" ? null : value)
                        }
                      >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="api_key_revoked">
                            API Key Revoked
                          </SelectItem>
                          <SelectItem value="admin_login">
                            Admin Login
                          </SelectItem>
                          <SelectItem value="admin_login_failed">
                            Failed Login
                          </SelectItem>
                          <SelectItem value="user_permission_changed">
                            Permission Change
                          </SelectItem>
                          <SelectItem value="prompt_modified">
                            Prompt Modified
                          </SelectItem>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={filterRisk || "all_levels"}
                        onValueChange={(value) =>
                          setFilterRisk(value === "all_levels" ? null : value)
                        }
                      >
                        <SelectTrigger>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Dealership</label>
                  <Select
                    value={filterDealership?.toString() || "all_dealerships"}
                    onValueChange={(value) =>
                      setFilterDealership(
                        value === "all_dealerships"
                          ? null
                          : value
                            ? parseInt(value)
                            : null,
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="all_dealerships">
                        All Dealerships
                      </SelectItem>
