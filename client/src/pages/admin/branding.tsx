...
const brandingSchema = z.object({
  ...
  primary_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, { message: "Please enter a valid hex color code (e.g., #000000)" }),
  secondary_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, { message: "Please enter a valid hex color code (e.g., #FFFFFF)" }),
  accent_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, { message: "Please enter a valid hex color code (e.g., #4F46E5)" }),
...
const BrandingPage: React.FC = () => {
  ...
  const { data: dealershipsData } = useQuery({
    ...
    queryFn: () => apiRequest<{ dealerships: { id: number, name: string }[] }>('/api/admin/dealerships'),
    enabled: isSuperAdmin,
  });
  
  // Determine which dealership ID to use for queries
  const effectiveDealershipId = isSuperAdmin ? selectedDealershipId : dealershipId;
  ...
  const { 
    ...
    queryFn: () => apiRequest<{ dealership: DealershipBranding }>(
  ...
  useEffect(() => {
    if (brandingData?.dealership) {
      ...
      form.reset({
        ...
        persona_tone: data.persona_tone as "friendly" | "professional" | "casual" | "formal",
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {dealershipsData.dealerships.map((dealership) => (
                    <Card 
                      ...
                      onClick={() => handleDealershipChange(dealership.id.toString())}
                    >
                      <CardHeader className="pb-2">
                        <CardTitle className="text-lg">{dealership.name}</CardTitle>
                      </CardHeader>
                      <CardFooter>
                        <Button variant="outline" size="sm" onClick={() => handleDealershipChange(dealership.id.toString())}>
                          Select
                        </Button>
                      </CardFooter>
                    </Card>
    ...
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-6">
        ...
        {isSuperAdmin && dealershipsData?.dealerships && (
          <Select value={selectedDealershipId?.toString()} onValueChange={handleDealershipChange}>
        ...
        <div className="lg:col-span-3">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Customize how your AI assistant looks, sounds, and interacts with customers
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <Tabs defaultValue="visual" className="w-full">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                Enter the URL of your dealership logo (recommended size: 200x50px)
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                      ...
                      <TabsContent value="persona" className="space-y-4">
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                The name of your AI assistant that customers will see
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                How your AI assistant will sound when talking to customers
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Persona Description (Optional)</FormLabel>
                              <FormControl>
                                <Textarea 
                              ...
                              <FormDescription>
                                Additional information about your AI assistant's persona
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                The first message customers will see when starting a conversation
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                    ...
                    <div className="mt-6 mb-2">
                      ...
                      >
                        ...
                        {previewMode ? "Hide Persona Preview" : "Show Persona Preview"}
                      </Button>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  {chatMessages.map((message, index) => (
                    ...
                    >
                      ...
                      {message.role === 'user' && (
                        <div 
                          className="h-8 w-8 rounded-full flex-shrink-0 flex items-center justify-center text-white bg-gray-500"
                        >
                          <User className="h-4 w-4" />
                        </div>
                      )}
