...
export function useExternalAPIFlags(): UseExternalAPIFlagsResult {
  const [flags, setFlags] = useState<Record<ExternalAPIFlags, boolean>>({} as Record<ExternalAPIFlags, boolean>);
  ...
  const refreshFlags = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'An error occurred while fetching external API flags');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  // Enable a flag
  const enableFlag = useCallback(async (flag: ExternalAPIFlags): Promise<boolean> => {
    try {
      const response = await axios.post(`/api/external-api-flags/${flag}/enable`);
      ...
      setError(err instanceof Error ? err.message : `An error occurred while enabling ${flag}`);
      return false;
    }
  }, []);

  // Disable a flag
  const disableFlag = useCallback(async (flag: ExternalAPIFlags, reason: string): Promise<boolean> => {
    try {
      const response = await axios.post(`/api/external-api-flags/${flag}/disable`, { reason });
      ...
      setError(err instanceof Error ? err.message : `An error occurred while disabling ${flag}`);
      return false;
    }
  }, []);

  // Check if a flag is enabled
  const isFlagEnabled = useCallback((flag: ExternalAPIFlags): boolean => {
