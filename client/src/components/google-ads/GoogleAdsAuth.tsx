...
import { InfoIcon, LinkIcon, UnlinkIcon, RefreshCwIcon, AlertCircleIcon, CheckCircleIcon } from 'lucide-react';
...
const GoogleAdsAuth: React.FC<GoogleAdsAuthProps> = ({
  ...
  const [selectedSandbox, setSelectedSandbox] = useState<number | null>(sandboxId || null);
  const [showSandboxDialog, setShowSandboxDialog] = useState(false);
  const [accountForSandbox, setAccountForSandbox] = useState<GoogleAdsAccount | null>(null);
  ...
  const fetchAccounts = async () => {
    ...
    try {
      const response = await fetch(`/api/ads/accounts?userId=${userId}${dealershipId ? `&dealershipId=${dealershipId}` : ''}`);
  ...
  const initiateOAuth = async () => {
    ...
    try {
      ...
      const authWindow = window.open(url, 'GoogleAdsAuth', 'width=600,height=700');
      ...
      setError(err instanceof Error ? err.message : 'Failed to initiate Google Ads authentication');
      setLinkingAccount(false);
      toast({
        title: "Authentication Error",
        description: "Failed to start Google Ads authentication. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const unlinkAccount = async (accountId: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to unlink Google Ads account');
      toast({
        title: "Error",
  ...
  const refreshAccount = async (accountId: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to refresh Google Ads account');
      toast({
        title: "Error",
  ...
  const associateWithSandbox = async () => {
    ...
    try {
      const response = await fetch(`/api/ads/accounts/${accountForSandbox.id}/sandbox`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error(`Error associating account with sandbox: ${response.statusText}`);
      }
      ...
      setError(err instanceof Error ? err.message : 'Failed to associate account with sandbox');
      toast({
        title: "Error",
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          Connect your Google Ads accounts to enable AI-powered campaign management and analytics.
        </CardDescription>
      </CardHeader>
          ...
          <>
            {accounts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center">
                ...
                <h3 className="text-lg font-medium mb-2">No Google Ads Accounts Connected</h3>
                <p className="text-sm text-muted-foreground mb-4 max-w-md">
                  Connect your Google Ads accounts to enable AI agents to analyze performance and create campaigns.
                </p>
              </div>
            ) : (
              <Table>
                  ...
                  {accounts.map((account) => (
                    <TableRow key={account.id}>
                      <TableCell>
                        ...
                        <div className="text-sm text-muted-foreground">CID: {account.cid}</div>
                      </TableCell>
                      <TableCell>
                        ...
                        {account.sandboxId ? (
                          <div className="flex items-center">
                            <Badge variant="secondary" className="mr-1">
                              {sandboxes.find(s => s.id === account.sandboxId)?.name || `Sandbox ${account.sandboxId}`}
                            </Badge>
                            <Button
                              variant="ghost"
                              size="icon"
      ...
      <CardFooter className="flex justify-between">
        ...
        <Button
          ...
          disabled={linkingAccount || loading}
        >
          {linkingAccount ? (
            <>
            ...
            <>
              ...
              {accounts.length > 0 ? 'Connect Another Account' : 'Connect Google Ads'}
            </>
          )}
        </Button>
      </CardFooter>
      ...
      <Dialog open={showSandboxDialog} onOpenChange={setShowSandboxDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Link this Google Ads account to a sandbox to enable AI agents to use it within that environment.
            </DialogDescription>
          </DialogHeader>
