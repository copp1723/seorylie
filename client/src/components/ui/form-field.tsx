...
export function FormField({
  ...
  const inputProps = {
    ...
    className: cn(
      ...
      // Success styles
      success && !hasError && "border-green-500 focus:border-green-500 focus:ring-green-500/20",
      inputClassName
    ),
  };
...
export const validators = {
  ...
  minLength: (min: number, message?: string) => (value: string) => {
    ...
    return value && value.length < min ? { message: message || defaultMessage } : undefined;
  },
  
  maxLength: (max: number, message?: string) => (value: string) => {
    const defaultMessage = `Must be no more than ${max} characters`;
    return value && value.length > max ? { message: message || defaultMessage } : undefined;
  },
  ...
  phone: (value: string, message = "Please enter a valid phone number") => {
    ...
    return value && !phoneRegex.test(value.replace(/[\s\-\(\)]/g, '')) ? { message } : undefined;
  }
};

// Compose multiple validators
export function composeValidators(...validators: Array<(value: string) => FieldError | undefined>) {
