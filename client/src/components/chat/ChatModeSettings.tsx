...
const defaultAiSettings = {
  systemPrompt: 'You are Rylie, a helpful automotive dealership assistant. Provide friendly, accurate information about our vehicles and services.',
  temperature: 0.7,
  maxTokens: 500
};
...
const ChatModeSettings: React.FC<ChatModeSettingsProps> = ({ 
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/dealerships/${dealershipId}/settings`);
  ...
  const handleAiSettingsChange = (
    ...
    setSettings({
      ...
      aiSettings: {
        ...settings.aiSettings,
        [field]: field === 'temperature' || field === 'maxTokens' 
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent>
        <Tabs defaultValue={settings.mode} onValueChange={(value) => handleModeChange(value as 'rylie_ai' | 'direct_agent')}>
          ...
          <TabsContent value="rylie_ai" className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Configure how the Rylie AI assistant will interact with customers.
              </p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    This is the base instruction that guides how the AI responds to customers.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleAiSettingsChange('temperature', e.target.value)}
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      Higher values (0.7-1.0) make responses more creative, lower values (0.2-0.5) make them more precise.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleAiSettingsChange('maxTokens', e.target.value)}
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      Maximum length of AI responses (in tokens). 500 is typically 350-400 words.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <TabsContent value="direct_agent" className="space-y-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Set when your human agents are available to handle customer chats.
              </p>
              
              <div className="space-y-3">
                {Object.entries(settings.workingHours).map(([day, hours]) => (
                  <div key={day} className="grid grid-cols-[100px_1fr_1fr_100px] gap-4 items-center">
                    <div className="capitalize">{day}</div>
                    <div>
                      <Label htmlFor={`${day}-start`} className="sr-only">Start Time</Label>
                      <Input
                        id={`${day}-start`}
                        type="time"
                        value={hours.start}
                    ...
                    <div>
                      <Label htmlFor={`${day}-end`} className="sr-only">End Time</Label>
                      <Input
                        id={`${day}-end`}
                        type="time"
                        value={hours.end}
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) => handleWorkingHoursChange(day as keyof WorkingHours, 'enabled', e.target.checked)}
                        className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                      />
                      <Label htmlFor={`${day}-enabled`} className="text-sm">Enabled</Label>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-6">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                  />
                  <Label htmlFor="fallbackToAi">Fall back to Rylie AI when agents are unavailable</Label>
                </div>
                <p className="text-xs text-muted-foreground mt-1 ml-6">
                  If enabled, Rylie AI will handle chats outside of working hours or when all agents are busy.
                </p>
              </div>
            </div>
          </TabsContent>
