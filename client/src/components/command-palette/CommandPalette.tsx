...
function fuzzySearch(search: string, commands: Command[]): Command[] {
  ...
  const patterns = terms.map(term => new RegExp(
    term.split('').join('.*?'), 'i'
  ));
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const [storedRecentCommands, setStoredRecentCommands] = useLocalStorage<string[]>(
  ...
  useEffect(() => {
    if (state.isOpen && commandListRef.current) {
      const selectedElement = commandListRef.current.querySelector(`[data-index="${state.selectedIndex}"]`);
      if (selectedElement) {
  ...
  return state.isOpen ? createPortal(
    <div 
      ...
      >
        <div className="command-palette-header">
          <div className="command-palette-search-container">
            {showIcons && (
              <svg className="command-palette-search-icon" width="16" height="16" viewBox="0 0 16 16">
                <path
                  fill="currentColor"
        ...
        <div className="command-palette-content">
          ...
          {!state.isLoading && (
            <div className="command-palette-commands" ref={commandListRef}>
              ...
              {state.filteredCommands.length === 0 && !state.search && showRecentCommands && state.recentCommands.length > 0 && (
                <>
                  <div className="command-palette-category">Recent Commands</div>
                ...
                <>
                  {showCategories && (
                    <div className="command-palette-categories">
                      {Array.from(new Set(state.filteredCommands.map(cmd => cmd.category))).map(category => (
                        <React.Fragment key={category}>
                          <div className="command-palette-category">{category}</div>
                          {state.filteredCommands
                            .filter(cmd => cmd.category === category)
                            .map((command, index) => {
                              const globalIndex = state.filteredCommands.findIndex(cmd => cmd.id === command.id);
                              return (
                                <div
                                  key={command.id}
                                  ...
                                  role="option"
                                  aria-selected={globalIndex === state.selectedIndex}
                                  tabIndex={-1}
                                >
                                  {showIcons && command.icon && (
                                    <div className="command-palette-item-icon">
                                  ...
                                  <div className="command-palette-item-content">
                                    ...
                                    {command.description && (
                                      <div className="command-palette-item-description">
                                        {highlightMatch(command.description, state.search)}
                                      </div>
                                    )}
                                  </div>
                                  {showShortcuts && command.shortcut && (
          ...
          {showPreviews && state.filteredCommands[state.selectedIndex]?.preview && (
