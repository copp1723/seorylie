...
export const AgentDesigner: React.FC<AgentDesignerProps> = ({
  ...
  const toolsByCategory = availableTools.reduce<Record<string, Tool[]>>(
    (acc, tool) => {
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              ...
              {validationErrors.name && (
                <span className="ml-2 text-xs">({validationErrors.name})</span>
              )}
            </Label>
            <Input
              id="agentName"
          ...
          <div>
            <Label
              htmlFor="agentPrompt"
              className={validationErrors.prompt ? "text-destructive" : ""}
            >
              System Prompt
