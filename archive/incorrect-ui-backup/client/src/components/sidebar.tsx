...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const favorites = navItems.filter((item: NavItem) =>
    favoriteItems.includes(item.href),
  );
  const regularItems = navItems.filter(
    (item: NavItem) => !favoriteItems.includes(item.href),
  );
  ...
  return (
    ...
    >
      ...
      <div className="overflow-y-auto h-[calc(100vh-8rem)]">
        {favorites.length > 0 && (
          <>
            <div className={cn("pt-4 pb-2", expanded ? "px-4" : "px-2")}>
              <p
                className={cn(
              ...
              <nav className={cn("space-y-1", expanded ? "px-1" : "px-0")}>
                {favorites.map((item) => (
                  <NavItem
                    key={item.href}
                    item={item}
        ...
        <div className={cn("pt-3 pb-2", expanded ? "px-4" : "px-2")}>
          <p
            className={cn(
              "text-xs uppercase font-medium text-gray-500 dark:text-gray-400 mb-2 transition-opacity",
              expanded ? "opacity-100 px-2" : "opacity-0 text-center",
      ...
      <div className="absolute bottom-0 w-full border-t border-gray-100 dark:border-gray-700 transition-all">
        <div className={cn("py-4", expanded ? "px-4" : "px-2")}>
          <div
            className={cn(
              "flex items-center",
