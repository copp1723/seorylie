 ...
 */
const checkFeatureEnabled = asyncHandler(
 ...
 */
const checkSandboxAccess = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      req.user?.dealershipId !== sandboxData.dealershipId &&
      !req.user?.isAdmin
    ) {
      throw new AppError(
 ...
 */
async function saveSandboxMetadata(
  ...
  try {
    ...
    logger.error("Error saving sandbox metadata", {
...
async function addToPauseHistory(entry: PauseHistoryEntry): Promise<boolean> {
  try {
    ...
    logger.error("Error adding to pause history", {
...
async function updateStateMetrics(): Promise<void> {
  try {
    ...
    const stateCounts = sandboxes.reduce(
      (counts, sandbox) => {
        if (sandbox) {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (
        state &&
        Object.values(SandboxState).includes(state as SandboxState)
      ) {
      ...
      monitoringService.incrementCounter("sandbox_operations_total", 1, [
        "list",
        "success",
      ]);
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      return ResponseHelper.success(
        res,
        results,
        `Paused ${successCount} of ${sandboxIds.length} sandboxes`,
      );
    } catch (error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const duration = seconds + nanoseconds / 1e9;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      return ResponseHelper.success(
        res,
        results,
        `Resumed ${successCount} of ${sandboxIds.length} sandboxes`,
      );
    } catch (error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const duration = seconds + nanoseconds / 1e9;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const history = entries.map(
        (entry) => JSON.parse(entry) as PauseHistoryEntry,
      );
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const updatedSandbox: SandboxMetadata = {
        ...
        autoPauseSettings: {
          enabled:
            autoPauseEnabled ?? sandboxData.autoPauseSettings?.enabled ?? false,
          idleTimeoutMinutes:
            idleTimeoutMinutes ??
            sandboxData.autoPauseSettings?.idleTimeoutMinutes ??
            30,
          maxExecutionTimeMinutes:
            maxExecutionTimeMinutes ??
            sandboxData.autoPauseSettings?.maxExecutionTimeMinutes ??
            60,
          maxMemoryUsageMB:
            maxMemoryUsageMB ??
            sandboxData.autoPauseSettings?.maxMemoryUsageMB ??
            1024,
        },
        lastActivityAt: new Date().toISOString(),
      };
      ...
      return ResponseHelper.success(
        res,
        updatedSandbox,
        "Auto-pause settings updated successfully",
      );
    } catch (error) {
      const [seconds, nanoseconds] = process.hrtime(startTime);
      const duration = seconds + nanoseconds / 1e9;
