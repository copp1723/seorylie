...
export const commandPaletteTourSteps: TourStep[] = [
  {
    ...
    content: (
      <div>
        <p>
          The Command Palette gives you quick access to all platform features
          with just a keyboard shortcut.
        </p>
        <p>
          Press <kbd>⌘K</kbd> (Mac) or <kbd>Ctrl+K</kbd> (Windows/Linux) to open
          it anytime.
        </p>
      </div>
    ),
  ...
  },
  {
    ...
    content: (
      <div>
        <p>
          Try opening the Command Palette now with <kbd>⌘K</kbd> or{" "}
          <kbd>Ctrl+K</kbd>.
        </p>
        <p>
          You can search for any command, navigate to any page, or perform
          actions without using your mouse.
        </p>
      </div>
    ),
    target: ".app-header",
  ...
  },
  {
    ...
    content: (
      <div>
        <p>
          The Command Palette uses fuzzy search to find what you need quickly.
        </p>
        <p>
          Just start typing and results will appear instantly - even with typos!
        </p>
      </div>
    ),
  ...
  },
  {
    ...
    content: (
      <div>
        <p>
          Use <kbd>↑</kbd> and <kbd>↓</kbd> arrows to navigate between results.
        </p>
        <p>
          Press <kbd>Enter</kbd> to select a command or <kbd>Esc</kbd> to close
          the palette.
        </p>
      </div>
    ),
...
export const bulkOperationsTourSteps: TourStep[] = [
  {
    ...
    content: (
      <div>
        <p>
          Bulk Operations let you perform actions on multiple items at once,
  ...
  },
  {
    ...
    content: (
      <div>
        <p>
          Once items are selected, use these action buttons to perform
          operations on all selected items at once.
        </p>
    ...
    onBeforeShow: () => {
      ...
      if (selectedItems.length === 0) {
        ...
        const firstCheckbox = document.querySelector(
          ".bulk-operations-item-checkbox input",
        ) as HTMLInputElement;
        if (firstCheckbox) {
          firstCheckbox.click();
          return true;
...
export const FeatureTour: React.FC<FeatureTourProps> = ({
  ...
  const [tourProgress, setTourProgress] = useLocalStorage<
  ...
  const mergedTheme: TourTheme = useMemo(() => {
    const baseTheme =
      appTheme.mode === "dark" ? defaultThemes.dark : defaultThemes.light;
  ...
  const showStep = useCallback(
    async (stepIndex: number) => {
      ...
      const shouldScroll =
        step.scrollToTarget !== undefined ? step.scrollToTarget : scrollToSteps;
    ...
    <>
      ...
      {targetElement && (currentStepData?.highlight ?? true) && (
        <Highlight
          ...
          disableInteraction={
            currentStepData?.disableInteraction ?? disableInteraction
          }
      ...
      >
        {/* Tooltip Content */}
        <div
          className="feature-tour-tooltip-content"
          style={{ padding: "16px" }}
        >
          {/* Title */}
          <div
          ...
          >
            ...
            >
              {/* Skip Button */}
              {(currentStepData?.showSkip ?? showSkip) &&
                currentStep < steps.length - 1 && (
                  <button
                    type="button"
              ...
              <button
                ...
                aria-label={
                  currentStep < steps.length - 1 ? "Next step" : "Finish tour"
                }
              >
