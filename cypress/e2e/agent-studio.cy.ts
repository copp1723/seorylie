...
describe("Agent Studio", () => {
  ...
  beforeEach(() => {
    ...
    cy.window().then((win) => {
      ...
      win.localStorage.setItem(
        "user",
        JSON.stringify({ id: 1, name: "Test User", role: "admin" }),
      );
    });
      ...
      class MockWebSocket extends EventTarget {
        ...
        send(data: string) {
          ...
          if (parsedData.type === "connect") {
            ...
            parsedData.type === "tool_request" &&
            parsedData.toolName === "watchdog_analysis"
          ) {
  ...
  it("should complete the Agent Studio happy path flow", () => {
    ...
    cy.get('[role="tabpanel"]').contains(
      "Connected to sandbox WebSocket channel",
    );
    ...
    cy.get('[role="tabpanel"]').contains("Starting tool: watchdog_analysis", {
      timeout: 10000,
    });
    cy.get('[role="tabpanel"]').contains(
      "Received data from tool: watchdog_analysis",
      { timeout: 10000 },
    );
    cy.get('[role="tabpanel"]').contains("Completed tool: watchdog_analysis", {
      timeout: 10000,
    });
